...
namespace UnitTestsCore {
	public class InsPlanT {
		...
		public static InsPlan CreateInsPlan(long carrierNum,EnumCobRule cobRule=EnumCobRule.Basic,long feeSched=53,long allowedFeeSched=0,
			long copayFeeSched=0,bool isMedical=false,string groupNum="",string groupName="",string planNote="",string planType="",byte monthRenew=0)
		{
		...
		///<summary>Deletes one InsPlan from the insplan table, even if the insplan is already in use.</summary>
		public static void DeleteInsPlan(long planNum) {
			string command="DELETE FROM insplan WHERE PlanNum="+planNum;
			DataCore.NonQ(command);
		}
	}
}
