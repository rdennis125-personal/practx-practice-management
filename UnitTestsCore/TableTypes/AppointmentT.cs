...
namespace UnitTestsCore {
	public class AppointmentT {
		...
		public static Appointment CreateAppointment(long patNum,DateTime aptDateTime,long opNum,long provNum,long provHyg=0,string pattern="//XXXX//"
			...
			,long appointmentTypeNum=0,long confirmed=0,DateTime dateTimeAskedToArrive=default,string procDescript="",bool isNewPatient=false)
		{
			...
			appointment.ProcDescript=procDescript;
			...
			appointment.IsNewPatient=isNewPatient;
			if(appointment.AptStatus==ApptStatus.Planned) {
				appointment.ItemOrderPlanned=Appointments.GetRefreshedPlannedAppts(appointment.PatNum).Count+1;
			}
		...
		public static AppointmentSearchData CreateScheduleAndOpsForProv(long numOps,long numClinics,long provNum,int daysForSchedule=0,long hygNum=0,
			bool isDynamic=false,bool skipWeekends=true,bool doAssignProviderOps=true)
		{
			AppointmentSearchData appSearchData=new UnitTestsCore.AppointmentSearchData();
			appSearchData.Patient=PatientT.CreatePatient(MethodBase.GetCurrentMethod().Name+"Pat");
			for(int i = 0;i < numOps;i++) {
				Operatory op;
			 if(!doAssignProviderOps) {
					op=OperatoryT.CreateOperatory("abbr"+i,"opName"+i,provDentist:0,provHygienist:hygNum);
				}
				else {
					op=OperatoryT.CreateOperatory("abbr"+i,"opName"+i,provDentist:provNum,provHygienist:hygNum);
				}
				appSearchData.ListOps.Add(op);
			}
			for(int i = 0;i < numClinics;i++) {
		...
		public static Procedure BreakAppointment(Appointment appt,Patient pat,ProcedureCode procCode,double feeOverride,bool isDeleteBrokenProc=false) 
		{
			...
			if(procCode!=null) {
				...
				brokenProcedure.PlaceService=Clinics.GetPlaceService(appt.ClinicNum); 
