...
namespace UnitTestsCore {
	public class PayPlanT {
			...
			bool runService=true,bool doInsertChargeForToday=true)
		{
			...
			if(runService) { 
				...
				while(DateTime_.Today<=DateTime.Today) {
					...
					period++;
					IncrementDateTime(date,frequency,period);
		...
		public static void IncrementDateTime(DateTime date,PayPlanFrequency frequency,int period=0) {
			DateTime dateTimeTemp;
			if(frequency==PayPlanFrequency.Weekly) {
				DateTime_.SetNow(() => date.AddDays(7*period));
			}
			else if(frequency==PayPlanFrequency.EveryOtherWeek) {
				DateTime_.SetNow(() => date.AddDays(14*period));
			}
				...
				DateTime_.SetNow(() => date.AddMonths(1*period));
			}
			else {//quarterly
				DateTime_.SetNow(() => date.AddMonths(3*period));
			}
		}
		...
		public static List<PayPlanCharge> CloseDynamicPaymentPlan(PayPlan payPlan,Family fam) {
			DynamicPaymentPlanModuleData moduleData=PayPlanEdit.GetDynamicPaymentPlanModuleData(payPlan);
			...
			PayPlanEdit.CloseOutDynamicPaymentPlan(PayPlanEdit.GetPayPlanTerms(payPlan,moduleData.ListPayPlanLinks),moduleData,moduleData.PayPlan.IsLocked,0);
		...
		public static PayPlanEdit.PayPlanRecalculationData GetRecalculationData(PayPlan payPlan,Patient pat,Family fam) {
			...
			PayPlanTerms terms=PayPlanEdit.GetPayPlanTerms(payPlan,listPayPlanLinks);
			List<PayPlanCharge> listPayPlanCharges=PayPlanCharges.GetForPayPlan(payPlan.PayPlanNum).FindAll(x=>x.ChargeDate<=DateTime_.Today);
