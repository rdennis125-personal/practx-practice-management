...
namespace UnitTestsCore {
	public class CarrierT {
		...
		///<summary>Deletes a carrier from the carrier table, even if the carrier is already in use.</summary>
		public static void DeleteCarrier(long carrierNum) {
			string command="DELETE FROM carrier WHERE CarrierNum="+carrierNum;
			DataCore.NonQ(command);
			Carriers.RefreshCache();
		}
	}
