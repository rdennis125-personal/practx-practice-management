...
namespace UnitTestsCore {
	public class DiscountPlanT {
		...
		///<summary>Deletes everything from the discountplan table.  Does not truncate the table so that PKs are not reused on accident.</summary>
		public static void ClearDiscountPlanTable() {
			string command="DELETE FROM discountplan WHERE DiscountPlanNum > 0";
