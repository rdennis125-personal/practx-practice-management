...
namespace DataConnectionBase {
	...
	public class DataConnection:IDisposable {
		...
		public static int CommandTimeout {
			...
			set {
				...
				if(!string.IsNullOrEmpty(ServerName)) {
					ConnectionString=BuildSimpleConnectionString(DBtype,ServerName,Database,MysqlUser,MysqlPass,sslCa:SslCa);
				}
			}
		}
		...
		///<summary>Path to the pem file that contains the SslCa for MariaDB SkySQL connections</summary>
		public static string SslCa {
			get {
				if(String.IsNullOrEmpty(_sslCaT)) {
					return _sslCa;
				}
				else {
					return _sslCaT;
		...
		///<summary>Form Local variable. Does not query the database</summary>
		public static string GetSslCa() {
			return SslCa;
		}
		...
		///<summary>The sslCa can be null or empty if not using SSL.</summary>
		public static string BuildSimpleConnectionString(DatabaseType pDbType,string pServer,string pDatabase,string pUserID,string pPassword,string sslCa="") {
			...
			if(String.IsNullOrEmpty(sslCa)) {
				...
				connectStr+=";SslMode=VerifyCA"
				+";SslCa="+sslCa;
			}
			return connectStr;
		}

		private string BuildSimpleConnectionString(string pServer,string pDatabase,string pUserID,string pPassword,string sslCa) {
			return BuildSimpleConnectionString(DBtype,pServer,pDatabase,pUserID,pPassword,sslCa);
		}
		...
		public static string GetLowConnectionString() {
			return BuildSimpleConnectionString(DatabaseType.MySql,ServerName,Database,MysqlUserLow,MysqlPassLow,SslCa);
		}
		...
		public void SetDb(string server,string db,string user,string password,string userLow,string passLow,DatabaseType dbtype,string sslCA="") {
			SetDb(server,db,user,password,userLow,passLow,dbtype,false,sslCA);
		}

		///<summary>This needs to be run every time we switch databases, especially on startup.  Will throw an exception if fails.  Calling class should catch exception.</summary>
		public void SetDb(string server,string db,string user,string password,string userLow,string passLow,DatabaseType dbtype,bool skipValidation,string sslCa="") {
			DBtype=dbtype;
			string connectStr=BuildSimpleConnectionString(server,db,user,password,sslCa);
			string connectStrLow="";
			if(userLow!="") {
				connectStrLow=BuildSimpleConnectionString(server,db,userLow,passLow,sslCa);
			}
		...
		public void SetDbT(string server,string db,string user,string password,string userLow,string passLow,DatabaseType dbtype,string sslCa="") {
			SetDbT(server,db,user,password,userLow,passLow,dbtype,false,sslCa);
		...
		public void SetDbT(string server,string db,string user,string password,string userLow,string passLow,DatabaseType dbtype,bool skipValidation,string sslCa="") {
			_dBtypeT=dbtype;
			string connectStr=BuildSimpleConnectionString(server,db,user,password,sslCa);
			string connectStrLow="";
			if(userLow!="") {
		...
		private bool IsCrashedTable(MySqlException ex,out string tableName) {
			...
			if(ex.ErrorCode!=MySqlErrorCode.CrashedOnUsage && !ex.Message.ToLower().Contains("is marked as crashed and should be repaired")) {
				return false;
			}
