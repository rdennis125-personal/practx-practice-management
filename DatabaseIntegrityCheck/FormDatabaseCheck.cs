	...
	public partial class FormDatabaseCheck:Form {
		...
		private bool DirectoryCopy(string databaseDirectory,string dbName) {
			...
			try {
				...
				if(!HasAvailableSpaceForBackup(dirInfo,files)) {
					MessageBox.Show("There isn't enough space on the drive to make a backup and safely run table repair. Table repair takes at least double the database size in order to run.");
					return false;
				}
		...
		private bool HasAvailableSpaceForBackup(DirectoryInfo dirInfo,FileInfo[] arrayFileInfo) {
			DriveInfo driveInfo=new DriveInfo(dirInfo.Root.FullName);
			long driveTotalFreeSpace=driveInfo.TotalFreeSpace;//TotalFreeSpace is in bytes.
			long backupTotalSpaceNeeded=0;
			for(int i=0;i<arrayFileInfo.Length;i++){
				backupTotalSpaceNeeded+=arrayFileInfo[i].Length;//File size is in bytes.
			}
			if(driveTotalFreeSpace>=backupTotalSpaceNeeded*2) {//Must have at least double the database size of free space on the drive backup will be made to.
				return true;
			}
			return false;
		}
