			...
			for(int i=0;i<listPrograms.Count;i++) {
				...
				if(!DatabaseIntegrities.IsPluginAllowed(listPrograms[i].PluginDllName)){
					...
					if(databaseIntegrity is null){
						...
						frmDatabaseIntegrity.MessageToShow=databaseIntegrity.Message.Replace("[Plugin]",listPrograms[i].PluginDllName);
...
//todo: fix for cloud:
									...
									File.Copy(dllPathVersionCentral,dllPath,overwrite:true);
								}
								catch(Exception e) {
									// Another instance of OD is tying up the file
									// No need to copy file again, already done by instance holding file
