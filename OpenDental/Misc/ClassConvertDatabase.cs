	...
	public partial class ClassConvertDatabase {
		...
		public bool Convert(string fromVersion,string toVersion,bool isSilent,Form currentForm,bool useDynamicMode) {
			...
			if(PrefC.GetString(PrefName.UpdateInProgressOnComputerName)!=""){
				if(PrefC.GetString(PrefName.UpdateInProgressOnComputerName)==ODEnvironment.MachineName){

				}
				else if(PrefC.GetString(PrefName.UpdateInProgressOnComputerName)==Environment.MachineName){//if RDP, servername
					//this exists just for conversion across 21.3. User doing conversion might be on RDP, and old version would have stored server name. 
				}
				else{
					isBlocked=true;
				}
				...
				if(FromVersion < new Version("7.5.17")) {//Insurance Plan schema conversion
					...
					if(FromVersion < new Version("7.5.1")) {
						...
						if(PrefC.GetBoolSilent(PrefName.InsurancePlansShared,true)) {
							form.YNInsPlanConversion_7_5_17_AutoMerge=YN.Yes;
						}
						else {
							form.YNInsPlanConversion_7_5_17_AutoMerge=YN.No;
						}
						form.ShowDialog();
						...
						InsPlanConverstion_7_5_17_AutoMergeYN=form.YNInsPlanConversion_7_5_17_AutoMerge;
					}
