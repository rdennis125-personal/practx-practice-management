...
namespace OpenDental {
	public partial class FormBugSubmissionHashVitals:FormODBase {
		...
		private void FormBugSubmissionHashVitals_Load(object sender,EventArgs e) {
			...
			Enum.GetValues(typeof(GroupingOptions)).OfType<GroupingOptions>().ForEach(x => comboGrouping.Items.Add(x.ToString()));
		...
		private void RefreshGrids(){
			...
			gridHashes.Columns.Clear();
			gridHashes.Columns.Add(new GridColumn("HashNum",100));
			gridHashes.Columns.Add(new GridColumn("BugId",100));
			gridHashes.Columns.Add(new GridColumn("FullHash",100){IsWidthDynamic=true });
			gridHashes.Columns.Add(new GridColumn("PartialHash",100){IsWidthDynamic=true });
