...
namespace OpenDental {
	public partial class FormWebChatSession:FormODBase {
		...
		private void FillMessageThread() {
			_listWebChatMessages=WebChatMessages.GetAllForSessions(_webChatSession.WebChatSessionNum);
			List<SmsThreadMessage> listSmsThreadMessages=new List<SmsThreadMessage>();
			for(int i=0;i<_listWebChatMessages.Count;i++) {
				string strMessageText=_listWebChatMessages[i].MessageText;
				if(_listWebChatMessages[i].MessageType==WebChatMessageType.EndSession) {
				...
				SmsThreadMessage smsThreadMessage=new SmsThreadMessage(_listWebChatMessages[i].WebChatMessageNum.ToString(),
					_listWebChatMessages[i].DateT,
					strMessageText,
					isAlignedLeft:(_listWebChatMessages[i].MessageType==WebChatMessageType.Customer),
					isImportant:false,
					isHighlighted:false,
					SmsDeliveryStatus.None,
					_listWebChatMessages[i].UserName
				);
		...
		private void butAskAi_Click(object sender, EventArgs e) {
			WebChatMessage questionMessage=_listWebChatMessages.LastOrDefault(x => x.MessageType==WebChatMessageType.Customer);
			if(questionMessage==null) {
				MsgBox.Show("No customer messages found.");
				return;
			}
			InputBox inputBox=new InputBox(Lans.g(this,"Confirm AI Chat Prompt"),questionMessage.MessageText);
			inputBox.ShowDialog();
		...
		private void butSearchAndAttach_Click(object sender,EventArgs e) {
			...
			patient=Patients.GetPat(frmPatientSelect.PatNumSelected);
