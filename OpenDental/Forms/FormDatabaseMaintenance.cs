...
namespace OpenDental {
	...
	public partial class FormDatabaseMaintenance:FormODBase {
		...
		private ODThread _threadRunDBM=null;
		...
		private void FillGrid() {
			...
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Break\r\nDown"),40,HorizontalAlignment.Center));
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Explain"),45,HorizontalAlignment.Center));
			gridMain.Columns.Add(new GridColumn(Lan.g(this,"Results"),300){ IsWidthDynamic=true });
			...
			for(int i=0;i<_listMethodInfosNotHiddenOrOld.Count;i++) {
				...
				bool hasExlpain=DatabaseMaintenances.MethodHasExplain(_listMethodInfosNotHiddenOrOld[i]);
		...
		private void FillGridHidden() {
			...
			gridHidden.Columns.Add(new GridColumn(Lan.g(this,"Name"),740));
			gridHidden.Columns.Add(new GridColumn(Lan.g(this,"Explain"),45,HorizontalAlignment.Center));
		...
		private void FillGridOld() {
			...
			gridOld.Columns.Add(new GridColumn(Lan.g(this,"Break\r\nDown"),40,HorizontalAlignment.Center));
			gridOld.Columns.Add(new GridColumn(Lan.g(this,"Explain"),45,HorizontalAlignment.Center));
			gridOld.Columns.Add(new GridColumn(Lan.g(this,"Results"),300){ IsWidthDynamic=true });
			...
			for(int i=0;i<_listMethodInfosOld.Count;i++) {
				...
				bool hasExlpain=DatabaseMaintenances.MethodHasExplain(_listMethodInfosOld[i]);
				row.Cells.Add(hasBreakDown ? "X" : "");
				row.Cells.Add(hasExlpain ? "X" : "");
				row.Cells.Add("");
		...
		private void FillGridTools() {
			...
			gridTools.Columns.Clear();
			gridTools.Columns.Add(new GridColumn(Lan.g(this,"Name"),300,GridSortingStrategy.StringCompare));
			gridTools.Columns.Add(new GridColumn(Lan.g(this,"Description"),300,GridSortingStrategy.StringCompare));
			...
			row=new GridRow(Lan.g(this,"Balance Families"),Lan.g(this,"Launches the Family Balancer window for all familes."));
			row.Tag=new Action(() => FamilyBalanceFix(isSingleFamilyMode:false));
			gridTools.ListGridRows.Add(row);
			row=new GridRow(Lan.g(this,"Balance Family"),Lan.g(this,"Launches the Family Balancer window for the family of the currently selected patient."));
			row.Tag=new Action(() => FamilyBalanceFix(isSingleFamilyMode:true));
			gridTools.ListGridRows.Add(row);
		...
		private List<MethodInfo> GetDbmMethodsForGrid(bool isHidden=false,bool isOld=false) {
			List<string> listMethodNames=_listDatabaseMaintenances.FindAll(x => x.IsHidden==isHidden && x.IsOld==isOld)
				...
				.ToList();
			return _listMethodInfosAll.FindAll(x => listMethodNames.Contains(x.Name));
		}
		...
		private void hideToolStripMenuItem_Click(object sender,EventArgs e) {
			...
			if(tabControlDBM.SelectedTab==tabChecks) {
		...
		private void ApptProcsFix() {
			...
			Cursor=Cursors.WaitCursor;
			List<Appointment> listAppointmentsForPeriod=Appointments.GetForPeriod(dateTStart:DateTime.Now.Date.AddMonths(-6),dateTEnd:DateTime.MaxValue.AddDays(-10)).ToList();
		...
		private void FamilyBalanceFix(bool isSingleFamilyMode=false) {
			...
			if(isSingleFamilyMode) {
				if(FormOpenDental.PatNumCur<=0) {
					MsgBox.Show(this,"Please have a patient selected before trying to use this tool.");
					return;
				}
				formFamilyBalancer.PatNum=FormOpenDental.PatNumCur;
			}
		...
		private void Run(GridOD grid,DbmMode dbmModeCur) {
			if(grid.ListGridRows.Count > 0 && grid.Columns.Count < 3) {//Enforce the requirement of having the Results column as the third column.
			...
			_isCancelled=false;
			Cursor=Cursors.WaitCursor;
			int colResultsIndex=3;
			if(grid==gridOld && checkShowHidden.Checked) {
				colResultsIndex=4;//There is an extra "Hidden" column to account for when setting the "Results" column.
			}
			...
			progressOD.ActionMain=() => result=DatabaseMaintenances.MySQLTables(isVerbose,dbmModeCur);
			progressOD.ShowDialog();
			if(progressOD.IsCancelled){
				Cursor=Cursors.Default;
				ToggleUI(false);
			...
			stringBuilderLogText.Append(result.Msg);
			//No database maintenance methods should be run unless this passes.
			if(!result.IsSuccess) {
				Cursor=Cursors.Default;
				MsgBoxCopyPaste msgBoxCopyPaste=new MsgBoxCopyPaste(result.Msg);//result is already translated.
				msgBoxCopyPaste.Show();//Let this window be non-modal so that they can keep it open while they fix their problems.
				ToggleUI(false);
				return;
			}
		...
		private void butExplain_Click(object sender,EventArgs e) {
			GridOD gridCur=gridOld;
			if(tabControlDBM.SelectedTab==tabChecks) {
				gridCur=gridMain;
			}
			else if(tabControlDBM.SelectedTab==tabHidden) {
				gridCur=gridHidden;
			}
			MethodInfo methodInfo=gridCur.SelectedTag<MethodInfo>();
			if(gridCur.SelectedIndices.Count()>1 || methodInfo==null) {
				MsgBox.Show(this,"You must select a single method to view explanation");
					return;
			}
			if(!DatabaseMaintenances.MethodHasExplain(methodInfo)) {
				MsgBox.Show(this,"Selected Method does not have an explanation");
				return;
			}
			DbmInfo dbminfo=DbmInfo.ListDbmInfos.Where(x=>x.MethodName.Contains(methodInfo.Name)).FirstOrDefault();
			string whyItHappensString="";
			if(dbminfo!=null) {
				if(!dbminfo.WhyItHappens.IsNullOrEmpty()) {;
					whyItHappensString = $"\r\n\r\nWhy It Happens:\r\n{dbminfo.WhyItHappens}";
				}
				MsgBoxCopyPaste msgBoxCopyPaste=new MsgBoxCopyPaste($"Explanation:\r\n{dbminfo.Explanation}\r\n\r\nManual Fix:\r\n{dbminfo.ManualFix}{whyItHappensString}");
