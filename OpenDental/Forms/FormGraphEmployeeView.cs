	...
	public partial class FormGraphEmployeeView : FormODBase {
		...
		private void GridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(phoneGraph==null){
				...
				phoneGraph.DateEntry=dateEntry;
			...
			formPhoneGraphEdit.ListSchedulesEmp=_listSchedulesEmpCur
				.FindAll(x=>x.EmployeeNum==phoneEmpDefault.EmployeeNum && x.SchedDate==dateEntry)
		...
		private void FillGrid() {
			...
			for(int i=0; i<=timeSpanBetween.Days;i++) {
				...
				row.Cells.Add(cell);
				List<Schedule> listScheduleEmp=_listSchedulesEmpCur
					.FindAll(x => x.SchedDate==dateTimeCur)
					.OrderBy(x => x.StartTime).ToList();
		...
		private void PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			int yPos=marginBounds.Top;
			#region printHeading
			if(!_isHeadingPrinted) {
				text=listBoxEmployees.GetSelected<Employee>().FName+"'s Schedule";
				g.DrawString(text,fontHeading,Brushes.Black,550-g.MeasureString(text,fontHeading).Width/2,yPos);
			...
			#endregion
			yPos=gridMain.PrintPage(g,_pagesPrinted,marginBounds,_heightHeadingPrint);
			_pagesPrinted++;
