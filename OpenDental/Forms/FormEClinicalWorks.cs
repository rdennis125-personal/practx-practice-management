	...
	public partial class FormEClinicalWorks:FormODBase {
		...
		private void FillForm(){
			...
			if(HL7Defs.IsExistingHL7Enabled()) {
				...
				textHL7Server.Text=hl7Def.HL7Server;
				textHL7ServiceName.Text=hl7Def.HL7ServiceName;
				textHL7FolderIn.Text=hl7Def.OutgoingFolder;//because these are the opposite of the way they are in the HL7Def
				textHL7FolderOut.Text=hl7Def.IncomingFolder;
				checkQuadAsToothNum.Checked=hl7Def.IsQuadAsToothNum;
			}
			else {
		...
		private void butDiagnostic_Click(object sender,EventArgs e) {
			...
			using FormEcwDiag formEcwDiag=new FormEcwDiag();
