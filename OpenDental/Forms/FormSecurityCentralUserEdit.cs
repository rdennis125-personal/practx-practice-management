...
namespace OpenDental {
	public partial class FormSecurityCentralUserEdit:FormODBase {
		...
		private void FillGrid() {
			List<Userod> listUserodsCEMT=Userods.GetUsersForCEMT();
			gridCentralUsers.BeginUpdate();
			gridCentralUsers.Columns.Clear();
			gridCentralUsers.Columns.Add(new GridColumn());//Only one column, so title is used as the header.
			gridCentralUsers.ListGridRows.Clear();
			for(int i=0;i<listUserodsCEMT.Count;i++) {
				GridRow row=new GridRow();
				row.Cells.Add(listUserodsCEMT[i].UserName);
				row.Tag=listUserodsCEMT[i];
