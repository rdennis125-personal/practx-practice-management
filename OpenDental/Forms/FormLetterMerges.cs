	...
	public partial class FormLetterMerges : FormODBase {
		...
		private void butPrint_Click(object sender, System.EventArgs e) {//TODO: Implement ODprintout pattern
			...
			if(letterMerge.ImageFolder!=0) {//if image folder exist for this letter, save to AtoZ folder
				try {
					...
					word_Application.ActiveDocument.SaveAs(objectFileName);//save the document 
			...
			_wordDocument.Saved=true;
			_wordDocument.Close(ref _objectFalse,ref _objectMissing,ref _objectMissing);
		...
		private void butPreview_Click(object sender, System.EventArgs e) {
			...
			if(letterMerge.ImageFolder!=0) {//if image folder exist for this letter, save to AtoZ folder
				...
				string tempFilePath=ODFileUtils.CreateRandomFile(Path.GetTempPath(),GetFileExtensionForWordDoc(templateFile));
				Object objectFileName=tempFilePath;
				word_Application.ActiveDocument.SaveAs(objectFileName);//save the document to temp location
		...
		private Document SaveToImageFolder(string fileSourcePath,LetterMerge letterMerge) {
			if(letterMerge.ImageFolder==0) {//This shouldn't happen
				return new Document();
			}
			string rawBase64="";
			...
			documentSave.DocCategory=letterMerge.ImageFolder;
			documentSave.Description=letterMerge.Description+documentSave.DocNum;//no extension.
		...
		private void butEditTemplate_Click(object sender, System.EventArgs e) {
			...
			//Open a document.
			Object objectName=templateFile;
			_wordDocument=word_Application.Documents.Open(ref objectName,ref _objectMissing,ref _objectMissing,ref _objectMissing,
				ref _objectMissing,ref _objectMissing,ref _objectMissing,ref _objectMissing,ref _objectMissing,ref _objectMissing,
