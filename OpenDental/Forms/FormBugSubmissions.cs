...
namespace OpenDental {
	public partial class FormBugSubmissions:FormODBase {
		...
		private void FillSubGrid(bool isRefreshNeeded=false,string grouping95="") {
			...
			gridSubs.Columns.Add(new GridColumn("Submitter",140));
			gridSubs.Columns.Add(new GridColumn("Vers.",55,GridSortingStrategy.VersionNumber));
			if(comboGrouping.SelectedIndex==0) {//Group by 'None'
				gridSubs.Columns.Add(new GridColumn("DateTime",75,GridSortingStrategy.DateParse));
			}
			else {
				gridSubs.Columns.Add(new GridColumn("#",30,HorizontalAlignment.Right,GridSortingStrategy.AmountParse));
			}
			gridSubs.Columns.Add(new GridColumn("Flag",50,HorizontalAlignment.Center));
			gridSubs.Columns.Add(new GridColumn("Msg Text",200) { IsWidthDynamic=true });
		...
		private void RefreshFilterGroupSortAndFillGridSubs(bool isRefreshNeeded,string grouping95,Action loadingProgress=null) {
			...
			if(isRefreshNeeded) {
				...
				if(_formBugSubmissionMode.In(FormBugSubmissionMode.ViewOnly,FormBugSubmissionMode.ValidationMode)) {
		...
		private void gridSubs_CellClick(object sender,UI.ODGridClickEventArgs e) {
			...
			if(_formBugSubmissionMode.In(FormBugSubmissionMode.SelectionMode,FormBugSubmissionMode.ValidationMode)) {
				butAddJob.Text="OK";
			}
		}
