...
namespace OpenDental {
	...
	public partial class FormPayment:FormODBase {
		...
		private bool DoShowAccountEntry(AccountEntry accountEntryCharge) {
			...
			List<long> listClinicNumsFiltered=comboClinicOutstandingFilter.GetListSelected<Clinic>().Select(x => x.ClinicNum).ToList();
		...
		private bool DoHighlightPaySplit(PaySplit paySplit) {
			...
			List<long> listClinicNumsFiltered=comboClinicsPaySplitsFilter.GetListSelected<Clinic>().Select(x => x.ClinicNum).ToList();
			if(comboClinicsPaySplitsFilter.IsAllSelected==false && PrefC.HasClinicsEnabled && !listClinicNumsFiltered.Contains(paySplit.ClinicNum)) {
				return false;
			}
			//Charge Amount Filter
		...
		private void FillFilterClinicsChargestreatPlan(bool doPreserveValues) {
			...
			List<long> listClinicNumsSelected=comboClinicOutstandingFilter.GetListSelected<Clinic>().Select(x => x.ClinicNum).ToList();
		...
		private void FillFilterClinicsPaySplits(bool doPreserveValues) {
			...
			List<long> listClinicNumsSelected=comboClinicsPaySplitsFilter.GetListSelected<Clinic>().Select(x => x.ClinicNum).ToList();
			if(!_listClinics.IsNullOrEmpty() && !_listClinics.Any(x => x.Abbr==Lan.g(this,"Unassigned"))) {
				_listClinics.Add(new Clinic() { Abbr=Lan.g(this,"Unassigned") });
			}
