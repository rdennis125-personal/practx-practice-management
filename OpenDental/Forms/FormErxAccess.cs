...
namespace OpenDental {
	...
	public partial class FormErxAccess:FormODBase {
		...
		private void butChangePatNum_Click(object sender,EventArgs e) {
			if(gridProviders.SelectedGridRows.Count==0) {
				MsgBox.Show(this,"You must first select one or more rows to perform this action.");
				return;
			}
			//Open Pat Select Window - get patNum
			FrmPatientSelect frmPatientSelect=new FrmPatientSelect();
			frmPatientSelect.ShowDialog();
			if(frmPatientSelect.IsDialogCancel) {
				return;
			}
			long newPatNum=frmPatientSelect.PatNumSelected;
			if(!MsgBox.Show(this,MsgBoxButtons.YesNo,"This will change the patient to "+Patients.GetNameLF(newPatNum)+" for the selected providers.\r\nAre you sure you want to proceed?")) {
				return;
			}
			//Update selected rows to new patnum
			List<ProviderErx> listProvidersErxNew=new List<ProviderErx>(_listProviderErxs);
			List<string> listAccountIds=new List<string>();
			for(int i=0;i<gridProviders.SelectedGridRows.Count;i++) {
				ProviderErx providerErx=(ProviderErx)gridProviders.SelectedGridRows[i].Tag;
				listProvidersErxNew.Find(x=>x==providerErx).PatNum=newPatNum;
				listAccountIds.Add(providerErx.AccountId);
			}
			//Update DB, make log, then refresh _listProviderERXs
			ProviderErxs.Sync(listProvidersErxNew,_listProviderErxs);
			string logMessage="Updated patnum to "+newPatNum+" for the following providers:\r\n";
			for(int i=0;i<listAccountIds.Count;i++) {
				logMessage+=listAccountIds[i]+"\r\n";
			}
			SecurityLogs.MakeLogEntry(EnumPermType.ProviderEdit,_patient.PatNum,logMessage);
			_listProviderErxs=ProviderErxs.Refresh(_patient.PatNum).FindAll(x => x.ErxType==ErxOption.NewCrop);
			FillProviders();
		}
	}
