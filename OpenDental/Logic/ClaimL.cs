...
namespace OpenDental {
	public class ClaimL {
		...
		public static CreateClaimDataWrapper CreateClaimFromWrapper(bool isVerbose,CreateClaimDataWrapper createClaimDataWrapper
			,bool hasPrimaryClaim=false,bool hasSecondaryClaim=false,bool hasTertiaryClaim=false)
		{
			createClaimDataWrapper.ErrorMessage="";
			if(!Security.IsAuthorized(EnumPermType.ClaimView,true)) {
				LogClaimError(createClaimDataWrapper,Lans.g("Security","Not authorized for")+"\r\n"+GroupPermissions.GetDesc(EnumPermType.ClaimView),isVerbose);
			...
			if(!hasPrimaryClaim) {
				...
				if(isVerbose && claimLeadOrdinal.ClaimNum!=0) {//Only provide the user with the option to cancel the claim if attempting to create a single claim manually.
				...
				else if(claimLeadOrdinal.ClaimNum!=0) {//isVerbose is false, still need to log.
		...
		public static void PromptForPrimaryOrSecondaryClaim(List<ClaimProc> listClaimProcsForClaim,bool isPromptForSecondaryClaim=true) {
			...
			else {//if not prompting for secondary, must be for primary
				claimType="primary";
