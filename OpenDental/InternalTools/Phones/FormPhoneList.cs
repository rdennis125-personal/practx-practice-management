...
namespace OpenDental {
	public partial class FormPhoneList : FormODBase {
		...
		private void checkHideFloaters_CheckedChanged(object sender,EventArgs e) {
			FilterAndSortPhoneList();
			FillGrid();
		}
		...
		private void FilterAndSortPhoneList() {
			...
			for(int i=0;i<_listPhonesAll.Count;i++) {
				...
				else if(checkHideFloaters.Checked && _listPhonesAll[i].EmployeeNum==0) {
					//Do not show phone tiles for phones for floater extensions (extensions not associated with employees) when 'Hide floaters' is checked.
				}
				else {
