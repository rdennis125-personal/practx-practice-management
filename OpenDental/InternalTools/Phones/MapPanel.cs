	...
	public partial class MapPanel:UserControl{
		...
		protected override void OnPaint(PaintEventArgs pe){
			...
			for(int i=0;i<ListMapAreas.Count;i++){
				...
				//if(ListMapAreas[i].Extension==1102 && ListMapAreas[i].Width<6){
		...
		private void MapPanel_MouseDown_Normal(MouseEventArgs e){
			...
			SetToolStripGroupText("menuItemChanSpy","ChanSpy: "+ListMapAreaMores[idx].ChanSpy);
			...
			if(!Security.IsAuthorized(EnumPermType.ChanSpy,true)) {
				menuItemChanSpy.Visible=false;
				menuItemListenToCall.Visible=false;
			}
		...
		private void DrawCubicleLarge(Graphics g,int i){
			...
			//if(ODBuild.IsDebug()){
			//	FitText("Working",font,brushText,rectangleF,stringFormatCenter,g);
			//}
			//else{
		...
		private void DrawLabel(Graphics g,int i){
			...
			float sizeFont=LayoutManager.UnscaleMS(ListMapAreas[i].FontSize/17);
			using Font font=new Font("Calibri",sizeFont,FontStyle.Bold);
			...
			float heightFont=font.GetHeight();
			float heightReduction=2f/17;
			RectangleF rectangleF=new RectangleF((float)ListMapAreas[i].XPos,(float)ListMapAreas[i].YPos,sizeF.Width,heightFont-heightReduction);
			g.FillRectangle(brushBack,rectangleF);
		...
		private void menuItemListenToCall_Click(object sender,EventArgs e) {
			long curUserEmployeeNum=Security.CurUser.EmployeeNum;
			int curUserExtension=OpenDentBusiness.Phones.GetExtensionForEmp(curUserEmployeeNum);
			if(curUserExtension==0) {//user extension could not be found so don't send signal
				return;
			}
			if(_phoneClicked==null) {
				MessageBox.Show("This cubicle is EMPTY, please select a cubicle that is occupied.");
				return;
			}
			MapAreaMore mapAreaMore=ListMapAreaMores.Find(x=>x.EmployeeNum==_phoneClicked.EmployeeNum);
			if(!mapAreaMore.IsPhone) {
				MessageBox.Show("You may not get a call if the person selected is not actually on a call.");
			}
			else {
				MessageBox.Show("A call from 'Anonymous' will come in shortly.");
			}
			PhoneUI.ListenToCall(curUserExtension,_phoneClicked.Extension);
		}
	}
