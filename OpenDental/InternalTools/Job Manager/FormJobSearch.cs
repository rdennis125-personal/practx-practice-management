...
namespace OpenDental {
	public partial class FormJobSearch:FormODBase {
		...
		public FormJobSearch(bool isMultiSelect=false) {
			InitializeComponent();
			InitializeLayoutManager(isLayoutMS:false);//this was set to false to prevent grid from resizing.
			//Not sure why it was resizing, but this fixes it.
		...
		private void FillGridMain() {
			...
			listFeatureRequestNums.RemoveAll(x => _listFeatureRequestsAll.Select(y => y.FeatReqNum).Contains(x));
			...
			listBugIds.RemoveAll(x => _listBugsAll.Select(y => y.BugId).Contains(x));
			...
			gridMain.Columns.Add(new GridColumn("Priority",50,HorizontalAlignment.Center));
			gridMain.Columns.Add(new GridColumn("Phase",85));
			gridMain.Columns.Add(new GridColumn("Category",80));
			gridMain.Columns.Add(new GridColumn("Job Title",160){ IsWidthDynamic=true });
			gridMain.Columns.Add(new GridColumn("Version",80));
			gridMain.Columns.Add(new GridColumn("Est. Version",80));
			gridMain.Columns.Add(new GridColumn("Expert",75));
			gridMain.Columns.Add(new GridColumn("Engineer",75));
			gridMain.Columns.Add(new GridColumn("Est.\r\nHours",60,GridSortingStrategy.AmountParse));
			gridMain.Columns.Add(new GridColumn("Act.\r\nHours",60,GridSortingStrategy.AmountParse));
			gridMain.Columns.Add(new GridColumn("Job\r\nMatch",45,HorizontalAlignment.Center));
			gridMain.Columns.Add(new GridColumn("Bug\r\nMatch",45,HorizontalAlignment.Center));
			gridMain.Columns.Add(new GridColumn("FR\r\nMatch",45,HorizontalAlignment.Center));
