...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlJobEdit:UserControl {
		...
		private void menuItemCustomersGoToChart_Click(object sender,EventArgs e) {
			...
			GlobalFormOpenDental.GoToModule(EnumModuleType.Chart,patNum:gridCustomers.SelectedTag<Patient>().PatNum);
		}
		...
		private void menuItemReviewComplete_Click(object sender,EventArgs e) {
			...
			if(Security.CurUser.UserNum!=review.ReviewerNum) {
				...
				if(FormLO.UserodSimpleSwitch.UserNum!=review.ReviewerNum) {
					return;
				}
			}
		...
		private void FillGridRoles() {
			...
			gridRoles.Columns.Add(new GridColumn("Role",90));
		...
		private void FillGridCustomers() {
			...
			gridCustomers.Columns.Add(new GridColumn("PatNum",50));
			gridCustomers.Columns.Add(new GridColumn("Name",50) { IsWidthDynamic=true });
			gridCustomers.Columns.Add(new GridColumn("BillType",50) { IsWidthDynamic=true });
		...
		private void FillGridWatchers() {
			...
			gridSubscribers.Columns.Add(new GridColumn("",50));
		...
		private void FillGridAppointments() {
			...
			gridAppointments.Columns.Add(new GridColumn("Appt Date",75));
			gridAppointments.Columns.Add(new GridColumn("Provider",75) { IsWidthDynamic=true });
			gridAppointments.Columns.Add(new GridColumn("Op",75) { IsWidthDynamic=true });
		...
		private void FillGridFeatureReq() {
			...
			gridFeatureReq.Columns.Add(new GridColumn("Feat Req Num",150));
		...
		private void FillGridBugs() {
			...
			gridBugs.Columns.Add(new GridColumn("Bug Num (From JRMT)",50));
		...
		private void FillGridWithJobNotes(GridOD grid,List<JobNote> listJobNotes) {
			...
			grid.Columns.Add(new GridColumn(Lan.g(this,"User"),80));
		...
		public void LoadJob(Job job,TreeNode treeNode=null) {
			...
			if(job!=null && job.TopParentNum==0) {
				...
				List<JobShort> listJobTree=Jobs.GetJobShortNoTopParent();
				JobShort jobShort=listJobTree.FirstOrDefault(x => x.JobNum==job.JobNum);
				while(jobShort.ParentNum!=0) {
					jobShort=listJobTree.FirstOrDefault(x => x.JobNum==jobShort.ParentNum);
				}
				listJobTreeOld=Jobs.GetChildJobs(jobShort,listJobTree);
				listJobTreeOld.Add(jobShort);
				Jobs.UpdateTopParentList(jobShort.JobNum,listJobTreeOld.Select(x => x.JobNum).ToList());
				JobManagerCore.AddTreeJobsToList(Jobs.GetMany(listJobTreeOld.Select(x => x.JobNum).ToList()));
		...
		private void TryMoveJobtoJob(TreeNode sourceNode,TreeNode destinationNode) {
			...
			if(destinationNode==null) {
				if(!MsgBox.Show(this,MsgBoxButtons.OKCancel,"Move selected job to be the top level parent?")) {
					return;
				}
				sourceJob.ParentNum=0;
				sourceJob.TopParentNum=sourceJob.JobNum;
			}
			...
			if(sourceJob.TopParentNum!=topParentNumOld) {
				...
				listJobsUpdated.AddRange(Jobs.GetChildJobs(sourceJob,listTopParentJobs));
			}
			for(int i = 0;i<listJobsUpdated.Count();i++) {
				listJobsUpdated[i].TopParentNum=sourceJob.TopParentNum;
			}
			Jobs.UpdateTopParentList(sourceJob.TopParentNum,listJobsUpdated.Select(x => x.JobNum).ToList());
			FillTreeRelated();
			for(int i = 0;i<listJobsUpdated.Count;i++) {
		...
		private bool ValidateJob(Job _jobCur,bool doRequirementCheck=true) {
			...
			if(_jobCur.Category==JobCategory.Bug
				...
				&& _jobCur.PhaseCur.In(JobPhase.Definition,JobPhase.Development))
			{
		...
		private bool NeedsRequirementReapproval() {
			if(JobPermissions.IsAuthorized(JobPerm.Approval,true)       //Users with the Approval job permission never need reapproval.
				|| _jobCur.Category.In(JobCategory.Bug,JobCategory.Query,JobCategory.MarketingDesign,JobCategory.Conversion,JobCategory.NeedNoApproval) //Bug, Query, Conversion, and NeedNoApproval jobs do not need requirement approval at all.
		...
		private void gridCustomers_TitleAddClick(object sender,EventArgs e) {
			...
			JobLink jobLink = new JobLink() {
				FKey=frmPatientSelect.PatNumSelected,
		...
		private void gridAppointments_TitleAddClick(object sender,EventArgs e) {
			...
			AddAppointment(frmPatientSelect.PatNumSelected);
		}
		...
		private void textDateTested_TextChanged(object sender,EventArgs e) {
			if(_isLoading) {
				return;
			}
			textDateTested.BackColor=Color.FromArgb(255,255,230);//light yellow
			timerTesting.Stop();
			timerTesting.Start();
		}
		...
		private void timerTesting_Tick(object sender,EventArgs e) {
			...
			if(IsNew) {
				...
				textDateTested.BackColor=Color.White;
		...
		private void butTested_Click(object sender,EventArgs e) {
			...
			if(PIn.DateT(textDateTested.Text)==DateTime.MinValue) {
