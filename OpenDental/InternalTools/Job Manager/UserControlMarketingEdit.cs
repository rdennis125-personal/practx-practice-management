...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlMarketingEdit:UserControl {
		...
		private void FillGridRoles() {
			...
			gridRoles.Columns.Add(new GridColumn("",90));
		...
		private void butAddTime_Click(object sender,EventArgs e) {
			...
			if(!JobPermissions.IsAuthorized(JobPerm.MarketingManager,true)
				...
				&& !JobPermissions.IsAuthorized(JobPerm.DocumentationManager,true))
			{
				return;
			}
			AddTime();
		...
		private void textQuoteDate_Leave(object sender,EventArgs e) {
			...
			if(DateTime.TryParse(textQuoteDate.Text,out DateTime date)) {
				Job jobOld=_jobCur.Copy();
		...
		private void textSchedDate_Leave(object sender,EventArgs e) {
			...
			if(DateTime.TryParse(textSchedDate.Text,out DateTime date)) {
				Job jobOld=_jobCur.Copy();
		...
		private void gridTasks_TitleAddClick(object sender,EventArgs e) {
			...
			if(!IsNew && Control.ModifierKeys == Keys.Shift) {
				...
				task.KeyNum=_jobCur.ListJobQuotes.First()?.PatNum??0;
				Tasks.Insert(task);
