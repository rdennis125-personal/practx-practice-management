...
namespace OpenDental.User_Controls.SetupWizard {
	public partial class UserControlSetupWizDefinitions:SetupWizControl {
		...
		private void UserControlSetupWizDefinitions_Load(object sender,EventArgs e) {
			...
			List<DefCatOptions> listDefCatOptionsOrdered = new List<DefCatOptions>();
			listDefCatOptionsOrdered=DefL.GetOptionsForDefCats(listDefCats);
			listDefCatOptionsOrdered = listDefCatOptionsOrdered.OrderBy(x => x.DefCat.GetDescription()).ToList(); //orders alphabetically.
			foreach(DefCatOptions defCOpt in listDefCatOptionsOrdered) {
				listCategory.Items.Add(Lans.g(this,defCOpt.DefCat.GetDescription()),defCOpt);
				if(defCOpt.DefCat==listDefCatOptionsOrdered[0].DefCat) {
		...
		private void ControlDone(object sender, EventArgs e) {
			...
			listDefUpdates.ForEach(x => DefL.Update(x));
