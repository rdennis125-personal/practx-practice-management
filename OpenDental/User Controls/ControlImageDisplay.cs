...
*/
		...
		///<summary>List of Pearl types that are toggled to display. Set as local copy of ControlImages._listPearlCategoriesShown.</summary>
		private List<OpenDentBusiness.Pearl.EnumCategoryOD> _listPearlCategoriesShown=new List<OpenDentBusiness.Pearl.EnumCategoryOD>();
		...
		public async void PrintPDF(string filePath) {
			...
			if(_odWebView2.CoreWebView2==null) {
				_odWebView2=new ODWebView2();
				try {
					await _odWebView2.Init();
					_odWebView2.NavigationCompleted+=odWebView2_NavigationCompleted;
				}
				catch(Exception ex) {
			...
			_odWebView2.ODWebView2Navigate(filePath);
		}
		...
		private async Task<bool> odWebView2_Print() {
			...
			CoreWebView2PrintStatus printStatus=await _odWebView2.CoreWebView2.PrintAsync(printerSettings);
			IsPrinting=false;
			try {
				File.Delete(_odWebView2.Source.AbsolutePath);//delete the temp file so printing doesn't cause their AtoZ folder to bloat.
			}
			catch(Exception ex) {
				ex.DoNothing();
			}
		...
		public void SetShowDrawings(bool showOD,bool showPearl,List<OpenDentBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs){
			_showDrawingsOD=showOD;
			_showDrawingsPearl=showPearl;
			if(!Programs.IsEnabled(ProgramName.Pearl)) {
				_showDrawingsPearl=false;
			}
			_listPearlCategoriesShown=listEnumCategoryODs;
			if(IsBitmapShowing()) {
		...
		private int HitTestHover(PointF pointfBitmap){
			...
			for(int i=0;i<_listImageDraws.Count;i++){
				...
				if(_listImageDraws[i].PearlLayer==OpenDentBusiness.Pearl.EnumCategoryOD.Measurements
					|| !_listPearlCategoriesShown.Contains(_listImageDraws[i].PearlLayer)
				) {
					continue;
				}
				List<PointF> listPointFs=_listImageDraws[i].GetPoints();
