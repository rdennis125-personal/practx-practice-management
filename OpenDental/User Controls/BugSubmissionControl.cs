	...
	public partial class BugSubmissionControl:UserControl {
		...
		private void FillOfficeInfoGrid(BugSubmission sub){
			...
			if(gridOfficeInfo.Columns.Count==0) {
				gridOfficeInfo.Columns.Add(new GridColumn("Field",130));
				gridOfficeInfo.Columns.Add(new GridColumn("Value",125));
			}
		...
		private void SetCustomerInfo(BugSubmission sub,bool isCustomerGridRefresh=true) {
			...
			switch(_groupSelection) {
				case 0:
					...
					gridCustomerSubs.Columns.Add(new GridColumn("Version",100,HorizontalAlignment.Center));
					gridCustomerSubs.Columns.Add(new GridColumn("Count",50,HorizontalAlignment.Center));
				...
				case 5:
					...
					gridCustomerSubs.BeginUpdate();
					gridCustomerSubs.Columns.Clear();
					gridCustomerSubs.Columns.Add(new GridColumn("Vers.",55,HorizontalAlignment.Center));
					gridCustomerSubs.Columns.Add(new GridColumn("RegKey",60,HorizontalAlignment.Center){ IsWidthDynamic=true });
					gridCustomerSubs.ListGridRows.Clear();
					_listSubs.ForEach(x => { 
						Patient rowPat;
		...
		private void butGoToAccount_Click(object sender,EventArgs e) {
			...
			GlobalFormOpenDental.GoToModule(EnumModuleType.Account,patNum:_patCur.PatNum);
		}
