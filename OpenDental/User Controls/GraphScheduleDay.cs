...
namespace OpenDental {
	...
	public partial class GraphScheduleDay:UserControl {
		...
		private int GetPanel1GraphHeight() {
			return Math.Max(panel1.Height,_schedulesList.Count*(BarSpacingPixels+LayoutManager.Scale(barHeightPixels96)));
		}
		...
		private float GetFontHeight() {
			...
			while(true) {
				...
				if((size.Height+.2f)<LayoutManager.Scale(barHeightPixels96)) { //still too small so increment the font size and try again
					emSize+=.1F;
					continue;
				}
				return emSize;		
		...
		private void panel1_Paint(object sender,PaintEventArgs e) {
			...
			try {
				...
				RectangleF rcBounds=new RectangleF(penLine.Width/2,penLine.Width/2,panel1.Width-penLine.Width,GetPanel1GraphHeight()+penLine.Width);
				//fill the background
				...
				for(int i=0;i<_schedulesList.Count;i++) {
					...
					for(int j=0;j<schedules.Count;j++) {
						...
						using(Brush brushBar=new SolidBrush(barColor)) {
							e.Graphics.FillRectangle(brushBar,xPosStart,yPosStart,xWidth,LayoutManager.Scale(barHeightPixels96));
						}
						using(Brush brushNoteText=new SolidBrush(textColor)) {
							e.Graphics.DrawString(desc,font,brushNoteText,xPosStart+2,yPosStart);
						}
					}
					yPosStart+=LayoutManager.Scale(barHeightPixels96);
				}
		...
		private void panel2_Paint(object sender,PaintEventArgs e) {
			...
			try {
				...
				RectangleF rcBounds=new RectangleF(penLine.Width/2,penLine.Width/2,panel2.Width-penLine.Width,panel2.Height-penLine.Width);
