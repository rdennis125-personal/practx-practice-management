...
namespace OpenDental {
	...
	public partial class DashIndividualDiscount:UserControl,IDashWidgetField {
		...
		public void RefreshDiscountPlan(Patient pat,DiscountPlanSub discountPlanSub,DiscountPlan discountPlan) {
			...
			DateTime dateEffective=DiscountPlanSubs.GetAnnualMaxDateEffective(_discountPlanSub.DateEffective);
			DateTime dateTerm=DiscountPlanSubs.GetAnnualMaxDateTerm(_discountPlanSub.DateTerm);
			if(DateTime.Now<dateEffective || DateTime.Now>dateTerm) {
				return;
			}
			DateTime dateEffectiveFinal=DiscountPlanSubs.GetDateEffectiveForAnnualDateRangeSegment(DateTime.Now,dateEffective,dateTerm);
			DateTime dateTermFinal=DiscountPlanSubs.GetDateTermForAnnualDateRangeSegment(DateTime.Now,dateEffective,dateTerm);
