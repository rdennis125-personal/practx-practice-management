...
namespace OpenDental{
	...
	public partial class FormOpenDental:FormODBase {
		...
		private bool PrefsStartup(bool isSilentUpdate,ChooseDatabaseInfo chooseDatabaseInfo){
			...
			//Check to see if this is a backup that needs it's indexes re-enabled. This check takes milliseconds if there are no indexes to re-enable.
			Shared.EnableIndexesIfNeeded(isSilentUpdate);
	...
	private void TaskGoTo(TaskObjectType taskObjectType,long keyNum){
		...
		private void menuItemAiChatSessions_Click(object sender, EventArgs e) {
			if(_formAiSession==null || _formAiSession.IsDisposed) {
				_formAiSession=new FormAiChatSession();
			}
			_formAiSession.Show();
			if(_formAiSession.WindowState==FormWindowState.Minimized) {
				_formAiSession.WindowState=FormWindowState.Normal;
			}
			_formAiSession.BringToFront();
		}
