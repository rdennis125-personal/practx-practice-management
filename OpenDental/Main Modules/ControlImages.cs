...
namespace OpenDental{
	...
	public partial class ControlImages : UserControl{
		...
		private bool _showDrawingsPearl=true;
		...
		private List<OpenDentBusiness.Pearl.EnumCategoryOD> _listPearlCategoriesShown=null;
			...
			if(_listPearlCategoriesShown is null){//happens once on startup
				if(Programs.IsEnabled(ProgramName.Pearl)){
					...
					if(programProperty==null) {
						_listPearlCategoriesShown=new List<OpenDentBusiness.Pearl.EnumCategoryOD>();
					}
					else{
						//Add all categories to list except tooth parts.
						_listPearlCategoriesShown=OpenDentBusiness.Bridges.Pearl.GetAllCategoryODsExceptToothParts();
					}
				}
				else{
					_listPearlCategoriesShown=new List<OpenDentBusiness.Pearl.EnumCategoryOD>();
		...
		private void butPearlLayers_Click(object sender, EventArgs e) {
		...
		private void checkPearlShow_Click(object sender, EventArgs e) {
			_showDrawingsPearl=checkPearlShow.Checked;
			pearl_EventRefreshDisplay(sender,e);
		}

		private void checkPearlToothParts_Click(object sender, EventArgs e) {
			List<OpenDentBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs=OpenDentBusiness.Bridges.Pearl.GetToothPartsCategoryODs();
			if(checkPearlToothParts.Checked) {
				_listPearlCategoriesShown=_listPearlCategoriesShown.Union(listEnumCategoryODs).ToList();
			}
			else {
				_listPearlCategoriesShown=_listPearlCategoriesShown.Except(listEnumCategoryODs).ToList();
			}
			pearl_EventRefreshDisplay(sender,e);
		}
		...
		private void pearl_EventRefreshDisplay(object sender,EventArgs eventArgs) {
			...
			try {
				...
				checkPearlToothParts.Checked=true;
				for(int i=0;i<listEnumCategoryODsToothParts.Count;i++) {
					if(!_listPearlCategoriesShown.Contains(listEnumCategoryODsToothParts[i])) {
		...
		private void ToolBarPrintCategory(object sender, EventArgs e){
			if(ODEnvironment.IsCloudServer || CloudStorage.IsCloudStorage) {
			...
			controlImageDisplayPrint.SetShowDrawings(_showDrawingsOD,_showDrawingsPearl,_listPearlCategoriesShown);
		...
		private void ToolBarProgram_Click(object sender,EventArgs e){
			...
			//All code below this line in this method handles the Pearl program link buttons.
		...
		public void LayoutControls(){
			...
			if(_isTreeDockerCollapsed){
				...
				panelPearl.Visible=ShouldPearlPanelShow();
				...
				if(!panelPearl.Visible) {
					heightPanelPearl=0;
				}
				LayoutManager.Move(panelPearl,new Rectangle(0,Height-heightPanelPearl,LayoutManager.Scale(_widthTree96),heightPanelPearl));
		...
		///<summary>Returns whether the docked Pearl panel should be shown or hidden. Only true when Pearl enabled and Pearl results exist for any currently shown image.</summary>
		private bool ShouldPearlPanelShow() {
			if(!Programs.IsEnabled(ProgramName.Pearl)) {
				return false;
			}
			ControlImageDisplay controlImageDisplay=GetControlImageDisplaySelected();
			...
			//Check for any received PearlRequests for the shown documents.
