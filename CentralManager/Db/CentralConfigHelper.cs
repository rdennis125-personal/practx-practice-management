...
namespace CentralManager {
	public class CentralConfigHelper {
		...
		public static void EnableAutoLogin(string uri,string username,string password) {
			...
			WindowsPasswordVaultWrapper.WritePassword(uri,username,password); //password already used successfully, do not need to validate
		}
		...
		public static void DisableAutoLogin(string uri,string username) {
			string password="";
			password=WindowsPasswordVaultWrapper.RetrievePassword(uri,username);   //hold password to restore if needed
			WindowsPasswordVaultWrapper.ClearCredentials(uri);
			...
			try {
				...
				WindowsPasswordVaultWrapper.WritePassword(uri,username,password);
				throw;
			}
		}
