	...
	public partial class FormCentralManager:Form {
		...
		private void butFilter_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridConns.ListGridRows.Count;i++) {
				...
				FilterConnectionsArgs filterConnectionsArgs=new FilterConnectionsArgs() {
					Connection=conn,
					StrClinic=textClinicSearch.Text,
					StrProvider=textProviderSearch.Text,
				};
				ODThread odThread=new ODThread(ConnectAndFilter,filterConnectionsArgs);
				odThread.Name="SearchThread"+i;
				odThread.GroupName="Search";
				odThread.AddExceptionHandler((ex) => {
					listErrors.Add($"{GetConnectionName(conn)}:\r\n{MiscUtils.GetExceptionText(ex,isUnhandledException:true)}");
				});
			...
			for(int i=0;i<listThreads.Count;i++) {
				...
				if(obj==null) {
					//The thread must have thrown an exception when checking to see if it should have been filtered out or not.
					//Do not remove this connection from the grid so that the user can try again after fixing the problem.
					listConnNums.Add(((FilterConnectionsArgs)listThreads[i].Parameters[0]).Connection.CentralConnectionNum);
					continue;
				}
		...
		private void butRefreshStatuses_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridConns.SelectedIndices.Length;i++) {
				...
				odThread.AddExceptionHandler((ex) => {
					odThread.Tag=new object[] { conn,"OFFLINE: "+StringTools.Truncate(ex.Message,100),eServiceSignalSeverity.None };//Can't connect
				});
				odThread.Start(false);
			}
		...
		private void butSearchPats_Click(object sender,EventArgs e) {
			...
			PtTableSearchParams ptTableSearchParams=new PtTableSearchParams(checkLimit.Checked,textLName.Text,textFName.Text,textPhone.Text,
				...
				SIn.DateT(textBirthdate.Text),0,textSubscriberID.Text,textEmail.Text,textCountry.Text,"","",textClinicPatSearch.Text,"");
			//Find all of the connections that have been checked that match the current connection filter.
			List<CentralConnection> listCentralConnections=_listConnsOK.FindAll(x => GetConnectionName(x).Contains(textConnPatSearch.Text));
			//Loops through all connections passed in and spawn a thread for each to go fetch patient data from each db using the given filters.
			for(int i=0;i<listCentralConnections.Count;i++) {
				//Fire up a thread to go get the patient data table for the search.
				CentralConnection centralConnection=listCentralConnections[i];
				ODThread odThread=new ODThread(GetDataTablePatForConn,new object[]{ centralConnection,ptTableSearchParams.Copy() });
				odThread.GroupName="FetchPats";
				odThread.AddExceptionHandler((ex) => {
					lock(_lockObj) {
						_invalidConnsLog+="\r\n"+GetConnectionName(centralConnection)+": "+Lan.g(this,"An error was encountered while searching for patients.")
...
//todo: this shouldn't be necessary, and I'm wondering if there are other caches that need to be refreshed.
		...
		#endregion Events - Menu Help

		#region Methods
		private void ConnectAndFilter(ODThread odThread) {
			FilterConnectionsArgs filterConnectionsArgs=(FilterConnectionsArgs)odThread.Parameters[0];
			CentralConnection connection=filterConnectionsArgs.Connection;
			...
			bool provMatch=string.IsNullOrWhiteSpace(filterConnectionsArgs.StrProvider);
			for(int i=0;i<listProvs.Count;i++) {
				if(filterConnectionsArgs.StrProvider=="") {
					provMatch=true;
					break;
				}
				if(listProvs[i].Abbr.ToLower().Contains(filterConnectionsArgs.StrProvider.ToLower())
					|| listProvs[i].LName.ToLower().Contains(filterConnectionsArgs.StrProvider.ToLower())
					|| listProvs[i].FName.ToLower().Contains(filterConnectionsArgs.StrProvider.ToLower())) 
				{
					provMatch=true;
					break;
				}
			...
			bool clinMatch=string.IsNullOrWhiteSpace(filterConnectionsArgs.StrClinic);
			for(int i=0;i<listClinics.Count;i++) {
				if(filterConnectionsArgs.StrClinic=="") {
					clinMatch=true;
					break;
				}
				if(listClinics[i].Description.ToLower().Contains(filterConnectionsArgs.StrClinic.ToLower())) {
					clinMatch=true;
					break;
				}
			}
		...
		private bool GetConfigAndConnect() {
			...
			if(_usingMiddleTier) {
				...
				if(_usingAutoLogin) {
					try {
						string password=WindowsPasswordVaultWrapper.RetrievePassword(_middleTierAddr,autoLoginUser);
		...
		private void GetDataTablePatForConn(ODThread odThread) {
			...
			List<DisplayField> fields=DisplayFields.GetForCategory(DisplayFieldCategory.CEMTSearchPatients);
			ptTableSearchParams.HasNextLastVisit=fields.Any(x => x.InternalName.In("NextVisit","LastVisit"));
...
//todo: this might be a bad test for a window being gone
		...
		///<summary>Helper class used to pass information into the ConnectAndFilter thread worker.</summary>
		private class FilterConnectionsArgs {
