...
namespace CentralManager {
	public partial class FormCentralProdInc:Form {
		...
		private void RunDaily() {
			...
			for(int i=0;i<ConnList.Count;i++) {
				...
				odThread.AddExceptionHandler((ex) => { 
					listErrors.Add($"Connection '{CentralConnections.GetConnectionString((CentralConnection)odThread.Parameters[0])}':"
			...
			if(!listErrors.IsNullOrEmpty()) {
				ShowConnectionErrors(listErrors);
				ODThread.QuitAsyncThreadsByGroupName("ConnectAndReportDaily",doRemoveThreads:true);
				return;
			}
		...
		private void RunMonthly() {
			...
			if(!listErrors.IsNullOrEmpty()) {
				ShowConnectionErrors(listErrors);
				ODThread.QuitAsyncThreadsByGroupName("ConnectAndReportMonthly",doRemoveThreads:true);
				return;
			}
		...
		private void RunAnnual() {
			...
			if(!listErrors.IsNullOrEmpty()) {
				ShowConnectionErrors(listErrors);
				ODThread.QuitAsyncThreadsByGroupName("ConnectAndReportAnnual",doRemoveThreads:true);
				return;
			}
		...
		private void RunProvider() {
			...
			if(!listErrors.IsNullOrEmpty()) {
				ShowConnectionErrors(listErrors);
				ODThread.QuitAsyncThreadsByGroupName("ConnectAndReportProvider",doRemoveThreads:true);
				return;
			}
		...
		private void ShowConnectionErrors(List<string> listErrors) {
			using CodeBase.MsgBoxCopyPaste msgBoxCP=new CodeBase.MsgBoxCopyPaste("The following connections had errors getting report data. Please try again.\r\n\r\n"
					+string.Join("\r\n\r\n",listErrors));
			msgBoxCP.ShowDialog();
		}
