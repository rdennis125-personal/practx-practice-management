...
namespace CentralManager{
	...
	public class FormCentralLogOn : System.Windows.Forms.Form{
		...
		private List<string> _listUserNames;
		...
		private void FillListBox(){
			...
			_listUserNames=Userods.GetUserNamesAllNoCache(isHidden:false);
			for(int i=0;i<_listUserNames.Count;i++){
				listUser.Items.Add(_listUserNames[i]);
			}
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			string selectedUsersName=null;
			if(IsMiddleTierSync) {
				selectedUser=new Userod();
				selectedUser.UserName=textUser.Text;
				selectedUser.SetPassword(Authentication.GenerateLoginDetails(textPassword.Text,HashTypes.SHA3_512));
				...
				if(PrefC.GetBool(PrefName.UserNameManualEntry)) {
					...
					if(selectedUsersName.IsNullOrEmpty()) {
						MessageBox.Show(this,"Login failed");
						return;
					}
				}
				else { 
					selectedUsersName=listUser.SelectedItem.ToString();
				}
				try {
					selectedUser=Userods.CheckUserAndPassword(selectedUsersName,textPassword.Text,false);
				}
				catch(Exception ex) {
					MessageBox.Show(ex.Message);
					return;
				}
				if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT && selectedUser.PasswordHash=="" && textPassword.Text=="") {
