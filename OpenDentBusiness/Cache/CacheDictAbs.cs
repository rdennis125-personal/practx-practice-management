...
namespace OpenDentBusiness {
	...
	public abstract class CacheDictAbs<T,KEY_TYPE,VALUE_TYPE> : CacheAbs<T> where T : TableBase {
		...
		///<summary>Clears the current cache by setting the dictionary of all items and the list of short keys to null.</summary>
		public override void ClearCache() {
			if(IsCacheNull()) {
				return;//Cache is already null.
			}
			_lock.EnterWriteLock();
			try {
				//Dictionary caches have a list of keys for getting the 'short' version of the cache which needs to be nulled out as well.
				_listShortKeys=null;
				_dictAllItems=null;
			}
			finally {
