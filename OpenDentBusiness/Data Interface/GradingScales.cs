...
namespace OpenDentBusiness{
	...
	public class GradingScales{
		...
		public static bool IsDupicateDescription(GradingScale gradingScale) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetBool(MethodBase.GetCurrentMethod(),gradingScale);
			}
			string command="SELECT COUNT(*) FROM gradingscale WHERE Description = '"+POut.String(gradingScale.Description)+"' "
				+"AND GradingScaleNum != "+POut.Long(gradingScale.GradingScaleNum);
		...
		public static bool IsInUseByEvaluation(GradingScale gradingScale) {
			...
			string command="SELECT COUNT(*) FROM evaluation,evaluationcriterion "
				+"WHERE evaluation.GradingScaleNum = "+POut.Long(gradingScale.GradingScaleNum)+" "
				+"OR evaluationcriterion.GradingScaleNum = "+POut.Long(gradingScale.GradingScaleNum);
			int count=PIn.Int(Db.GetCount(command));
			if(count>0) {
				return true;
			}
