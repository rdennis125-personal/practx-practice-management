...
namespace OpenDentBusiness{
	...
	public class PatPlans {
		...
		public static int GetOrdinal(PriSecMed priSecMed,List<PatPlan> PatPlanList,List<InsPlan> planList,List<InsSub> subList) {
			Meth.NoCheckMiddleTierRole();
			int dentalOrdinal=0;
			List<PatPlan> listPatPlanOrdered=PatPlanList.OrderBy(x=>x.Ordinal).ToList();
			for(int i=0;i<listPatPlanOrdered.Count;i++) {
				InsSub sub=InsSubs.GetSub(listPatPlanOrdered[i].InsSubNum,subList);
				InsPlan plan=InsPlans.GetPlan(sub.PlanNum,planList);
				if(plan.IsMedical) {
					if(priSecMed==PriSecMed.Medical) {
						return listPatPlanOrdered[i].Ordinal;
					}
				}
				else { //dental
					dentalOrdinal++;
					if(dentalOrdinal==1) {
						...
						if(priSecMed==PriSecMed.Tertiary) {
							return listPatPlanOrdered[i].Ordinal;
						}
					}
				}
...
//Cameron_ Possibly create outbound ADT message to update insurance info
		...
		///<summary>Checks all attached inssubs to make sure they have valid insplans. returns true if list is valid. If it returns false, it fixed the db, and a new list will be needed.</summary>
