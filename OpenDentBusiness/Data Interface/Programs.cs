...
namespace OpenDentBusiness {
	...
	public class Programs{
		...
		public static bool IsDllEnabledByHq(Program progCur) {
			List<HqProgram> listHqPrograms=HqProgram.GetAll().ToList();
			HqProgram hqProgram=listHqPrograms.FirstOrDefault(x => x.PluginDllName.Trim()==progCur.PluginDllName.Trim());
			if(hqProgram==null) {
				return false;
			}
		...
		public static void ScrubExportedPatientData() {
			...
			//EzDenti:
			Program program=Programs.GetCur(ProgramName.EzDenti);
			RemoveLinkageXMLFile(program);
			//Ez3Di:
			program=Programs.GetCur(ProgramName.Ez3Di);
			RemoveLinkageXMLFile(program);
		...
		private static void ScrubFileForProperty(ProgramName programName,string strFileProperty,string strFilePropertySuffix,bool isRemovable) {
			...
			string strFileToScrub=ProgramProperties.GetPropVal(program.ProgramNum,strFileProperty);
			if(!strFilePropertySuffix.IsNullOrEmpty()) {
				strFileToScrub=ODFileUtils.CombinePaths(strFileToScrub,strFilePropertySuffix);
			}
