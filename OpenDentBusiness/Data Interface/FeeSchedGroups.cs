	...
	public class FeeSchedGroups{
		...
		public static FeeSchedGroup GetOneForFeeSchedAndClinic(long feeSchedNum, long clinicNum) {
			...
			string command="SELECT * FROM feeschedgroup"
				...
				+" AND FIND_IN_SET('"+POut.Long(clinicNum)+"',ClinicNums)";//example ClinicNums='23,67,34'. Any match returns >0, which evaluates to true.
		...
		public static List<FeeSchedGroup> GetAllForFeeSched(long feeSchedNum) {
			...
			string command="SELECT * FROM feeschedgroup";
			if(feeSchedNum>0) {
				command+=" WHERE FeeSchedNum="+POut.Long(feeSchedNum);
			}
			return Crud.FeeSchedGroupCrud.SelectMany(command);
		}
		...
		public static void SyncGroupFees(List<Fee> listFeesNew,List<Fee> listFeesDb) {
			...
			List<Fee> listFeesUpsert=new List<Fee>();
			List<Fee> listFeesDel=new List<Fee>();
			listFeesNew.Sort((Fee x,Fee y) => { return x.FeeNum.CompareTo(y.FeeNum); });//Anonymous function, sorts by compairing PK.
			listFeesDb.Sort((Fee x,Fee y) => { return x.FeeNum.CompareTo(y.FeeNum); });//Anonymous function, sorts by compairing PK.
			int idxNew=0;
			int idxDb=0;
			Fee feeNew;
			Fee feeDb;
			...
			while(true) {
				if(idxNew>=listFeesNew.Count && idxDb>=listFeesDb.Count) {
					break;
				}
				feeNew=null;
				if(idxNew<listFeesNew.Count) {
					feeNew=listFeesNew[idxNew];
				}
				feeDb=null;
				if(idxDb<listFeesDb.Count) {
					feeDb=listFeesDb[idxDb];
				}
				...
				if(Crud.FeeCrud.UpdateComparison(feeNew,feeDb)) {
					listFeesUpsert.Add(feeNew);
				}
				idxNew++;
				idxDb++;
