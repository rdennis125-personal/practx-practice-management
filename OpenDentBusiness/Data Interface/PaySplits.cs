...
namespace OpenDentBusiness{
	...
	public class PaySplits {
		...
		public static List<PaySplit> GetPatientData(long patNum) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<PaySplit>>(MethodBase.GetCurrentMethod(),patNum);
			}
			/*This query was too slow
			string command=
				"SELECT DISTINCT paysplit.* FROM paysplit,payment "
		...
		public static bool Sync(List<PaySplit> listNew,long payNum) {
			...
			for(int i=0;i<listNew.Count;i++) {
				...
				PaySplit paySplitOld=listOld.FirstOrDefault(x => listNew[i].SplitNum==x.SplitNum);
				if(paySplitOld!=null) {
		...
		public static void DeleteMany(params long[] arraySplitNums) {
			if(arraySplitNums.IsNullOrEmpty()) {
				return;
			}
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),arraySplitNums);
				return;
			}
			string command=$"DELETE FROM paysplit WHERE SplitNum IN({string.Join(",",arraySplitNums.Select(x => POut.Long(x)))})";
			Db.NonQ(command);
		}
		...
		///<summary>Returns the salted hash for the paysplit. Will return an empty string if the calling program is unable to use CDT.dll. </summary>
		public static string HashFields(PaySplit split) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetString(MethodBase.GetCurrentMethod(),split);
			}
			string unhashedText=split.PatNum.ToString()+split.SplitAmt.ToString("F2")+split.DatePay.ToString("yyyy-MM-dd");
			try {
		...
		///<summary>Validates the hash string in paysplit.SecurityHash. Returns true if it matches the expected hash, otherwise false. </summary>
