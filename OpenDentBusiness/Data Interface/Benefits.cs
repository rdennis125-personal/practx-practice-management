	...
	public class Benefits {
			...
			List<ClaimProcHist> listClaimProcHists,List<ClaimProcHist> listClaimProcHistsLoop,InsPlan insPlan,long patNum) 
		{
			...
			for(int i=0;i<listBenefitsShort.Count;i++){
				...
				if(listBenefitsShort[i].CodeGroupNum>0) {
					continue;
				}
			...
			for(int i=0;i<listBenefitsShort.Count;i++) {
				...
				CodeGroup codeGroup=CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
				if(!codeGroup.ProcCodes.Contains(procCode)) {
					continue;
				}
				...
				if(benefitInd!=null && benefitInd.CodeGroupNum!=0) {
					...
					if(listBenefitsShort[i].MonetaryAmt>benefitInd.MonetaryAmt) {
						benefitInd=listBenefitsShort[i];
					}
				}
				else {
			...
			for(int i=0;i<listBenefitsShort.Count;i++) {
				...
				if(benefitFam!=null && benefitFam.CodeGroupNum!=0) {
					//This should never happen, but for the sake of answering "what if," we defer to the highest deductible
					if(listBenefitsShort[i].MonetaryAmt>benefitFam.MonetaryAmt) {
						benefitFam=listBenefitsShort[i];
					}
				}
				else {
			...
			for(int i=0;i<listClaimProcHistsLoop.Count;i++) {
				...
				else if(benefitInd.CodeGroupNum!=0) {//specific codegroup
					if(!CodeGroups.IsProcInCodeGroup(listClaimProcHistsLoop[i].StrProcCode,benefitInd.CodeGroupNum)) {
						continue;
					}
				}
			...
			for(int i=0;i<listClaimProcHistsLoop.Count;i++) {
				...
				else if(benefitFam.CodeGroupNum!=0) {//specific codegroup
					if(!CodeGroups.IsProcInCodeGroup(listClaimProcHistsLoop[i].StrProcCode,benefitFam.CodeGroupNum)) {
						continue;
					}
				}
		...
		public static double GetLimitationByCode(List<Benefit> listBenefits,long planNum,long patPlanNum,DateTime procDate,
			...
			for(int i=0;i<listBenefitsShort.Count;i++) {
				...
				CodeGroup codeGroup=CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
				if(!codeGroup.ProcCodes.Contains(procCodeStr)) {
					continue;
				}
				//If it's an age based limitation, then make sure the patient age matches.
				//If we have an age match, then we exit the method right here.
			...
			if(benefitInd!=null) {
				...
				for(int i=0;i<listClaimProcHists.Count;i++) {
					...
					if(benefitInd.CodeNum!=0) {//specific code
						...
						CodeGroup codeGroup=CodeGroups.GetOne(benefitInd.CodeGroupNum);
						if(!codeGroup.ProcCodes.Contains(listClaimProcHists[i].StrProcCode)) {
							continue;
						}
					}
					else if(benefitInd.CovCatNum!=0) {//specific category
			...
			if(benefitInd!=null) {
				for(int i=0;i<listClaimProcHistsLoop.Count;i++) {
					...
					if(benefitInd.CodeNum!=0) {//specific code
						...
						CodeGroup codeGroup=CodeGroups.GetOne(benefitInd.CodeGroupNum);
						if(!codeGroup.ProcCodes.Contains(listClaimProcHistsLoop[i].StrProcCode)) {
							continue;
						}
					}
					else if(benefitInd.CovCatNum!=0) {//specific category
			...
			for(int i=0;i<listClaimProcHists.Count;i++) {
				...
				if(benefitFam.CodeNum!=0) {//specific code
					...
					CodeGroup codeGroup=CodeGroups.GetOne(benefitFam.CodeGroupNum);
					if(!codeGroup.ProcCodes.Contains(listClaimProcHists[i].StrProcCode)) {
						continue;
					}
				}
		...
		private static bool TighterLimitExists(List<Benefit> listBenefitsShort,Benefit benefit,ClaimProcHist claimProcHist) 
		{
			...
			for(int b=0;b<listBenefitsShort.Count;b++) {
				...
				if(listBenefitsShort[b].CodeGroupNum!=0) {
					CodeGroup codeGroup=CodeGroups.GetOne(listBenefitsShort[b].CodeGroupNum);
					if(!codeGroup.ProcCodes.Contains(claimProcHist.StrProcCode)) {
		...
		public static int GetPercent(string procCodeStr,string planType,long planNum,long patPlanNum,List<Benefit> listBenefits) {
			...
			for(int i=0;i<listBenefitsShort.Count;i++) {
				...
				CodeGroup codeGroup=CodeGroups.GetOne(listBenefitsShort[i].CodeGroupNum);
				if(!codeGroup.ProcCodes.ToLower().Contains(procCodeStr.ToLower())) {
					continue;
				}
				...
				if(listBenefitsShort[i].PatPlanNum!=0) {
					...
					if(benefitPat!=null && benefitPat.CodeGroupNum!=0) {
						...
						if(CodeGroups.GetOrder(listBenefitsShort[i].CodeGroupNum)>CodeGroups.GetOrder(benefitPat.CodeGroupNum)) {
							benefitPat=listBenefitsShort[i];
						}
					}
					else {
				...
				if(benefitPlan!=null && benefitPlan.CodeGroupNum!=0) {
					//This should never happen, but for the sake of answering "what if," we defer to the higher-order codegroup
					if(CodeGroups.GetOrder(listBenefitsShort[i].CodeGroupNum)>CodeGroups.GetOrder(benefitPlan.CodeGroupNum)) {
						benefitPlan=listBenefitsShort[i];
					}
				}
				else {
			...
			for(int i=0;i<listBenefits.Count;i++) {
				...
				if(listBenefits[i].CodeGroupNum>0) {
					if(CodeGroups.IsProcInCodeGroup(strProcCode,listBenefits[i].CodeGroupNum)) {
						return true;//Codegroup benefit excludes this procedure.
					}
					continue;
				}
				if(listBenefits[i].CovCatNum==0) {
		...
		/// <summary>Returns true if the given benefit represents a diagnostic deductible benefit. Can be either family or individual based on coverageLevel passed in.</summary>
		...
		/// <summary>Returns true if the given benefit represents an xray deductible benefit. Can be either family or individual based on coverageLevel passed in.</summary>
		...
		/// <summary>Returns true if the given benefit represents a preventative deductible benefit. Can be either family or individual based on coverageLevel passed in.</summary>
		...
		/// <summary>Returns true if the given benefit represents a coinsurance benefit for the textboxes in the simplified view of FormInsBenefits.</summary>
		...
		/// <summary>Returns true if the given benefit represents a waiting period benefit for the textboxes in the simplified view of FormInsBenefits.</summary>
		...
		/// <summary>Gets a distinct list of all benfits where their patplannum is in listpatplannumss or their plannum is in listplannums. Returns empty list if no matches are found.</summary>
		public static List<Benefit> GetAllForPlanNumsAndPatPlanNums(List<long> listPlanNums,List<long> listPatPlanNums) {
			...
			if(listPlanNums.IsNullOrEmpty() && listPatPlanNums.IsNullOrEmpty()){
				return new List<Benefit>();
			}
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<Benefit>>(MethodBase.GetCurrentMethod(),listPlanNums,listPatPlanNums);
			}
			string command=$"SELECT * FROM benefit "
				+$"WHERE BenefitType IN ({POut.Enum(InsBenefitType.Exclusions)},{POut.Enum(InsBenefitType.CoInsurance)}) "
