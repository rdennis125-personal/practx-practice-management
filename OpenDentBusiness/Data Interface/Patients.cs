...
namespace OpenDentBusiness {
	...
	public class Patients {
		...
		public static string GetEmailAddressForGuarantorOrPatient(Patient patient) {
			...
			if(patientGuar==null || EmailAddresses.GetValidMailAddress(patientGuar.Email)==null) {
		...
		public static Def GetPatientSpecialtyDef(long patNum) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<Def>(MethodBase.GetCurrentMethod(),patNum);
			}
			string command="SELECT DefNum FROM deflink WHERE LinkType="+POut.Int((int)DefLinkType.Patient)+" AND FKey="+POut.Long(patNum);
			long defNum=Db.GetLong(command);
		...
		public static List<Patient> GetPatientsByPhone(string phoneNumber,string countryCode,List<PhoneType> listPhoneTypes=null) {
			...
			try {
				...
				if(PrefC.GetBool(PrefName.PatientPhoneUsePhonenumberTable)) {//Search the PhoneNumber table
		...
		public static void SetDateBalBegan(long clinicNum,ref List<PatAging> listPatAgingAll,ref List<ClinicBalBegans> listClinicBalBegans) {
			Meth.NoCheckMiddleTierRole();//Uses ref parameters.
		...
		{
			Meth.NoCheckMiddleTierRole();//Private method with arguments that act like out parameters.
		...
		public static List<Patient> GetPatientData(long patNum){
			...
			for(int i=0;i<listPatients.Count;i++) {
				listPatients[i].Age = DateToAge(listPatients[i].Birthdate);
			}
			return listPatients;
		}
			...
			DateTime birthdate,int patStatus,long clinicNum,DateTime dateTStamp,long priProv,int gender,int position,long guarantor,long superFamily,long employerNum)
		{
			...
			List<Patient> listPatients=Crud.PatientCrud.SelectMany(command);
			List<PatientWithServerDT> listPatientForApis=new List<PatientWithServerDT>();
			for(int i=0;i<listPatients.Count;i++) {
				PatientWithServerDT patientForApi=new PatientWithServerDT();
		...
		public static DataTable GetPtDataTable(PtTableSearchParams ptSearchArgs)
		{
			...
			bool usePhonenumTable=PrefC.GetBool(PrefName.PatientPhoneUsePhonenumberTable) && phDigitsTrimmed.Length>0;
			...
			string lNameMobileQueryString="";
			string fNameMobileQueryString="";
			if(ptSearchArgs.IsFromMobile) {
				lNameMobileQueryString+=string.IsNullOrEmpty(ptSearchArgs.LName)?"":@$"AND (
				...
				fNameMobileQueryString+=string.IsNullOrEmpty(ptSearchArgs.FName)?"":@$"AND (
			...
			if(!usePhonenumTable) {
				...
				if(ptSearchArgs.PageNum>1){
					command+=$@" OFFSET {(ptSearchArgs.PageNum-1)*41}";
				}
			}
			...
			foreach(DataRow dRow in arrayRows) {
				...
				DateTime dateAdmit=PIn.Date(dRow["AdmitDate"].ToString());
				DateTime dateDischarge=DateTime.MinValue;
				if(dateAdmit.Year>1880) {
					r["AdmitDate"]=dateAdmit.ToShortDateString();
				}
				long patNum=PIn.Long(r["PatNum"].ToString());
				dateDischarge=listEhrPatients.Find(x=>x.PatNum==patNum)?.DischargeDate??new DateTime();
				if(dateDischarge.Year>1880) {
					r["DischargeDate"]=dateDischarge.ToShortDateString();
				}
		...
		///<summary>Key=patNum, value=formatted name. Stop using this.  Just use GetLimForPats, and then something like listPatients.Find...patient.GetNameLF().</summary>
		[Obsolete()]
		...
		///<summary>Key=PatNum, value=formatted name.  Only allowed to be used in the 1 current place.  This gets all patients in the db, which can be very time consuming.  If you want to convert PatNums to names somewhere else, use Patients.GetLimForPats() followed by ListPatients.Find(...etc.</summary>
		...
		/*
		==Jordan 2023 Considering some options for offices with extremely large numbers of patients,
		but we decided that this would require further optimization followed by stress testing, so not doing it yet.
		At least one customer has millions of patients, and getting them all lockes up their UI.
		So maybe they've just learned not to use FormUserQuery.
		///<summary>Key=PatNum, value=formatted name.  Only allowed to be used in the 1 current place for performance. If you want to convert PatNums to names somewhere else, use Patients.GetLimForPats() followed by ListPatients.Find(...etc.</summary>
		public static Dictionary<long,string> GetDictPatientNames(List<long> listPatNums) {
			Meth.NoCheckMiddleTierRole();
			List<Patient> listPatients=GetLimForPats(listPatNums);
			Dictionary<long,string> dictionaryPatNumName=new Dictionary<long,string>();
			for(int i=0;i<listPatients.Count;i++) {
				dictionaryPatNumName.Add(listPatients[i].PatNum,listPatients[i].GetNameLF());
			}
			return dictionaryPatNumName;
		}*/
		...
		public static void UpdateAddressForFam(Patient pat,bool isSuperFam,bool isAuthArchivedEdit) {
			...
			if(didPhoneChange && PrefC.GetBool(PrefName.PatientPhoneUsePhonenumberTable)) {
			...
			bool excludeNoTil=false,bool excludeNotBilledSince=false,bool isFinanceBilling=false,List<long> listPatNumsToExclude=null)
		{
			...
			string strBalExclude="(ROUND(guar.BalTotal"+strMinusIns+",3) >= ROUND("+POut.Double(excludeLessThan)+",3) OR (guar.PayPlanDue > 0 AND guar.PayPlanDue >= ROUND("+POut.Double(excludeLessThan)+",3))";
			...
			if(isSuperStatements) {
				...
				List<long> listSuperFamilyNumsNeeded=listSuperNums.FindAll(x => !listPatNums.Contains(x)); 
		...
		public static void ReformatAllPhoneNumbers() {
			...
			bool doSyncPhNumTable=PrefC.GetBool(PrefName.PatientPhoneUsePhonenumberTable);
			for(int i=0;i<table.Rows.Count;i++) {
		...
		///<summary>Returns the patient's preferred pronouns. If ShowPreferredPronounsForPats is turned off, assigns He/Him to Male, She/Her to Female,
		///They/Them to Other, and None to Unknown.
		///If turned on and the patient has None set but also has an unknown gender, returns pronounPreferred.None</summary>
		public static PronounPreferred GetPronoun(PatientGender patientGender,PronounPreferred pronounPreferred) {
			...
			if(PrefC.GetBool(PrefName.ShowPreferredPronounsForPats)){
				if(pronounPreferred!=PronounPreferred.None){//they specified a pronoun override
		...
		public static void UpdateProv(long patNum,long provNumNew) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),patNum,provNumNew);
				return;
			}
			string command="UPDATE patient SET PriProv ="+POut.Long(provNumNew)+" WHERE PatNum = "+POut.Long(patNum);
			Db.NonQ(command);
		}
		...
		public static string HashFields(Patient patient) {
			...
			try {
				return CDT.Class1.CreateSaltedHash(unhashedText,isPatientTable:true);
			}
			catch(Exception ex) {
				ex.DoNothing();
		...
		public static List<string> GetCanadianEligibilityCodes() {
			List<string> listCanadianEligibilityCodes=new List<string>();
		...
		public static long GetEmployerNumForPatient(Patient patient,string employerName){
			if(patient.EmployerNum==0){//no employer was previously entered.
				if(employerName==""){
			...
			//if text has changed
			else if(Employers.GetName(patient.EmployerNum)!=employerName){
				return Employers.GetEmployerNum(employerName);
			}
			return patient.EmployerNum;//no change
		}
	...
	public class PatientEditSameForFamily {
		...
		public PatientEditSameForFamily(Patient patient, Family family) {
			...
			List<Patient> listPatientsFamily=family.ListPats.ToList();
			if(!isAuthArchivedEdit) {
			...
			for(int i=0;i<listPatientsFamily.Count;i++){
				...
				if(patient.AddrNote!=listPatientsFamily[i].AddrNote){
				...
				if(patient.AskToArriveEarly!=listPatientsFamily[i].AskToArriveEarly) {
					ArriveEarlySameForFamily=false;
				}
			}
	...
	public class PatComm : WebTypes.WebBase {
			...
			string smsPhoneCountryCode) 
		{
			...
			Email=GetEmailAddresses(PIn.String(dataRow["Email"].ToString()));
		...
		public string GetReasonCantEmail(CommOptOutType type=0) {
			...
			if(EmailAddresses.GetValidMailAddress(Email)==null) {
	...
	public class PatientFor834Import : IComparable {
		...
		public static List<PatientFor834Import> GetPatientLimitedsByNameAndBirthday(Patient patient,List<PatientFor834Import> listSortedPatients) {
			PatientFor834Import pat =new PatientFor834Import(patient);
			if(pat.LName.Trim().ToLower().Length==0 || pat.FName.Trim().ToLower().Length==0 || pat.Birthdate.Year < 1880) {
				//We do not allow a match unless Last Name, First Name, AND birthdate are specified.  Otherwise at match could be meaningless.
