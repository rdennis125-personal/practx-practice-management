...
namespace OpenDentBusiness {
	public class Procedures {
		...
		public static double GetProcFee(Patient pat,List<PatPlan> listPatPlans,List<InsSub> listInsSubs,List<InsPlan> listInsPlans,Procedure procedure,List<Benefit> listBenefits=null,List<Fee> listFees=null)
		{
			...
			if(procedure.DateComplete.Year>1880 && procedure.DateComplete<DateTime.Today && procedure.ProcStatus==ProcStat.C) {
				return procedure.ProcFee;
			}
			double procFeeRet;
			...
			if(PrefC.GetBool(PrefName.MedicalFeeUsedForNewProcs) && !string.IsNullOrEmpty(procedure.MedicalCode)) {
				long feeSch=FeeScheds.GetMedFeeSched(pat,listInsPlans,listPatPlans,listInsSubs,procedure.ProvNum);
				...
				long feeSch=FeeScheds.GetFeeSched(pat,listInsPlans,listPatPlans,listInsSubs,procedure.ProvNum);
			...
			if(insPlanPrimary!=null && insPlanPrimary.PlanType=="p") {//PPO
				double ucrFee=Fees.GetAmount0(procedure.CodeNum,Providers.GetProv(Patients.GetProvNum(pat)).FeeSched,procedure.ClinicNum,procedure.ProvNum,listFees);
			...
			if(priInsPlan!=null && InsPlans.UsesUcrFeeForExclusions(priInsPlan.ExclusionFeeRule)) {
				...
				if(listBenefits==null) {
					listBenefits=Benefits.GetForPlanOrPatPlan(priInsPlan.PlanNum,priPatPlan.PatPlanNum);
				}
		...
		public static double GetDiscountAmountForDiscountPlanAndValidate(Procedure procedure,DiscountPlanSub discountPlanSub=null,DiscountPlan discountPlan=null,double runningTotal=0,List<Procedure> listAddHistProcs=null)
		{
			...
			DateTime dateEffective=DiscountPlanSubs.GetAnnualMaxDateEffective(discountPlanSub.DateEffective);
			DateTime dateTerm=DiscountPlanSubs.GetAnnualMaxDateTerm(discountPlanSub.DateTerm);
			...
			try {
				if(discountPlanSub!=null) {
					discountPlanFrequencyLimits=DiscountPlans.CheckDiscountFrequencyAndValidateDiscountPlanSub(ListTools.FromSingle(procedure),procedure.PatNum,
						procedure.ProcDate,discountPlanSub:discountPlanSub,listProceduresAddHist:listAddHistProcs);
				}
			} 
		...
		public static long GlobalUpdateFees(List<Fee> listFeesHQ,long clinicNumGlobal,string progressText) {
			...
			long batchNumber=0;
			List<Benefit> listBenefitsAll=new List<Benefit>();
			...
			try {
				while(!_isQueueDone || _queueDataTables.Count>0) {//if batch thread is done and queue is empty, loop is finished
					...
					listPatPlanNums=table.Select().Select(x => PIn.Long(x["patPlanNum"].ToString())).ToList();
					listPlanNums=table.Select().Select(x => PIn.Long(x["planNum"].ToString())).ToList();
					...
					foreach(DataRow rowCur in table.Rows) {
						...
						long patPlanNum=PIn.Long(rowCur["patPlanNum"].ToString());
						long planNum=PIn.Long(rowCur["planNum"].ToString());
						string planType=PIn.String(rowCur["planType"].ToString());
						ExclusionRule exclusionRule=PIn.Enum<ExclusionRule>(rowCur["exclusionFeeRule"].ToString());
						...
						if(planType=="p") {//PPO plan, might use UCR fee
							feeSchedCur=0;
						...
						if(InsPlans.UsesUcrFeeForExclusions(exclusionRule)) {
							List<Benefit> listBenefits=listBenefitsAll.FindAll(x => x.PatPlanNum==patPlanNum || x.PlanNum==planNum);
			...
			//lookupFees passed in will also contain all possible alternate codes and medical codes
		{
			...
			for(int i=0;i<claimProcs.Count;i++) {
				...
				if(claimProcs[i].ClaimNum==0) {
					...
					ProcMultiVisit procMultiVisit=ProcMultiVisits.GetFirstOrDefault(x => x.ProcNum==proc.ProcNum);
					if(procMultiVisit!=null) {
						List<ProcMultiVisit> listProcMultiVisits=ProcMultiVisits.GetWhere(x => x.GroupProcMultiVisitNum==procMultiVisit.GroupProcMultiVisitNum);
						if(ProcMultiVisits.IsGroupInProcess(listProcMultiVisits)) {
							claimProcs[i].ProcDate=DateTime.Today;//Default to today if Multi Visit Group is in Process
						}
					}
				}
				...
				if(listWaitBenefits.Count>0 && hasEstimateCalculation) {
					...
					bool hasCodeGroupWaitPeriod = false;
					...
					if(!hasProcCodeWaitPeriod) {
						for(int b=0;b<listWaitBenefits.Count;b++) {
							bool isBenefitRelevant=CodeGroups.IsProcInCodeGroup(procCode.ProcCode,listWaitBenefits[b].CodeGroupNum);
							if(!isBenefitRelevant) {
								continue;
							}
							...
							else if(listWaitBenefits[b].QuantityQualifier==BenefitQuantity.Years) {
								dateCutoff=inssubCur.DateEffective.AddYears(listWaitBenefits[b].Quantity);
							}
							//If still within waiting period, zero out the claim and add a note
							if(dateClaimProc<dateCutoff.Date && dateClaimProc>=inssubCur.DateEffective.Date) {
								claimProcs[i].BaseEst=0;
		...
		{
			Meth.NoCheckMiddleTierRole();
			if(histList==null || listBenefits==null || !PrefC.GetBool(PrefName.InsChecksFrequency) || procedure.ProcDate.Year<1880 || claimProc.NoBillIns) {
				return false;
			}
			List<ClaimProcHist> listClaimProcHists=new List<ClaimProcHist>(histList);
			if(loopList!=null) {
				listClaimProcHists.AddRange(loopList);
			}
			...
			for(int i=0;i<listBenefits.Count;i++) {
				...
				if(listBenefits[i].PatPlanNum==0 && listBenefits[i].PlanNum!=planCur.PlanNum) {
					continue;
				}
				if(listBenefits[i].PlanNum!=planCur.PlanNum && listBenefits[i].PatPlanNum!=patPlanNum && patPlanNum!=0) {
					continue;//Only look at frequency limitation benefits for the current plan, not all plans.
				}
				if(!Benefits.IsFrequencyLimitation(listBenefits[i])) {
					continue;
				}
				...
				if(listBenefits[i].CodeNum > 0) {
					...
					List<ProcedureCode> listProcedureCodes=ProcedureCodes.GetWhereFromList(x => listCodeNums.Contains(x.CodeNum));
			...
			for(int i=0;i<listBenefits.Count;i++) {
				...
				if(listBenefits[i].QuantityQualifier==BenefitQuantity.Months || listBenefits[i].QuantityQualifier==BenefitQuantity.Years) {
					...
					if(listBenefits[i].TimePeriod==BenefitTimePeriod.NumberInLast12Months) {
					...
					else {//CalendarYear or ServiceYear
						...
						if(listBenefits[i].TimePeriod==BenefitTimePeriod.ServiceYear) {
							...
							InsPlan insPlan=listInsPlans.Find(x => x.PlanNum==listBenefits[i].PlanNum);
							...
							if(!listInsSubs.IsNullOrEmpty() && patPlanNum!=0 && listBenefits[i].PatPlanNum==patPlanNum) {
								...
								if(insSub==null) {
									continue;//The calling method did not provide the correct InsSubs.
								}
								insPlan=listInsPlans.Find(x => x.PlanNum==insSub.PlanNum);
							}
							if(insPlan==null) {
								continue;//The MonthRenew value cannot be determined.
							}
							dateLowerBound=new DateTime(procedure.ProcDate.Year,Math.Max(insPlan.MonthRenew,(byte)1),1);
							if(procedure.ProcDate.Date<=dateLowerBound && dateLowerBound>DateTime.Today) {
								dateLowerBound=dateLowerBound.AddYears(-1);
							}
						}
					}
					dateUpperBound=dateLowerBound.AddYears(1);
				}
				else {//Unsupported QuantityQualifier
					continue;
				}
			...
			List<Procedure> listProcsForAppt,List<InsSub> subList,Userod curUser)
		{
			...
			OrthoCaseProcedureLinker orthoCaseProcedureLinker=OrthoCaseProcedureLinker.CreateOneForPatient(patient.PatNum);
			...
			foreach(Procedure procCur in listProcsForAppt) {
				...
				OrthoProcLink orthoProcLink=orthoCaseProcedureLinker.LinkProcedureToActiveOrthoCaseIfNeeded(procCur);
			...
			List<InsPlan> listInsPlans,List<InsSub> listInsSubs,List<Fee> listFees=null)
		{
			...
			proc.ProcFee=Procedures.GetProcFee(pat,listPatPlans,listInsSubs,listInsPlans,proc,listFees:listFees);
			...
			List<InsSub> listInsSubs,List<InsPlan> listInsPlans,List<Benefit> listBenefits,Func<string,bool> funcYesNoPrompt)
		{
				...
				&& procedure.ProcFee==procedureOld.ProcFee)
			{
				...
				if(isUpdatingFee) {//Made it past the pref check.
					...
					if(isUpdatingFee) {
						procedure.ProcFee=GetProcFee(patient,listPatPlans,listInsSubs,listInsPlans,procedure,listBenefits,listFees:listFees
						);
					}
				}
			}
