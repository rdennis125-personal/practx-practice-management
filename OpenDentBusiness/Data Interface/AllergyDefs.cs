...
namespace OpenDentBusiness{
	...
	public class AllergyDefs{
		...
		///<summary>Gets all AllergyDefs from db. This table isn't cached.</summary>
		public static List<AllergyDef> GetAll(bool includeHidden) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT){
				return Meth.GetObject<List<AllergyDef>>(MethodBase.GetCurrentMethod(),includeHidden);
			}
			string command="";
			if(includeHidden) {
		...
		///<summary>Used to combine allergies. Starts with updating the FKs on any necessary tables, then removes the allergy from the allergydef table.</summary>
		public static void Combine(long allergyDefNumKeep,long allergyDefNumCombine) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),allergyDefNumKeep,allergyDefNumCombine);
				return;
			}
			//Update FKs
			//allergy table
			string command="UPDATE allergy SET AllergyDefNum = "
				+POut.Long(allergyDefNumKeep)
				+" WHERE AllergyDefNum ="
				+POut.Long(allergyDefNumCombine);
			Db.NonQ(command);
			//reminderrule table
			command="UPDATE reminderrule SET CriterionFK = "
				+POut.Long(allergyDefNumKeep)
				+" WHERE ReminderCriterion = "+POut.Enum(EhrCriterion.Allergy)
				+" AND CriterionFK = "
				+POut.Long(allergyDefNumCombine);
			Db.NonQ(command);
			//rxalert table
			...
			for(int i=0;i<listEhrTriggers.Count;i++) {
				...
				List<string> listAllergyDefNums
					=listEhrTriggers[i].AllergyDefNumList.Split(new string[] {" "},StringSplitOptions.RemoveEmptyEntries)
					.ToList();
				...
				string allergyDefNumList="";
				for(int j=0;j<listAllergyDefNums.Count;j++) {
					if(listAllergyDefNums[j]==allergyDefNumCombine.ToString()) {
						listAllergyDefNums[j]=allergyDefNumKeep.ToString();
					}
					allergyDefNumList+=" "+listAllergyDefNums[j]+" ";//Add surrounding whitespace back
				}
				command="Update ehrtrigger SET AllergyDefNumList = "
					+"'"+POut.String(allergyDefNumList)+"'"
					+" WHERE EhrTriggerNum = "
					+POut.Long(listEhrTriggers[i].EhrTriggerNum);
				Db.NonQ(command);
			}
			//Delete allergydef
