...
namespace OpenDentBusiness{
	...
	public class InstallmentPlans{
		...
		///<summary>Gets the installment plans for these families.  If there are none for a family, the guarantor will not be present in the list.</summary>
		...
		public static List<InstallmentPlan> GetListForFams(List<long> listGuarNums) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<InstallmentPlan>>(MethodBase.GetCurrentMethod(),listGuarNums);
			}
			string command="SELECT * FROM installmentplan WHERE PatNum IN("+string.Join(",",listGuarNums.Select(x => POut.Long(x)))+") ";
			List<InstallmentPlan> listInstallmentPlans=Crud.InstallmentPlanCrud.SelectMany(command).ToList();
