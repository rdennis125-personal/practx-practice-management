...
namespace OpenDentBusiness{
	...
	public class Deposits {
		...
		public static void Update(Deposit deposit){
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),deposit);
				return;
			}
			Crud.DepositCrud.Update(deposit);
		}

		///<summary></summary>
		public static void Update(Deposit deposit, Deposit depositOld) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),deposit,depositOld);
				return;
			}
			Crud.DepositCrud.Update(deposit,depositOld);
		}

		///<summary></summary>
		public static long Insert(Deposit deposit) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				deposit.DepositNum=Meth.GetLong(MethodBase.GetCurrentMethod(),deposit);
				return deposit.DepositNum;
			}
			return Crud.DepositCrud.Insert(deposit);
		}
		...
		public static void Delete(Deposit deposit){
			...
			//check dependencies
			string command="SELECT COUNT(*) FROM transaction WHERE DepositNum ="+POut.Long(deposit.DepositNum);
			...
			//ready to delete
			command="UPDATE payment SET DepositNum=0 WHERE DepositNum="+POut.Long(deposit.DepositNum);
			Db.NonQ(command);
			command="UPDATE claimpayment SET DepositNum=0 WHERE DepositNum="+POut.Long(deposit.DepositNum);
			Db.NonQ(command);
			Crud.DepositCrud.Delete(deposit.DepositNum);
		}
