...
namespace OpenDentBusiness {
	...
	public class HieQueues{
		...
		public static string ProcessQueues() {
			...
			List<HieClinic> listHieClinicsEnabled=HieClinics.GetAllEnabled();
			if(listHieClinicsEnabled.IsNullOrEmpty()) {
			...
			HieClinic hieClinicHQ=listHieClinicsEnabled.FirstOrDefault(x => x.ClinicNum==0);
			...
			for(int i=0;i<listHieQueuesToProcess.Count;i++) {
				try {
					Patient patient=listPatients.Find(x => x.PatNum==listHieQueuesToProcess[i].PatNum);
					if(patient==null){
						patient=Patients.GetPat(listHieQueuesToProcess[i].PatNum);
					}
					if(patient==null){
					...
					HieClinic hieClinicForPat=listHieClinicsEnabled.Find(x => x.ClinicNum==patient.ClinicNum);
					if(hieClinicForPat==null){
						hieClinicForPat=hieClinicHQ;
					}
					if(hieClinicForPat==null){
						listPatNumsFiltered.Add(listHieQueuesToProcess[i].PatNum);
					...
					if(hieClinicForPat.SupportedCarrierFlags!=HieCarrierFlags.AllCarriers) {//hieclinic has medicaid flag
						List<PatPlan> listPatPlansForPat=listPatPlans.FindAll(x => x.PatNum==patient.PatNum);
						if(listPatPlansForPat.IsNullOrEmpty()) {
							listPatNumsFiltered.Add(listHieQueuesToProcess[i].PatNum);
							continue;
						}
						List<InsSub> listInsSubsForPat=listInsSubs.FindAll(x => listPatPlansForPat.Select(x => x.InsSubNum).ToList().Contains(x.InsSubNum));
						if(listInsSubsForPat.IsNullOrEmpty()) {
							listPatNumsFiltered.Add(listHieQueuesToProcess[i].PatNum);
							continue;
						}
						List<InsPlan> listInsPlansForPat=listInsPlans.FindAll(x => listInsSubsForPat.Select(x => x.PlanNum).Contains(x.PlanNum));
						if(listInsPlansForPat.IsNullOrEmpty()) {
							listPatNumsFiltered.Add(listHieQueuesToProcess[i].PatNum);
							continue;
						}
						List<Carrier> listCarriersForPat=listCarriers.FindAll(x => listInsPlansForPat.Select(x => x.CarrierNum).ToList().Contains(x.CarrierNum));
					...
					if(!ODBuild.IsUnitTest) {//Don't create ccd export if running Unit Tests
						...
						string ccdTextForPat=EhrCCD.GenerateSummaryOfCare(patient,out string _,canValidate:false);
						string pathCcdExportWOExt=ODFileUtils.CombinePaths(hieClinicForPat.PathExportCCD,$"ccd_{DateTime_.Now.ToString("yyyyMMdd")}_{patient.PatNum}");
						string fileExt=".xml";
						string pathCcdExport=pathCcdExportWOExt+fileExt;
						if(File.Exists(pathCcdExport)) {
							int loopCount=1;
							while(true){
								if(!File.Exists(pathCcdExportWOExt+$"_{loopCount}{fileExt}")){
									break;
								}
								loopCount++;
							}
