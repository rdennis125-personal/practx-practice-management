...
namespace OpenDentBusiness{
	...
	public class Appointments {
		...
		public static List<long> GetApptNumsAttachedToTask(List<long> listApptNums){
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<long>>(MethodBase.GetCurrentMethod(),listApptNums);
			}
			//Select taskNums where the task contains one of the aptNums as an FK.
			string command= "SELECT KeyNum FROM task WHERE ObjectType="+(POut.Int((int)TaskObjectType.Appointment))+" and KeyNum IN("+string.Join(",",listApptNums.Select(x => POut.Long(x)))+")";
		...
		public static List<Appointment> GetRefreshedPlannedAppts(long patNum) {
			...
			string command=@$"SELECT * FROM appointment WHERE appointment.AptStatus={POut.Int((int)ApptStatus.Planned)} AND appointment.PatNum={POut.Long(patNum)}";
			return Crud.AppointmentCrud.SelectMany(command);
		}
		#endregion
