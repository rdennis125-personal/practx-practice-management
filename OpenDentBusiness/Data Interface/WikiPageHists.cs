	...
	public class WikiPageHists{
		...
		public static List<string> GetDeletedPages(string searchText,bool ignoreContent) {
			...
			for(int i=0;i<tableResults.Rows.Count;i++) {
				if(listRetVals.Contains(tableResults.Rows[i]["PageTitle"].ToString())) {
				...
				for(int j=0;j<tableNewestDateTimes.Rows.Count;j++) {
					...
					listRetVals.Add(tableResults.Rows[i]["PageTitle"].ToString());
					break;
				}
			}
		...
		public static long DeleteBeforeDate(DateTime dateCutoff){
			...
			string command=$"DELETE FROM wikipagehist WHERE DateTimeSaved <= {POut.DateT(dateCutoff)} ";
		...
		public static WikiPage RevertFrom(WikiPageHist wikiPageHist) {
			...
			WikiPage wikiPage=WikiPages.GetByTitle(wikiPageHist.PageTitle);
			if(wikiPage==null) {
			...
			wikiPage.KeyWords="";
			Match match=Regex.Match(wikiPageHist.PageContent,@"\[\[(keywords:).*?\]\]");
			if(match.Length>0) {
				wikiPage.KeyWords=match.Value.Substring(11).TrimEnd(']');
			}
