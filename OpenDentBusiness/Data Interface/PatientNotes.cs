...
namespace OpenDentBusiness{
	...
	public class PatientNotes{
		...
		private static PatientNote GetOne(long patNum) {
			Meth.NoCheckMiddleTierRole();//Private static method.
		...
		///<summary>Returns 0 if not locked. This gets hit every 4 seconds by every open ortho chart, but that's a pretty low load.</summary>
		public static long GetUserNumOrthoLocked(long patNum){
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetLong(MethodBase.GetCurrentMethod(),patNum);
			}
			string command = "SELECT UserNumOrthoLocked FROM patientnote "
				+" WHERE PatNum = '"+POut.Long (patNum)+"'";
			string raw=Db.GetScalar(command);
			if(raw is null){
				InsertRow(patNum);
				return 0;
			}
		...
		public static void SetUserNumOrthoLocked(long patNum,long userNum){
			...
			string command = "UPDATE patientnote "
				+" SET UserNumOrthoLocked= '"+POut.Long (userNum)+"'"
				+" WHERE PatNum = '"+POut.Long (patNum)+"'";
			Db.NonQ(command);
		}
