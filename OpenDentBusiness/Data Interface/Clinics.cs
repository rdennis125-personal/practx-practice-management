...
namespace OpenDentBusiness{
	...
	public class Clinics {
		...
		///<summary>Gets clinic using passed in clinicNum, if null gets default texting clinic, if null then gets practice as clinic zero.</summary>
		public static Clinic GetClinicOrSmsDefaultOrPracticeClinic(long clinicNum) {
			Meth.NoCheckMiddleTierRole();
			Clinic clinic=GetClinic(clinicNum);
			if(clinic!=null) {
				return clinic;
			}
			clinic=GetDefaultForTexting();
			if(clinic!=null) {
				return clinic;
			}
			return GetPracticeAsClinicZero();
		}
