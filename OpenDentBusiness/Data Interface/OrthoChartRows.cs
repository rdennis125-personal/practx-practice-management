		...
		public static bool Sync(List<OrthoChartRow> listNew,long patNum) {
			...
			for(int i=0;i<listUpdNew.Count;i++) {
				if(Crud.OrthoChartRowCrud.Update(listUpdNew[i],listUpdDB[i])) {
					...
					if(!string.IsNullOrEmpty(listUpdDB[i].Signature) && !string.IsNullOrEmpty(listUpdNew[i].Signature)) {
						logText+=Lans.g("OrthoCharts","Signature modified.")+" ";
					}
					else if(!string.IsNullOrEmpty(listUpdDB[i].Signature)&& string.IsNullOrEmpty(listUpdNew[i].Signature)) {
						logText+=Lans.g("OrthoCharts","Signature deleted.")+" ";
					}
					if(!string.IsNullOrEmpty(logText)) {
						logText+=listUpdDB[i].DateTimeService.ToString("yyyyMMdd");//This date stamp must be the last 8 characters for new OrthoEdit audit trail entries.
						SecurityLogs.MakeLogEntry(EnumPermType.OrthoChartEditFull,listUpdNew[i].PatNum,logText);
					}
				}
			}
