...
namespace OpenDentBusiness{
	...
	public class DrugUnits{
		...
		public static List<DrugUnit> GetMultDrugUnits(List<long> listDrugUnitNums) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<DrugUnit>>(MethodBase.GetCurrentMethod(),listDrugUnitNums);
			}
			if(listDrugUnitNums.Count==0) {
				return new List<DrugUnit>();
			}
			string strDrugUnitNums="";
			for(int i=0;i<listDrugUnitNums.Count;i++) {
				if(i>0) {
					strDrugUnitNums+="OR ";
				}
				strDrugUnitNums+="DrugUnitNum='"+listDrugUnitNums[i].ToString()+"' ";
			}
