...
namespace OpenDentBusiness{
	...
	public class FieldDefLinks{
		...
		public static bool Sync(List<FieldDefLink> listFieldDefLinksNew) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetBool(MethodBase.GetCurrentMethod(),listFieldDefLinksNew);
			}
			string command="SELECT * FROM fielddeflink";
			List<FieldDefLink> listFieldDefLinksDb=Crud.FieldDefLinkCrud.SelectMany(command);
			return Crud.FieldDefLinkCrud.Sync(listFieldDefLinksNew,listFieldDefLinksDb);
		}
		...
		public static void DeleteForFieldDefNum(long fieldDefNum,FieldDefTypes fieldDefTypes) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),fieldDefNum,fieldDefTypes);
				return;
			}
			if(fieldDefNum==0) {
				return;
			...
			Db.NonQ("DELETE FROM fielddeflink WHERE FieldDefNum="+POut.Long(fieldDefNum)+" AND FieldDefType="+POut.Int((int)fieldDefTypes));
		}
	}
}