...
namespace OpenDentBusiness{
	...
	public class PayConnectResponseWebs{
		...
		public static PayConnectResponseWeb GetOneByRefNumber(string refNumber) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<PayConnectResponseWeb>(MethodBase.GetCurrentMethod(),refNumber);
			}
			string command=$"SELECT * FROM payconnectresponseweb WHERE RefNumber='{POut.String(refNumber)}'";
			return Crud.PayConnectResponseWebCrud.SelectOne(command);
		}
