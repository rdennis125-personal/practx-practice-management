	...
	public class Hcpcses{
		...
		*/
		...
		public static List<string> GetAllCodes() {
			...
			List<string> listStringsHcpcsCode=new List<string>();
			string command="SELECT HcpcsCode FROM hcpcs";
			DataTable table=DataCore.GetTable(command);
			for(int i=0;i<table.Rows.Count;i++){
				listStringsHcpcsCode.Add(table.Rows[i][0].ToString());
			}
		...
		public static List<Hcpcs> GetBySearchText(string searchText) {
			...
			for(int i=0;i<listSearchTokens.Count;i++){
				if(i==0){
					command+="WHERE (HcpcsCode LIKE '%"+POut.String(listSearchTokens[i])+"%' OR DescriptionShort LIKE '%"+POut.String(listSearchTokens[i])+"%') ";
				}
				else{
					command+="AND (HcpcsCode LIKE '%"+POut.String(listSearchTokens[i])+"%' OR DescriptionShort LIKE '%"+POut.String(listSearchTokens[i])+"%') ";
				}
			}
			return Crud.HcpcsCrud.SelectMany(command);
		}
