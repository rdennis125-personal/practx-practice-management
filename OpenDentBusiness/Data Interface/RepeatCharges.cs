...
namespace OpenDentBusiness {
	...
	public class RepeatCharges {
		...
		///<summary>Gets a list of all RepeatCharges based on super family patNum, procCode, chargeAmt and date. 
		///patNumSuperFamily=0 to get a list of all matching repeat charge patients by ProcCode. Used for FormRepeatChargeEditMulti.</summary>
		public static List<RepeatCharge> GetRepeatChargesMulti(long patNumSuperFamily,string procCode,double chargeAmt,DateTime dateGreaterThan) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<RepeatCharge>>(MethodBase.GetCurrentMethod(),patNumSuperFamily,procCode,chargeAmt,dateGreaterThan);
			}
			string command="SELECT rc.* FROM repeatcharge rc ";
			if(patNumSuperFamily==0) {
				command+="WHERE rc.ProcCode='"+POut.String(procCode)+"' "
								+"AND rc.ChargeAmt="+POut.Double(chargeAmt)+" "
								+"AND rc.DateStart>="+POut.DateT(dateGreaterThan);
			}
			else {
				command+="INNER JOIN patient p ON p.PatNum=rc.PatNum "
								+"WHERE p.SuperFamily="+POut.Long(patNumSuperFamily)+" "
								+"AND rc.ProcCode='"+POut.String(procCode)+"' "
								+"AND rc.ChargeAmt="+POut.Double(chargeAmt)+" "
								+"AND rc.DateStart>="+POut.DateT(dateGreaterThan);
			}
			...
			bool isAutomated=false,LogSources source=LogSources.None, Patient newPat=null) 
		{
			...
			if(newCharge!=null) {
				newVal=newCharge.Frequency.ToString();
			}
		...
		private static string GetRepeatChargeChangeString(string paramName,string oldVal,string newVal,EnumPermType permType,ref bool hasChanges,bool isNote=false) {
			Meth.NoCheckMiddleTierRole();//Private method and ref parameter.
		...
		public static RepeatChargeResult RunRepeatingCharges(DateTime dateRun,bool doComputeAging=true) {
			...
			try {
				...
				List<long> listAllFamilyPatNums=Patients.GetAllFamilyPatNums(listRepeatChargePatNums); //Get every family member for every PatNum in our list of repeating charge PatNums
				...
				EnumRepeatChargeFrequency largestFrequency=listRepeatingCharges.Max(x => x.Frequency);
				...
				List<Procedure> listExistingProcs=Procedures.GetCompletedForDateRange(dateTimeRun,dateRun.AddDays(1),listRepeatingCharges.Select(x => x.ProcCode).Distinct().Select(x => ProcedureCodes.GetProcCode(x).CodeNum).ToList(),listRepeatChargePatNums);
				...
				List<OrthoCaseProcedureLinker> listOrthoCaseProcedureLinkers=OrthoCaseProcedureLinker.CreateManyForPatients(listRepeatChargePatNums);
				foreach(RepeatCharge repeatCharge in listRepeatingCharges) {
					...
					if(PrefC.GetBool(PrefName.BillingUseBillingCycleDay)) {
						Family family=listFamilies.First(x=>x.IsInFamily(repeatCharge.PatNum));
					...
					OrthoCaseProcedureLinker orthoCaseProcedureLinker=listOrthoCaseProcedureLinkers.First(x => x.PatNum==repeatCharge.PatNum);
					...
					foreach(DateTime billingDate in listBillingDates) {
						...
						try {
							...
							if(!listInvalidProcCodes.Contains(repeatCharge.ProcCode)) {
								listInvalidProcCodes.Add(repeatCharge.ProcCode);
							}
							continue;
						}
						...
						List<PatPlan> listPlansForPat=listAllPatPlans.FindAll(x=>x.PatNum==repeatCharge.PatNum);
						if(listPlansForPat.Count==1 && PatPlans.GetOrdinal(PriSecMed.Medical,listPlansForPat,listAllInsPlans,listAllInsSubs)>0) {//if there's exactly one medical plan
							InsSub insSub=InsSubs.GetSub(PatPlans.GetInsSubNum(listPlansForPat,PatPlans.GetOrdinal(PriSecMed.Medical,listPlansForPat,listAllInsPlans,listAllInsSubs)),listAllInsSubs);
							insPlan=InsPlans.GetPlan(insSub.PlanNum,listAllInsPlans);
						}
						else { //Otherwise use the primary insurance.
							InsSub insSub=InsSubs.GetSub(PatPlans.GetInsSubNum(listPlansForPat,PatPlans.GetOrdinal(PriSecMed.Primary,listPlansForPat,listAllInsPlans,listAllInsSubs)),listAllInsSubs);
							insPlan=InsPlans.GetPlan(insSub.PlanNum,listAllInsPlans);
						}
						if(repeatCharge.CreatesClaim && !InsPlanPreferences.NoBillIns(ProcedureCodes.GetProcCode(repeatCharge.ProcCode),insPlan)) {
							...
							Family family=listFamilies.First(x=>x.IsInFamily(repeatCharge.PatNum));
							List<long> listFamilyPatNums=family.GetPatNums();
							List<InsSub> listInsSubs=listAllInsSubs.FindAll(x=>listFamilyPatNums.Contains(x.Subscriber));
							List<InsPlan> listInsPlans=listAllInsPlans.FindAll(x=>listInsSubs.Select(x=>x.PlanNum).Contains(x.PlanNum));
				...
				if(listInvalidProcCodes.Count > 0) {
					result.ErrorMsg.AppendLine(Lans.g("RepeatCharges","One or more procedures were skipped due to the procedures being in a hidden category:")+$" {string.Join(", ",listInvalidProcCodes)}");
				}
				if(AvaTax.IsEnabled() && didEncounterAvaTaxError) {
					result.ErrorMsg.AppendLine(Lans.g("RepeatCharges","One or more procedures were skipped due to AvaTax related errors. See local logs for more details."));
				}
		...
		public static List<DateTime> GetBillingDatesHelper(RepeatCharge repeatCharge,DateTime dateRun,int billingCycleDay=0) {
			...
			if(repeatCharge.Frequency==EnumRepeatChargeFrequency.Monthly) {
				...
				if(repeatCharge.DateStop.Year>1880) {
					retVal.RemoveAll(x => x > repeatCharge.DateStop.AddMonths(monthAdd));
				}
			}
		...
		public static void AllocateUnearned(RepeatCharge repeatCharge,Procedure procedure,DateTime billingDate) {
			...
			if(AvaTax.IsEnabled()) {
				//Get adjustments for the procedure that are of AdjType==AvaTax.SalesTaxAdjType.
				//Add those adjustments as account entries to listAccountEntries.
				Adjustment adjustment=Adjustments.GetSalesTaxForProc(procedure.ProcNum);
				if(adjustment!=null) {
