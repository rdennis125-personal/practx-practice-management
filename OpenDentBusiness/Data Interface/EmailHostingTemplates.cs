	...
	public class EmailHostingTemplates{
		...
		public static void SyncWithHq() {
			...
			for(int i=0;i<listClinicNums.Count;i++) {
				try {
					SyncWithHq(listClinicNums[i],listHostingTemplatesAll.FindAll(x => x.ClinicNum==listClinicNums[i]));
				}
				catch(Exception ex) {
					Logger.WriteLine(Lans.g("MassEmail","Unable to sync MassEmail templates for clinicNum:")+" "+listClinicNums[i]+".\n"+MiscUtils.GetExceptionText(ex)
						,ODFileUtils.CombinePaths(MASS_EMAIL_LOG_DIR,listClinicNums[i].ToString()));
				}
			}
		}
		...
		private static void SyncWithHq(long clinicNum,List<EmailHostingTemplate> listEmailHostingTemplatesDatabase) {
			if(!Clinics.IsSecureEmailEnabled(clinicNum) && !Clinics.IsMassEmailEnabled(clinicNum)) {
				return; 
			}
			...
			List<ApptReminderRule> listApptReminderRulesBirthday=ApptReminderRules.GetForTypes(ApptReminderType.Birthday).Where(x => x.ClinicNum==clinicNum).ToList();
			List<EmailHostingTemplate> listEmailHostingTemplatesBirthday=listEmailHostingTemplatesDatabase.FindAll(x => x.TemplateType==PromotionType.Birthday);
			List<long> listEmailHostingTemplateNumsNoRules=listEmailHostingTemplatesBirthday
			...
			for(int i=0;i<listEmailHostingTemplateNumsNoRules.Count;i++) {
				Delete(listEmailHostingTemplateNumsNoRules[i]);
			}
			#endregion
			#endregion
			#region Get API templates
			GetAllTemplatesByAccountRequest getAllTemplatesByAccountRequest=new GetAllTemplatesByAccountRequest();
			GetAllTemplatesByAccountResponse getAllTemplatesByAccountResponse=api.GetAllTemplatesByAccount(getAllTemplatesByAccountRequest);
			List<long> listEmailHostingTemplateNumsToRemove=new List<long>();
			...
			List<long> listDictKeys=getAllTemplatesByAccountResponse.DictionaryTemplates.Keys.ToList();
			for(int i=0;i<listDictKeys.Count;i++) {
			...
			for(int i=0;i<listEmailHostingTemplatesDatabase.Count;i++) {
				bool existsApi=getAllTemplatesByAccountResponse.DictionaryTemplates.TryGetValue(listEmailHostingTemplatesDatabase[i].TemplateId,out Template templateApi);
				string htmlBody=GetHtmlBody(listEmailHostingTemplatesDatabase[i]);
				...
				if(existsApi) {
						...
						|| templateApi.TemplateBodyHtml!=htmlBody)) 
					{
						...
						UpdateTemplateResponse response=api.UpdateTemplate(updateTemplateRequest);
				...
				CreateTemplateResponse createTemplateResponse=api.CreateTemplate(createTemplateRequest);
			...
			for(int i=0;i<listEmailHostingTemplateNumsToRemove.Count;i++) {
		...
		private static string GetHtmlBody(EmailHostingTemplate emailHostingTemplate) {
			if(emailHostingTemplate.EmailTemplateType==EmailType.Html) {
				return MarkupEdit.TranslateToXhtml(emailHostingTemplate.BodyHTML,true,false,true);
			}
...
[{[{ OfficePhone }]}]";
		...
		#region Misc Methods
		public static bool IsBlank(EmailHostingTemplate emailHostingTemplate) {
			Meth.NoCheckMiddleTierRole();
			if(!string.IsNullOrEmpty(emailHostingTemplate.Subject)){
				return false;
			}
			if(!string.IsNullOrEmpty(emailHostingTemplate.BodyHTML)) {
				return false;
			}
			if(!string.IsNullOrEmpty(emailHostingTemplate.BodyPlainText)) {
				return false;
			}
			return true;
		}
		...
		public static List<string> GetListReplacements(string subjectOrBody) {
			...
			MatchCollection matchCollection=Regex.Matches(subjectOrBody,@"\[{\[{\s?([A-Za-z0-9]*)\s?}\]}\]");
			for(int i=0;i<matchCollection.Count;i++) {
				retVal.Add(matchCollection[i].Groups[1].Value.Trim());
			}
			return retVal;
		}
