	...
	public class ProviderClinics {
		...
		*/
		...
		public static List<ProviderClinic> GetByProvNumsAndClinicNum(List<long> listProvNums, long clinicNum = 0, bool includeUnsassigned = false) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<ProviderClinic>>(MethodBase.GetCurrentMethod(),listProvNums, clinicNum, includeUnsassigned);
			}
			if(listProvNums==null || listProvNums.Count==0) {
				return new List<ProviderClinic>();
			}
		...
		public static List<string> GetCareCreditMerchantNums(long clinicNum=-1) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<string>>(MethodBase.GetCurrentMethod(),clinicNum);
			}
			string whereClinic="";
			if(clinicNum>-1) {
				whereClinic=$"AND ClinicNum={POut.Long(clinicNum)}";
