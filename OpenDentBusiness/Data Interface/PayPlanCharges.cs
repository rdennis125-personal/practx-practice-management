...
namespace OpenDentBusiness{
	...
	public class PayPlanCharges {
		...
		public static List<PayPlanCharge> GetMany(List<long> listPayPlanChargeNums) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<PayPlanCharge>>(MethodBase.GetCurrentMethod(),listPayPlanChargeNums);
			}
			string command=
				"SELECT * FROM payplancharge "
		...
		///<summary>Returns a list of payment plan charges that are not safe to delete (either they are credits, or are charges with payments attached). If doDelete is true, all debits passed in will be deleted if they are safe to be deleted. Calling methods should use the list of charges returned to know which ones were not deleted.</summary>
		public static List<PayPlanCharge> DeleteDebitsWithoutPayments(List<PayPlanCharge> listCharges, bool doDelete=true) {
			Meth.NoCheckMiddleTierRole();
			List<PayPlanCharge> listPayPlanChargesNotDeleted=new List<PayPlanCharge>();
			...
			List<long> listPayPlanChargeNumsPreserve=PaySplits.GetForPayPlanCharges(listCharges.Where(x=>x.PayPlanChargeNum!=0).Select(x=>x.PayPlanChargeNum).ToList())
