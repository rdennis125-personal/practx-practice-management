...
namespace OpenDentBusiness {
	...
	public class Recalls {
		...
		public static string GetRecallTimePattern(Recall recallCur,List<Recall> listRecalls,Patient patCur,List<string> listProcStrs) {
			Meth.NoCheckMiddleTierRole();//Also, listProcStrs is used like an "out" or "ref" parameter.
			...
			bool doShowReminded=false,List<RecallType> listRecallTypes=null,bool isForWebSched=false) 
		{
			...
			Dictionary<long,List<DateTime>> dictCommlogs=Db.GetTable(command).Select()
				.GroupBy(x => PIn.Long(x["PatNum"].ToString()),x => new { CommDate=PIn.Date(x["CommDateTime"].ToString()),CommSource=PIn.Enum<CommItemSource>(x["CommSource"].ToString()) })
			...
			sw.Restart();
			Dictionary<long,List<WebSchedRecall>> dictWebSchedRecalls=tableWebSchedRecalls.AsEnumerable()
				.Select(x => new WebSchedRecall {
					WebSchedRecallNum=PIn.Long(x["WebSchedRecallNum"].ToString()),
					PatNum=PIn.Long(x["PatNum"].ToString()),
					MessageType=PIn.Enum<CommType>(PIn.Int(x["MessageType"].ToString())),
					SendStatus=PIn.Enum<AutoCommStatus>(PIn.Int(x["SendStatus"].ToString())),
					DateTimeSendFailed=PIn.DateT(x["DateTimeSendFailed"].ToString()),
					ResponseDescript=PIn.String(x["ResponseDescript"].ToString()),
					DateTimeSent=PIn.DateT(x["DateTimeSent"].ToString()),
				})
			...
			foreach(KeyValuePair<long,List<DataRow>> kvp in dictRecallRows) {
				...
				foreach(DataRow rowCur in kvp.Value) {
					...
					if(dictWebSchedRecalls.TryGetValue(patNum,out List<WebSchedRecall> listWebSchedSendMostRecent)) {
						//We should expect a WebSchedRecall for each configured send method in this list, ex. one for sms, one for email.
						WebSchedRecall webSchedRecall=listWebSchedSendMostRecent
		...
		public static bool HasTooManyReminders(int numberOfReminders,DateTime dateRemind,long maxReminders,bool isForWebSched=false) {
			...
			if(maxReminders>-1 && numberOfReminders>maxReminders) {
				return true;
			}
			return false;
		}
		...
		public static DataTable GetAddrTableForWebSched(List<long> recallNums,bool groupByFamily,RecallListSort sortBy,List<CommType> listCommTypes=null) {
			...
			for(int i=0;i<rawRows.Count;i++) {
				...
				row["dateDue"]=PIn.Date(rawRaw["DateDue"].ToString()).ToShortDateString();
				row["numberOfReminders"]=PIn.Long(rawRaw["numberOfReminders"].ToString()).ToString();
		...
		public static void UpdateStatus(long recallNum,long newStatus) {
			...
			Recall recall=GetRecall(recallNum);
			long statusEmailed=PrefC.GetLong(PrefName.RecallStatusEmailed);
			long statusTexted=PrefC.GetLong(PrefName.RecallStatusTexted);
			long statusTextedEmailed=PrefC.GetLong(PrefName.RecallStatusEmailedTexted);
			List<long> listRecallStatuses=new List<long> { statusEmailed,statusTexted,statusTextedEmailed,0 };
			if(recall is null || newStatus==recall.RecallStatus) {
			...
			if(listRecallStatuses.Contains(newStatus)) {
				if(recall.RecallStatus==statusTextedEmailed) {
					return;//Already both
				}
				else if(newStatus==0 && recall.RecallStatus==statusEmailed) {//Text failed, but email already successful.
					return;//Don't overwrite a successful Emailed.
				}
				else if((newStatus==statusEmailed && recall.RecallStatus==statusTexted) || //Adding Emailed to Texted
