...
namespace OpenDentBusiness{
	...
	public class ProcButtons {
		...
		public static ProcButton GetOne(long procButtonNum) {
			Meth.NoCheckMiddleTierRole();
			List<ProcButton> listProcButtons=GetWhere(x => x.ProcButtonNum==procButtonNum);
			ProcButton procButton=null;
			if(listProcButtons!=null && listProcButtons.Count>0) {
				procButton=listProcButtons[0];
			}
			return procButton;
		}
		...
		public static void SetToDefault() {
			...
			Db.NonQ(command);
			command="DELETE FROM definition WHERE Category="+POut.Enum(DefCat.ProcButtonCats);
			Db.NonQ(command);
