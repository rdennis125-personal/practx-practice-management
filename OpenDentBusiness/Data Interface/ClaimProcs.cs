...
namespace OpenDentBusiness{
	...
	public class ClaimProcs{
		...
		private static void TransferPayAsTotal(PayAsTotal payAsTotal,List<ClaimProc> listClaimProcs,List<Procedure> listProcedures,ref ClaimTransferResult claimTransferResult) {
			Meth.NoCheckMiddleTierRole();//Ref parameters
		...
		///<summary>Gets all ClaimProcs contributing to the -InsEst value for a given family. Mimics aging, specifically the Regular Claimproc By DateCP region of Ledgers.GetTransQueryString().</summary>
		public static DataTable GetClaimProcEstimatesForPatients(List<long> listPatNums) {
			...
			string command="SELECT cp.ClaimProcNum, cp.ProcNum, cp.ClaimNum, cp.PatNum, cp.InsPayEst, cp.WriteOff, p.ProcStatus, p.CodeNum, c.DateService "
				+"FROM claimproc cp "
				+"LEFT JOIN procedurelog p ON cp.ProcNum = p.ProcNum "
				+"LEFT JOIN claim c ON cp.ClaimNum = c.ClaimNum "
				+"WHERE cp.Status = "+POut.Long((long)ClaimProcStatus.NotReceived)+" "
				+"AND (cp.ProcNum = 0 OR p.ProcStatus = "+POut.Long((long)ProcStat.C)+") "
