...
namespace OpenDentBusiness {
	...
	public class Lans {
		...
		private class LanguageCache : CacheDictNonPkAbs<Language,string,Language> {
			...
			protected override DataTable DictToTable(Dictionary<string,Language> dictionaryLanguages) {
				return Crud.LanguageCrud.ListToTable(dictionaryLanguages.Values.ToList(),"Language");
			}
			...
			protected override DataTable ListToTable(List<Language> listLanguages) {
				return Crud.LanguageCrud.ListToTable(listLanguages);
			}
		}
		...
		public static void DeleteItems(string classType,List<string> listEnglish) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),classType,listEnglish);
				return;
			}
			...
			for(int i=0;i<listEnglish.Count;i++) {
				...
				command+="English='"+POut.String(listEnglish[i])+"' ";
		...
		public static List<string> GetListCat() {
			...
			for(int i=0;i<table.Rows.Count;i++) {
				listCats.Add(PIn.String(table.Rows[i][0].ToString()));
			}
			return listCats;
		}
		...
		public static List<Language> GetListForCat(string classType) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<Language>>(MethodBase.GetCurrentMethod(),classType);
			}
			string command="SELECT * FROM language "
