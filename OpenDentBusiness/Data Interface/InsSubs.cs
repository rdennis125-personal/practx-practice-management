...
namespace OpenDentBusiness{
	...
	public class InsSubs {
		...
		public static InsSub GetSub(long insSubNum,List<InsSub> listInsSubs) {
			...
			InsSub insSub=listInsSubs.LastOrDefault(x => x.InsSubNum==insSubNum);
			if(insSub==null) {
				insSub=GetOne(insSubNum);
			}
			if(insSub==null) {
				return new InsSub();
			}
		...
		public static List<InsSub> GetManyByInsFilingCodes(List<long> listInsFilingCodeNums) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<InsSub>>(MethodBase.GetCurrentMethod(),listInsFilingCodeNums);
			}
		...
		public static List<InsSub> RefreshForFam(Family family) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<InsSub>>(MethodBase.GetCurrentMethod(),family);
			}
			...
			for(int i = 0;i<family.ListPats.Length;i++) {
				...
				command+=" A.Subscriber="+POut.Long(family.ListPats[i].PatNum);
			}
			...
			for(int i = 0;i<family.ListPats.Length;i++) {
				...
				command+=" P.PatNum="+POut.Long(family.ListPats[i].PatNum);
			}
			command+="))) C "
		...
		public static SerializableDictionary<Family,List<InsSub>> GetDictInsSubsForFams(List<Family> listFamilies) {
			...
			for(int i=0;i<listFamilies.Count;i++){
				List<long> listPatNumsFam=listFamilies[i].ListPats.Select(x => x.PatNum).ToList();
				List<InsSub> listInsSubs=new List<InsSub>();
				List<DataRow> listDataRows=table.Select().Where(x => listPatNumsFam.Exists(y => y==PIn.Long(x["OnBehalfOf"].ToString()))).ToList();
				DataTable tableFamilyInsSubs=table.Clone();
				for(int j=0;j<listDataRows.Count;j++){
					tableFamilyInsSubs.ImportRow(listDataRows[j]);
				}
				dictFamilyInsSubs[listFamilies[i]]=Crud.InsSubCrud.TableToList(tableFamilyInsSubs);
			}
			return dictFamilyInsSubs;
		}
		...
		public static void ValidateNoKeys(long insSubNum, bool isStrict){
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),insSubNum,isStrict);
				return;
			}
			string command="SELECT 1 FROM claim WHERE InsSubNum="+POut.Long(insSubNum)+" OR InsSubNum2="+POut.Long(insSubNum)+" "+DbHelper.LimitAnd(1);
			...
			if(isStrict) {
				command="SELECT 1 FROM claimproc WHERE InsSubNum="+POut.Long(insSubNum)+" AND Status!="+POut.Int((int)ClaimProcStatus.Estimate)+" "+DbHelper.LimitAnd(1);//ignore estimates
				if(!string.IsNullOrEmpty(Db.GetScalar(command))) {
			...
			command="SELECT 1 FROM etrans WHERE InsSubNum="+POut.Long(insSubNum)+" "+DbHelper.LimitAnd(1);
			if(!string.IsNullOrEmpty(Db.GetScalar(command))) {
			...
			command="SELECT 1 FROM payplan WHERE InsSubNum="+POut.Long(insSubNum)+" "+DbHelper.LimitAnd(1);
			if(!string.IsNullOrEmpty(Db.GetScalar(command))) {
		...
		public static int GetSubscriberCountForPlan(long planNum,bool isExcludedSub) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetInt(MethodBase.GetCurrentMethod(),planNum,isExcludedSub);
			}
		...
		///<summary>Only used when there are more than 10,000 subscribers.  Gets a list of subscriber names from the database that have the specified plan. Used to display in the insplan window.  The returned list never includes the inssub that we're viewing.</summary>
		public static List<string> GetSubscribersForPlan(long planNum,long insSubNumExclude) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<string>>(MethodBase.GetCurrentMethod(),planNum,insSubNumExclude);
			}
			string command="SELECT CONCAT(CONCAT(LName,', '),FName) "
				+"FROM inssub LEFT JOIN patient ON patient.PatNum=inssub.Subscriber "
				+"WHERE inssub.PlanNum="+POut.Long(planNum)+" "
				+"AND inssub.InsSubNum !="+POut.Long(insSubNumExclude)+" "
				+" ORDER BY LName,FName";
			DataTable table=Db.GetTable(command);
			List<string> listSubscriberNames=new List<string>(table.Rows.Count);
			for(int i=0;i<table.Rows.Count;i++) {
				listSubscriberNames.Add(PIn.String(table.Rows[i][0].ToString()));
			}
		...
		public static string GetBenefitNotes(long planNum,long insSubNumExclude) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetString(MethodBase.GetCurrentMethod(),planNum,insSubNumExclude);
			}
			string command="SELECT BenefitNotes FROM inssub WHERE BenefitNotes != '' AND PlanNum="+POut.Long(planNum)+" AND InsSubNum !="+POut.Long(insSubNumExclude)+" "+DbHelper.LimitAnd(1);
			DataTable table=Db.GetTable(command);
		...
		public static void SynchPlanNumsForNewPlan(InsSub insSub) {
			...
			string command=$@"UPDATE claim
				...
				SET insbluebook.PlanNum={POut.Long(insSub.PlanNum)}
				WHERE claim.InsSubNum={POut.Long(insSub.InsSubNum)} AND claim.PlanNum!={POut.Long(insSub.PlanNum)}";
			Db.NonQ(command);
			//claim.PlanNum
			command="UPDATE claim SET claim.PlanNum="+POut.Long(insSub.PlanNum)+" "
				+"WHERE claim.InsSubNum="+POut.Long(insSub.InsSubNum)+" AND claim.PlanNum!="+POut.Long(insSub.PlanNum);
			Db.NonQ(command);
			//claim.PlanNum2
			command="UPDATE claim SET claim.PlanNum2="+POut.Long(insSub.PlanNum)+" "
				+"WHERE claim.InsSubNum2="+POut.Long(insSub.InsSubNum)+" AND claim.PlanNum2!="+POut.Long(insSub.PlanNum);
			Db.NonQ(command);
			//claimproc.PlanNum
			command="UPDATE claimproc SET claimproc.PlanNum="+POut.Long(insSub.PlanNum)+" "
				+"WHERE claimproc.InsSubNum="+POut.Long(insSub.InsSubNum)+" AND claimproc.PlanNum!="+POut.Long(insSub.PlanNum);
			Db.NonQ(command);
			//payplan.PlanNum
			command="UPDATE payplan SET payplan.PlanNum="+POut.Long(insSub.PlanNum)+" "
				+"WHERE payplan.InsSubNum="+POut.Long(insSub.InsSubNum)+" AND payplan.PlanNum!="+POut.Long(insSub.PlanNum);
			Db.NonQ(command);
			//etrans.PlanNum, only used if EtransType.BenefitInquiry270 and BenefitResponse271 and Eligibility_CA.
			command="UPDATE etrans SET etrans.PlanNum="+POut.Long(insSub.PlanNum)+" "
				+"WHERE etrans.InsSubNum!=0 AND etrans.InsSubNum="+POut.Long(insSub.InsSubNum)+" AND etrans.PlanNum!="+POut.Long(insSub.PlanNum);
			Db.NonQ(command);
		}
		...
		public static long MoveSubscribers(long insPlanNumFrom,long insPlanNumTo) {
			...
			for(int i=0;i<listInsSubsFrom.Count;i++){
				...
				List<PatPlan> listPatPlansFrom=PatPlans.Refresh(insSubFrom.Subscriber);
				for(int j=0;j<listPatPlansFrom.Count;j++) {
					PatPlan patPlanFrom=listPatPlansFrom[j];
			...
			if(listBlockedPatNums.Count>0) {
				...
				for(int i=0;i<listBlockedPatNums.Count;i++) {
					...
					Patient patient=Patients.GetPat(listBlockedPatNums[i]);
					stringBuilder.Append("#"+listBlockedPatNums[i]+" "+patient.GetNameFLFormal());
				}
				throw new ApplicationException(Lans.g("InsSubs","Before changing the subscribers on the insurance plan being moved from, please delete all of today's claims related to the insurance plan being moved from for the following patients")+":"+stringBuilder.ToString());
			}
			//This loop mimics some of the logic in PatPlans.Delete().
			int insSubMovedCount=0;
			for(int i=0;i<listInsSubsFrom.Count;i++){
				InsSub insSub=listInsSubsFrom[i];
				long insSubNumOld=insSub.InsSubNum;
				...
				string command="SELECT PatNum FROM patplan WHERE InsSubNum="+POut.Long(insSubNumOld);
				...
				for(int j=0;j<tablePatsForInsSub.Rows.Count;j++) {
					...
					List<Procedure> listProcedures=Procedures.GetProcsByStatusForPat(patNum,ProcStat.TP,ProcStat.TPi);
			...
			InsPlan insPlanOld = insPlanFrom.Copy();
		...
		public static void AssignBlankPlanToInsSub(InsSub insSub) {
			...
			InsPlan insPlan=InsPlans.GetByCarrierName(strCarrierUnknownName).FirstOrDefault();
		...
		public static bool ValidatePlanNum(long insSubNum,bool doFixIfInvalid=true,List<InsSub> listInsSubs=null,List<InsPlan> listInsPlans=null) {
			...
			long subscriberNum=insSub.Subscriber;
			...
			try {
				...
				InsSubs.Delete(insSub.InsSubNum);
				...
				InsSubs.AssignBlankPlanToInsSub(insSub);
		...
		public static bool ValidatePlanNumForList(List<long> listInsSubNums,bool doFixIfInvalid = true) {
			bool isValid=true;
			for(int i=0;i<listInsSubNums.Count;i++) {
				if(!InsSubs.ValidatePlanNum(listInsSubNums[i],doFixIfInvalid)){
					isValid=false;
				}
			}
			return isValid;
