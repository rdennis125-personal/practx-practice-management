...
namespace OpenDentBusiness{
	...
	public class Payments {
		...
		public static List<Payment> GetPayments(List<long> listPayNums) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<Payment>>(MethodBase.GetCurrentMethod(),listPayNums);
			}
			string command=$"SELECT * FROM payment WHERE PayNum IN({string.Join(",",listPayNums.Select(x => POut.Long(x)))})";
			return Crud.PaymentCrud.SelectMany(command);
		}
		...
		public static List<PaymentForApi> GetPaymentsForApi(int limit,int offset,long patNum,long defNumPayType,DateTime dateEntry) {
			...
			for(int i=0;i<listPayments.Count;i++) {
				PaymentForApi paymentForApi=new PaymentForApi();
		...
		///<summary>Gets all PayNums flagged as income transfers. Set isPayTypeIgnored to true to ignore payment.PayType and instead return all PayNums for payments that sum to $0.
		///Set isPayTypeIgnored to false to only return PayNums where the PayType is 0. Optionally pass in PatNums to only get transfers for specific patients.</summary>
		public static List<long> GetPayNumsForTransfers(bool isPayTypeIgnored,params long[] arrayPatNums) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<List<long>>(MethodBase.GetCurrentMethod(),isPayTypeIgnored,arrayPatNums);
			}
			string command="SELECT payment.PayNum FROM payment ";
			if(isPayTypeIgnored) {
			...
			if(!arrayPatNums.IsNullOrEmpty()) {
				command+=$"AND payment.PatNum IN({string.Join(",",arrayPatNums.Select(x => POut.Long(x)))}) ";
			}
			if(isPayTypeIgnored) {
		...
		///<summary>Gets all payments that have a ProcessStatus of OnlinePending. Pass in an empty list to get payments for all clinics. Referenced by OpenDentalService.</summary>
		...
		public static long InsertFromPaySimple(long patNum,long provNum,long clinicNum,double amount,string payNote,CreditCardSource creditCardSource) {
			string strPayType;
			...
			payment.PayType=PIn.Long(ProgramProperties.GetPropVal(Programs.GetCur(ProgramName.PaySimple).ProgramNum,strPayType,clinicNum));
			payment.ProcessStatus=ProcessStat.OnlinePending;
			payment.PayNote=payNote;
			payment.IsCcCompleted=true;
			if(PrefC.GetBool(PrefName.OnlinePaymentsMarkAsProcessed)) {
		...
		/// <summary>Inserts and returns a Payment object that returns the payment passed in specifically for XWeb. Optionally set a credit card processing company needs specific values for returning payments.</summary>
		public static Payment InsertReturnXWebPayment(Payment payment,string payNote,double payAmt,ProcessStat processStat=ProcessStat.OfficeProcessed) {
			Payment paymentReturn=payment.Clone();
			...
			if(paymentReturn.PayNote!="") {
				paymentReturn.PayNote+="\r\n";
			}
			...
			double payAmtAllocateRemaining=Math.Abs(payAmt);
			foreach(PaySplit paySplit in listPaySplits) {
				PaySplit paySplitCopy=paySplit.Copy();
				paySplitCopy.SplitAmt*=-1;
				if(isPartial) {
					if(CompareDouble.IsLessThanOrEqualToZero(payAmtAllocateRemaining)) {
						break;
					}
					double amtToAllocate=Math.Min(Math.Abs(paySplitCopy.SplitAmt),Math.Abs(payAmtAllocateRemaining));
		...
		/// <summary>Inserts and returns a Payment object that voids the payment passed in. Optionally set a custom payAmt when the credit card processing company needs specific values for voiding payments.</summary>
		public static Payment InsertVoidPayment(Payment payment,List<PaySplit> listPaySplits,string receipt,string payNote,CreditCardSource creditCardSource,ProcessStat processStat=ProcessStat.OfficeProcessed,double payAmt=0) {
			Payment paymentVoid=payment.Clone();
			if(payAmt!=0) {
				paymentVoid.PayAmt=payAmt;
			}
		...
		///<summary>Determines if all the paysplits in the payment have valid hashes. Uses passed list, not database. Returns false if a single paysplit is invalid, otherwise true. </summary>
		public static bool ArePaySplitHashesValid(long payNum,List<PaySplit> listPaySplits) {
			Meth.NoCheckMiddleTierRole();
			if(listPaySplits.Count==0) {
				return true;
			}
			List<PaySplit> listPaySplitsForPayment=listPaySplits.FindAll(x => x.PayNum==payNum);
			for(int i=0;i<listPaySplitsForPayment.Count;i++) {
