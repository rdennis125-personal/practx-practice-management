	...
	public class MedLabResults{
		...
		*/
		...
		public static List<MedLabResult> GetAllForLabs(List<MedLab> listMedLabs) {
			...
			List<MedLabResult> listMedLabResults=new List<MedLabResult>();
			if(listMedLabs.Count==0) {
				return listMedLabResults;
			}
		...
		public static void DeleteAll(List<long> listMedLabResultNums) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),listMedLabResultNums);
				return;
			}
			string command= "DELETE FROM medlabresult WHERE MedLabResultNum IN("+String.Join(",",listMedLabResultNums)+")";
			Db.NonQ(command);
		}
