	...
	public class Sheets{
		...
		public static DataTable GetPatientFormsTable(long patNum) {
			...
			command="SELECT EFormNum,DateTimeShown,Description,DateTEdited "
				+"FROM eform "
				+"WHERE PatNum ="+POut.Long(patNum);
			DataTable tableRawEForm=Db.GetTable(command);
			for(int i=0;i<tableRawEForm.Rows.Count;i++) {
				dataRow=table.NewRow();
				dateT=PIn.DateT(tableRawEForm.Rows[i]["DateTimeShown"].ToString());
				dataRow["date"]=dateT.ToShortDateString();
				dataRow["dateOnly"]=dateT.Date;
				dataRow["dateTime"]=dateT;
				dataRow["DateTSheetEdited"]=PIn.DateT(tableRawEForm.Rows[i]["DateTEdited"].ToString());
