...
namespace OpenDentBusiness{
	...
	public class Etranss {
		...
		///<summary>Changes the status of the claim back to W.  If it encounters an entry that's not a claim, it skips it for now.  Later, it will handle all types of undo.  It will also check Canadian claims to prevent alteration if an ack or EOB has been received.</summary>
			...
			if(Xobj!=null) {//Is a correctly formatted X12 message.
				if(Xobj.IsAckInterchange()) {
					...
					for(int i=0;i<listEtrans.Count;i++) {
						...
						List<Hx835_ShortClaim> listShortClaims=listShortClaimsAll.FindAll(x => listClaimNums.Contains(x.ClaimNum));
						List<long> listPlanNumsForClaims=listShortClaims.Select(x => x.PlanNum).Distinct().ToList();
						List<InsPlan> listInsPlansForClaims=listInsPlansAll.FindAll(x => listPlanNumsForClaims.Contains(x.PlanNum));
		...
		public static bool IsEtransAutomatable(List<Carrier> listCarriersForClaims,string payerName,bool isFullyAutomatic) {
			...
			if(listCarriersForClaims.Count>0) {
				...
				return listCarriersForClaims.Any(x => listAllowedEraAutomationModes.Contains(x.GetEraAutomationMode()));
			}
			...
			if(listCarrierNameMatches.Count==0) {
				...
				return listAllowedEraAutomationModes.Contains(PrefC.GetEnum<EraAutomationMode>(PrefName.EraAutomationBehavior));
			}
			//If we have no attached claims and one or more names match, we allow automation if any name-matched carriers allow it.
			return listCarrierNameMatches.Any(x => listAllowedEraAutomationModes.Contains(x.GetEraAutomationMode()));
		}
		...
		public static List<EraAutomationResult> TryAutoProcessEras(List<Etrans> listEtrans,List<Etrans835Attach> listAttaches,bool isFullyAutomatic) {
			...
			List<EraAutomationResult> listAllAutomationResults=new List<EraAutomationResult>();
			for(int i=0;i<listEtrans.Count;i++) {
				...
				else {//Etrans has a single transaction
				...
				if(overallStatus!=X835Status.Finalized) {
					...
					if(isFullyAutomatic) {
						autoProcessedStatus=X835AutoProcessed.FullAutoIncomplete;
					}
				}
				else {
					autoProcessedStatus=X835AutoProcessed.SemiAutoComplete;
					if(isFullyAutomatic) {
						autoProcessedStatus=X835AutoProcessed.FullAutoComplete;
					}
				}
		...
		///<summary>If etrans.TranSetId835 is blank and we have multiple TranSetIds in the list, we know we are dealing with an Etrans from 14.2 or an older version
		///that represents multiple transactions from a single 835. We loop through the transactions and process each of them separately.
		///This should not be called for newer etrans because they always represent a single transaction.</summary>
		private static List<EraAutomationResult> AutoProcessMultiTransactionEtrans(Etrans etrans,string messageText835,
			List<string> listTranSetIds,List<Etrans835Attach> listAttaches,bool isFullyAutomatic)
		{
			List<EraAutomationResult> listAutomationResults=new List<EraAutomationResult>();
			for(int i=0;i<listTranSetIds.Count;i++) {
				X835 x835=new X835(etrans,messageText835,listTranSetIds[i],listAttaches);
				EraAutomationResult automationResult=TryAutoProcessEraEob(x835,listAttaches,isFullyAutomatic);
				automationResult.TransactionNumber=i+1;
		...
		public static EraAutomationResult TryAutoProcessEraEob(X835 x835,List<Etrans835Attach> listAttaches,bool isFullyAutomatic) {
			...
			if(listReceivedClaimProcs.Count > 0) {
				List<Claim> listSecondaryClaims=Claims.GetPrimaryOrSecondaryClaimsNotReceived(listReceivedClaimProcs);
			...
			if(automationResult.AreAllClaimsReceived()) {//Only attempt to finalize the payment if automation has processed all of the claims.
				...
				List<ClaimProc> listClaimProcsForFinalization=listClaimProcsAll.FindAll(x => listClaimNumsForFinalization.Contains(x.ClaimNum));
		...
		public static List<PayPlan> FilterValidInsPayPlansForClaimHelper(List<PayPlan> listPayPlans,List<ClaimProc> listClaimProcsAll,Claim claim) {
			...
			PayPlan payPlanForClaim=listPayPlans.FirstOrDefault(x => listPayPlanNumsForClaimProcs.Contains(x.PayPlanNum));
			...
			for(int i=0;i<listPayPlans.Count;i++) {
				...
				if(listPayPlanNumsForAllClaimProcs.Contains(listPayPlans[i].PayPlanNum)) {
			...
			Claim claim,Patient pat,bool isAutomatic,List<ClaimProc> listClaimProcsForClaim,long insPayPlanNum,EraAutomationResult automationResult=null)
		{
			...
			for(int i=0;i<listClaimProcsToEdit.Count;i++) {
				...
				for(int j=0;j<listProcsForProcNum.Count;j++) {
					...
					listClaimAdjReasonCodes.AddRange(procPaidPartial.ListProcAdjustments.Select(x => x.ReasonCode));
				}
		...
		///received claims, false allows the claim to be marked received if all ClaimProcs in listClaimProcsForClaim meet requirements.</summary>
