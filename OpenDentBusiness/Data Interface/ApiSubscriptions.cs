...
namespace OpenDentBusiness{
	...
	public class ApiSubscriptions{
		...
		///<summary>Gets one ApiSubscription from the db.</summary>
		public static ApiSubscription GetOne(string customerKey,string endPoint,string workstation,EnumWatchTable enumWatchTable){//,string watchColumn){
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT){
				return Meth.GetObject<ApiSubscription>(MethodBase.GetCurrentMethod(),customerKey,endPoint,workstation,enumWatchTable);
			}
			string command="SELECT * FROM apisubscription "+
				...
				"AND WatchTable="+POut.Enum(enumWatchTable)+" "+
		...
		private class ApiSubscriptionCache : CacheListAbs<ApiSubscription> {
			...
			protected override void FillCacheIfNeeded() {
				ApiSubscriptions.GetTableFromCache(doRefreshCache:false);
			}
		}
		...
		public static void ClearCache() {
			_apiSubscriptionCache.ClearCache();
		}
