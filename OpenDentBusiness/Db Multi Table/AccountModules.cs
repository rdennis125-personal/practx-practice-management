	...
	public class AccountModules {
		...
		public static DataTable GetCommLog(Patient pat,Family fam) {
			...
			if(PrefC.IsODHQ) {
				...
				List<WebChatNote> listWebChatNotesAll=new List<WebChatNote>();
				WebChatMisc.DbAction(delegate() {
					...
					listWebChatNotesAll=WebChatNotes.GetAllForSessions(listWebChatSessions.Select(x => x.WebChatSessionNum).ToList());
				});
		...
		{
			Meth.NoCheckMiddleTierRole();//This method contains out parameters.
			...
			if(PrefC.IsODHQ && Resellers.IsResellerFamily(family.ListPats[0])) {
				isReseller=true;
			}
			...
			for(int i=0;i<rawProc.Rows.Count;i++){
				...
				dataRow["DateTime"]=dateT;
				dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
				dataRow["dateTimeSort"]=PIn.DateT(rawProcRow["DateTStamp"].ToString());//DateTStamp will be used for sorting if RandomKeys is enabled
			...
			for(int i=0;i<rawAdj.Rows.Count;i++){
				...
				dataRow["DateTime"]=dateT;
				dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
				dataRow["dateTimeSort"]=PIn.DateT(rawAdj.Rows[i]["SecDateTEdit"].ToString());//SecDateTEdit will be used for sorting if RandomKeys is enabled
			...
			for(int i=0;i<rawPay.Rows.Count;i++) {
				...
				dataRow["DateTime"]=dateT;
				dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
				dataRow["dateTimeSort"]=PIn.DateT(rawPay.Rows[i]["SecDateTEdit"].ToString());//MAX SecDateTEdit will be used for sorting if RandomKeys is enabled
			...
			for(int i=0;i<rawClaim.Rows.Count;i++){//rawClaim will have 0 rows if isInvoice or StatementType is LimitedStatement
				...
				dataRow["DateTime"]=dateT;
				dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
				dataRow["dateTimeSort"]=PIn.DateT(rawClaim.Rows[i]["SecDateTEdit"].ToString());//MAX SecDateTEdit will be used for sorting if RandomKeys is enabled
			...
			foreach(DataRow rowCur in rawState.Rows) {//rawState will have 0 rows if isInvoice or StatementType is LimitedStatement
				...
				dataRow["DateTime"]=dateT;
				dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
				dataRow["dateTimeSort"]=PIn.DateT(rowCur["DateTStamp"].ToString());//DateTStamp will be used for sorting if RandomKeys is enabled
				...
				dataRow["patient"]=GetPatName(patNumCur,family,isReseller);
				dataRow["PatNum"]=patNumCur;
				dataRow["PayNum"]=0;
				dataRow["PayPlanNum"]=0;
				dataRow["PayPlanChargeNum"]="0";
			...
			if(payPlanVersionCur.In(PayPlanVersions.DoNotAge,PayPlanVersions.NoCharges)) {//Version 1 and 4
				...
				for(int i=0;i<rawPayPlan.Rows.Count;i++) {
					...
					dataRow["DateTime"]=dateT;
					dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
					dataRow["dateTimeSort"]=PIn.DateT(rawPayPlan.Rows[i]["SecDateTEntry"].ToString());//MAX SecDateTEntry will be used for sorting if RandomKeys is enabled
			...
			if(payPlanVersionCur==PayPlanVersions.AgeCreditsAndDebits) { //this information is only required for v2
				...
				for(int i=0;i<rawPayPlan2.Rows.Count;i++) {
					...
					dataRow["DateTime"]=dateT;
					dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
					dataRow["dateTimeSort"]=PIn.DateT(rawPayPlan2.Rows[i]["SecDateTEntry"].ToString());//SecDateTEntry will be used for sorting if RandomKeys is enabled
			...
			if(payPlanVersionCur==PayPlanVersions.AgeCreditsOnly) {
				...
				for(int i = 0;i<rawPayPlan3.Rows.Count;i++) {
					...
					dataRow["DateTime"]=dateT;
					dataRow["date"]=dateT.ToString(Lans.GetShortDateTimeFormat());
					dataRow["dateTimeSort"]=PIn.DateT(rawPayPlan3.Rows[i]["SecDateTEntry"].ToString());//SecDateTEntry will be used for sorting if RandomKeys is enabled
		...
		{
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetObject<Claim>(MethodBase.GetCurrentMethod(),claim,claimType,PatPlanList,planList,ClaimProcList,
