...
namespace OpenDentBusiness {
	...
	public class MiscData {
		...
		///<summary>Returns the command passed with prepended 'SET SESSION sql_log_bin=0' and appended 'SET SESSION sql_log_bin=1'.</summary>
		private static string DisableBinaryLoggingForCommand(string command) {
			//Command must end with ';'
			if(!command.EndsWith(";")) {
				command=command+";";
			}
			return "SET SESSION sql_log_bin=0;"+command+"SET SESSION sql_log_bin=1;";
		}
		...
		public static string MakeABackup(string serverName="",string user="",string pass="",bool doVerify=false,bool isAutoBackup=true,string sslCa="") {
			...
			if(isAutoBackup && PrefC.GetStringNoCache(PrefName.UpdateStreamLinePassword)=="abracadabra") {
				return "";
			}
			bool useSameServer;
			string currentServerName=DataConnection.GetServerName();
			//Disable binary logging when running SELECT statements in the table creations. Large offices on replication will experience extreme 
			//slowness because the entire SELECT statement gets bin logged. Backup dbs do not get replicated across all servers. Turning off bin logging is safe.
			bool doTurnOffBinLogging=IsMariaDB() && ReplicationServers.IsUsingReplication();
			if(currentServerName==null) {//Will be null when utilizing advanced connection strings.
				useSameServer=string.IsNullOrWhiteSpace(serverName);//Simply use the optional connection information when it is provided. Don't get cute.
			}
			else {
				currentServerName=currentServerName.ToLower();//Preserves old behavior.
				useSameServer=string.IsNullOrWhiteSpace(serverName) || currentServerName.Equals(serverName,StringComparison.CurrentCultureIgnoreCase);
			...
			try {
				...
				foreach(DataRow row in table.Rows) {
					...
					if(doTurnOffBinLogging) {
						command=DisableBinaryLoggingForCommand(command);
					}
					...
					bool backupIndexesDisabled=PrefC.GetBoolSilent(PrefName.BackupIndexesDisabled,false);
					if(isAutoBackup && backupIndexesDisabled) {
						//Disable the indexes to speed up the backup process.
						command="ALTER TABLE `"+POut.String(newDb)+"`.`"+POut.String(tableName)+"` DISABLE KEYS";
						dconBackupServerNoTimout.NonQ(command);
					}
					...
					if(useSameServer) {
						...
						dconBackupServerNoTimout.NonQ(command);
						command=$"FLUSH TABLE `{newDb}`.`{tableName}`"; //writes data to disk after running flush command.
						dconBackupServerNoTimout.NonQ(command);
					}
					else {
						command=$"SELECT COUNT(*) FROM `{oldDb}`.`{tableName}`;";
						if(doTurnOffBinLogging) {
							command=DisableBinaryLoggingForCommand(command);
						}
						long count=PIn.Long(dcon.GetCount(command));
						int limit=10000;
						...
						int offset=0;
						while(count > offset) {
							command=$"SELECT * FROM `{oldDb}`.`{tableName}` LIMIT {limit} OFFSET {offset};";
							if(doTurnOffBinLogging) {
								command=DisableBinaryLoggingForCommand(command);
							}
							DataTable dtOld=dcon.GetTable(command);
		...
		public static T RunFuncOnArchiveDatabase<T>(Func<T> f) {
			...
			if(RemotingClient.MiddleTierRole.In(MiddleTierRole.ClientMT,MiddleTierRole.ServerMT)) {//May already be behind a remoting role check.
		...
		///<summary>Returns true if the Database Management System(DBMS) server is MariaDB.</summary>
		public static bool IsMariaDB() {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetBool(MethodBase.GetCurrentMethod());
			}
			//MariaDB @@version should always contain the version, followed by '-MariaDB', followed by a possible few other suffixes.
			//For example, a configuration with binary(replication) logging enabled will look like '10.5.11-MariaDB-log'
			//If for some reason, '-MariaDB' is not included by default, we can modify the my.ini to include it. https://mariadb.com/kb/en/version/
			string strVersionLower=Db.GetScalar("SELECT @@version").ToLower();
			return strVersionLower.Contains("mariadb");
		}
		...
		public static void EnableIndexesIfNeeded(List<string> listTableNames) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				Meth.GetVoid(MethodBase.GetCurrentMethod(),listTableNames);
				return;
			}
			string command="";
