...
namespace OpenDentBusiness.Eclaims {
	...
	public class CCDFieldInputter{
		...
		public CCDFieldInputter(string message){
			messageText=message;
			string version=message.Substring(18,2);//Field A03
			...
			if(version=="04") {
				switch(MsgType) {
					...
					case "04":
						...
						break;
					//case "05"://Summary Reconciliation Request
						//break;
					//case "06"://Payment Reconciliation Request
						//break;
					case "07":
		...
		public string InputField(string message,string fieldId){
			CCDField field=new CCDField(fieldId,MsgType,isVersion2);
		...
		public string InputFields(string message,string fieldOrderStr) {
			...
			for(int i=0;i<fieldOrderStr.Length;i+=3) {
				...
				else if(token=="___") {//Designates one character "For Future Use".
		...
		private void ParseAttachmentRequest_09(string message) {
			...
			CCDField fieldF44=this.GetFieldById("F44");
			if(fieldF44==null) {
				return;//error
			}
			...
			for(int i=0;i<Convert.ToInt32(fieldF44.valuestr);i++) {
