...
namespace OpenDentBusiness {
	...
	public class Phones {
		...
		///<summary>Nested class that defines a color scheme for a phone. FormMapHQ uses DUAL while PhoneTile uses SINGLE.</summary>
		...
		///<summary>Gets every phone entry in the database.</summary>
		...
		///<summary>This is a pretty short string: 4-6 char.</summary>
		...
		public static void SetPhoneStatus(ClockStatusEnum clockStatus,int extens,long employeeNum=-1) {
			...
			//Once the old map is gone, this section should be removed.
		...
		public static void GetPhoneColor(Phone phone,PhoneEmpDefault phoneEmpDefault,bool isForDualColorScheme,out Color colorOuter,out Color colorInner,out Color colorFont,out bool isTriageOperatorOnTheClock) 
		{
			...
			if(phone.ClockStatus==ClockStatusEnum.Home
				...
				colorOuter=colorScheme.ColorOuterHome;
				colorInner=colorScheme.ColorInnerHome;
				return;
			}
			if(phone.ClockStatus==ClockStatusEnum.NeedsHelp) { //get this person help now!
				colorOuter=colorScheme.ColorOuterNeedsHelp;
				colorInner=colorScheme.ColorInnerNeedsHelp;
				return;
			}
			if(phone.ClockStatus==ClockStatusEnum.HelpOnTheWay) { //getting this person help now!!!
				colorOuter=colorScheme.ColorOuterHelpOnTheWay;
				colorInner=colorScheme.ColorInnerHelpOnTheWay;
				return;
			}
			if(!phoneEmpDefault.HasColor) { //not colored (generally an engineer or admin)
				colorOuter=colorScheme.ColorOuterNoColor;
				colorInner=colorScheme.ColorInnerNoColor;
				return;
			}
			if(phone.ClockStatus==ClockStatusEnum.Unavailable) { //Unavailable is very rare and must be approved by management. Make them look like admin/engineer.
				colorOuter=colorScheme.ColorOuterUnavailable;
				colorInner=colorScheme.ColorInnerUnavailable;
				return;
			}
			//If we get this far then the person is a tech who is working today.
			if(phoneEmpDefault.IsTriageOperator) {
				colorOuter=colorScheme.ColorOuterTriage;
			...
			if(phone.Description!="") { //Description field only has 'in use' when person is on the phone. That is the only time the field is not empty.
				colorOuter=colorScheme.ColorOuterOnPhone;
				colorInner=colorScheme.ColorInnerOnPhone;
				return;
			}
			...
			switch(phone.ClockStatus) {
				case ClockStatusEnum.Lunch:
				case ClockStatusEnum.Break:
					colorOuter=colorScheme.ColorOuterLunchBreak;
					colorInner=colorScheme.ColorInnerLunchBreak;
					return;
				case ClockStatusEnum.Available:
					colorOuter=colorScheme.ColorOuterAvailable;
					colorInner=colorScheme.ColorInnerAvailable;
					return;
				case ClockStatusEnum.WrapUp:
				case ClockStatusEnum.Training:
					colorOuter=colorScheme.ColorOuterTrainingWrap;
					colorInner=colorScheme.ColorInnerTrainingWrap;
					return;
				case ClockStatusEnum.TeamAssist:
				case ClockStatusEnum.OfflineAssist:
				case ClockStatusEnum.TCResponder:
					colorOuter=colorScheme.ColorOuterAssist;
					colorInner=colorScheme.ColorInnerAssist;
					return;
				case ClockStatusEnum.Backup:
					colorOuter=colorScheme.ColorOuterBackup;
					colorInner=colorScheme.ColorInnerBackup;
					return;
				default:
					break;
			}
		...
		public static Phone GetPhoneForExtension(List<Phone> phoneList,int extens) {
			Meth.NoCheckMiddleTierRole();
			return phoneList.Find(x => x.Extension==extens);
		}
