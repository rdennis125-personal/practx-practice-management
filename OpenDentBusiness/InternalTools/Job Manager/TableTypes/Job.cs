...
namespace OpenDentBusiness {
	...
	public class Job:TableBase {
		...
		[CrudColumn(SpecialType=CrudSpecialColType.IsText)]//Text
		...
		///<summary>FK to job.JobNum. This top level node of the job tree.</summary>
		public long TopParentNum;
			...
			get {
				...
				switch(PhaseCur) {
					...
					case JobPhase.Definition:
						return UserNumExpert;
					case JobPhase.Quote:
						return UserNumExpert>0 ? UserNumExpert : UserNumQuoter;
					case JobPhase.Development:
						if(GetMostRecentReview() is JobReview jobReview && jobReview.ReviewStatus==JobReviewStatus.Done) {
							return UserNumTester>0 ? UserNumTester : jobReview.ReviewerNum;
						}
						return UserNumEngineer>0 ? UserNumEngineer : UserNumTester;
					case JobPhase.Documentation:
						return UserNumEngineer>0 ? UserNumEngineer : UserNumDocumenter;
					default:
						return 0;
				}
			}
			...
			get {
				...
				switch(this.PhaseCur) {
					...
					case JobPhase.Development:
						...
						if(this.ListJobReviews.Any(x=>x.ReviewStatus.In(JobReviewStatus.Seen, JobReviewStatus.Sent, JobReviewStatus.UnderReview))) {
		...
		public JobAction OwnerActionForQuery {
			get {
				...
				switch(PhaseCur) {
					case JobPhase.Concept:
						Patient quotePatient=Patients.GetPat(ListJobQuotes.FirstOrDefault()?.PatNum??0);
						if(quotePatient.BalTotal-quotePatient.Bal_0_30>0.00) {
							return JobAction.ApproveConcept;
						}
						return JobAction.WriteConcept;
					case JobPhase.Definition:
						...
						if(DateTime.Today>=DateTimeCustContact.AddDays(2)) {
							return JobAction.ContactCustomer;
						}
						return JobAction.WriteJob;
					case JobPhase.Quote:
						if(ListJobQuotes.Any(x=>PIn.Float(x.Hours??"")==0 && PIn.Float(x.Amount??"")==0)) {
							return JobAction.NeedsQuote;
						}
						return JobAction.NeedsCustomerQuoteApproval;
					case JobPhase.Development:
						...
						if(UserNumTester==0 && IsApprovalNeeded) {
							return JobAction.NeedsReviewer;
						}
						//If at least one review is marked done then the job needs no more reviews.
						//Unless it came back with issues...
						if(ListJobReviews.LastOrDefault() is JobReview jobReview) {
							//DateTimeImplemented is used by Query Jobs to store when the customer approved the quote
							if(jobReview.ReviewStatus==JobReviewStatus.Done && jobReview.DateTStamp > AckDateTime) {
								return JobAction.DeliverFinalQuery;
							}
							if(jobReview.ReviewerNum==0 || jobReview.ReviewStatus.In(JobReviewStatus.Sent, JobReviewStatus.Seen, JobReviewStatus.UnderReview, JobReviewStatus.NeedsAdditionalReview)) {
								return JobAction.ReviewCode;
							}
						}
						return JobAction.WriteCode;
					case JobPhase.Documentation:
						if(DateTime.Today>AckDateTime.AddDays(90)) {
							return JobAction.NeedsTechnicalWriter;
						}
		...
		public JobAction ActionForQueryUser(Userod user=null) {
			user??=Security.CurUser;
			if(Category!=JobCategory.Query || !JobPermissions.HasQueryPermission(userNum:user.UserNum)) {
				return ActionForUser(user);
			}
			JobAction baseAction=OwnerActionForQuery;
			switch(baseAction) {
				case JobAction.Advise:
					if(JobPermissions.HasQueryPermission(JobPerm.SeniorQueryCoordinator)) {
						return baseAction;
					}
					break;
				case JobAction.AnswerQuestion:
					if(UserNumInfo==user.UserNum) {
						return baseAction;
					}
					break;
				...
				case JobAction.ApproveQuote:
					...
					if(user.UserNum.In(UserNumEngineer, UserNumExpert) || (UserNumConcept==user.UserNum && PhaseCur==JobPhase.Concept)) {
				...
				case JobAction.ContactCustomer:
					if(UserNumCustContact==user.UserNum || (JobPermissions.HasQueryPermission(JobPerm.QueryCoordinator) && UserNumCustContact==0)) {
						return baseAction;
					}
					break;
				case JobAction.DeliverFinalQuery:
					if(UserNumTester==user.UserNum || JobPermissions.HasQueryPermission(JobPerm.SeniorQueryCoordinator)) {
						return baseAction;
					}
					break;
				...
				case JobAction.NeedsQuote:
					//If there is no quoter set or you are the quoter && you have appropriate permissions
					if((UserNumQuoter==0 || UserNumQuoter==user.UserNum) && JobPermissions.HasQueryPermission(JobPerm.QueryCoordinator)) {
						return baseAction;
					}
					return JobAction.Undefined;
				case JobAction.NeedsReviewer:
					if(JobPermissions.HasQueryPermission(JobPerm.QueryCoordinator)) {
						return baseAction;
					}
					return JobAction.Undefined;
				...
				case JobAction.Document:
					...
					if((UserNumCustContact==0 || UserNumCustContact==user.UserNum) && DateTimeCustContact.Year<1880) {
						return JobAction.ContactCustomerPreDoc;
					}
					break;
				case JobAction.ReviewCode:
					if(UserNumEngineer==user.UserNum) {
						return JobAction.WaitForReview;
					}
					//If there is no reviewer set or you are the reviewer && you have appropriate permissions
					if((UserNumTester==0 || UserNumTester==user.UserNum) && JobPermissions.HasQueryPermission(JobPerm.SeniorQueryCoordinator)) {
						return baseAction;
					}
					return JobAction.Undefined;
				...
				case JobAction.WriteCode:
					...
					if(UserNumExpert==user.UserNum) {
						return JobAction.Advise;
					}
					break;
				case JobAction.WriteConcept:
					if(UserNumConcept==0 || UserNumConcept==user.UserNum) {
						return baseAction;
					}
					break;
				case JobAction.WriteJob:
					if((UserNumExpert==0 || UserNumExpert==user.UserNum) && JobPermissions.HasQueryPermission(JobPerm.QueryCoordinator)) {
						return baseAction;
					}
					break;
				case JobAction.Undefined:
				case JobAction.None:
					return baseAction;
				default:
		...
		///<summary>Similar To owner action, but allows you to specify a user. Unlike OwnerAction Property, this method also checks user permissions to prevent people from getting to stuff they shouldn't via viewing jobs "as someone else".</summary>
		...
		private JobReview GetMostRecentReview() {
			return ListJobReviews.Find(x=>x.DateTStamp==ListJobReviews.Max(y=>y.DateTStamp));
		}
	...
	///<summary>A stripped down job object designed for use when generating the related job tree.</summary>
	[Serializable]
	public class JobShort {
		...
		///<summary></summary>
		public JobShort(long jobNum,long parentNum,long topParentNum) {
			JobNum=jobNum;
			ParentNum=parentNum;
			TopParentNum=topParentNum;
		}
	}
