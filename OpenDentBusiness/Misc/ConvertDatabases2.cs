						...
						for(int j=0;j<procsForOne.Length;j++) {
							...
							switch(procCode.TreatArea.ToString()) {
								case "Surf"://TreatmentArea.Surf:
									procDescript+="#"+Tooth.Display(procsForOne[j].ToothNum)+"-"
									+procsForOne[j].Surf+"-";//""#12-MOD-"
									break;
								case "Tooth"://TreatmentArea.Tooth:
									procDescript+="#"+Tooth.Display(procsForOne[j].ToothNum)+"-";//"#12-"
									break;
			...
			if(FromVersion<new Version("12.4.1.0")) {
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					for(int i=0;i<table.Rows.Count;i++) {
						groupNum=PIn.Long(table.Rows[i]["UserGroupNum"].ToString());
						command="INSERT INTO grouppermission (UserGroupNum,PermType) "
				      +"VALUES("+POut.Long(groupNum)+",63)"; // 63 - ProblemDefEdit permission
						Db.NonQ(command);
					}
				}
				else {//oracle
					for(int i=0;i<table.Rows.Count;i++) {
						groupNum=PIn.Long(table.Rows[i]["UserGroupNum"].ToString());
						command="INSERT INTO grouppermission (GroupPermNum,NewerDays,UserGroupNum,PermType) "
				      +"VALUES((SELECT MAX(GroupPermNum)+1 FROM grouppermission),0,"+POut.Long(groupNum)+",63)"; // 63 - ProblemDefEdit permission
						Db.NonQ(command);
					}
				}
				if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To12_4_32() {
			if(FromVersion<new Version("12.4.32.0")) {
				...
				if(CultureInfo.CurrentCulture.Name.EndsWith("US")) {//United States
					...
					Db.NonQ(command);
					//Appt procs quick add, change the fluoride code to D1208 instead of D1203/4.
					...
					//proc buttons, change fluoride  to D1208 instead of D1203/4. We don't care about possible duplicates because they are highly unlikely.
