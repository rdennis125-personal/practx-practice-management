	...
	public partial class ConvertDatabases {
		...
		private static void To23_1_0() {
			...
			if(table.Rows.Count>0) {
				...
				bool showFeatureSuperFamilies=PIn.Bool(PIn.String(Db.GetTable(command).Rows[0]["ValueString"].ToString()));
				...
				if(showFeatureSuperFamilies) {
					...
					for(int j=0;j<table.Rows.Count;j++) {
						if(PIn.String(table.Rows[j]["InternalName"].ToString())=="Patient") {
