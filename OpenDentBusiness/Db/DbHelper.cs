...
namespace OpenDentBusiness {
	...
	public class DbHelper {
		...
		///a string comparison such as PatNum LIKE '1234%'. If 'isFromMobile' is true, will use 'OR' statement instead of 'AND' statement.</summary>
		public static string LongBetween(string colName,string val,bool isFromMobile=false) {
			long valLong=0;
			StringBuilder retVal=new StringBuilder();
			long.TryParse(val,out valLong);
			if(valLong>0) {
				if(isFromMobile) {
					//isFromMobile indicates the Patients.GetPtDataTable() method was called from ODMobile. When calling from ODMobile, we cannot differentiate
					//between patnums and phonenumbers, so this 'OR' allows us to search for patients that match on phone number OR on patnum.
					retVal.Append("OR ("+POut.String(colName)+"="+POut.Long(valLong)+" ");
				}
				else {
