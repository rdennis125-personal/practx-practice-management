...
namespace OpenDentBusiness {
	...
	public class LargeTableHelper {
		...
		public static void RenameDatabase(string databaseNameCur,string databaseNameNew) {
			...
			#endregion
			#region Rename
			//Get the names of all tables within databaseNameCur.  Only 'BASE TABLE' types to exclude views
			command="SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA='"+POut.String(databaseNameCur)+"' AND TABLE_TYPE='BASE TABLE'";
			table=Db.GetTable(command);
			...
			//Verify that there are no tables left within databaseNameCur.  Only 'BASE TABLE' types to exclude views
			command="SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA='"+POut.String(databaseNameCur)+"' AND TABLE_TYPE='BASE TABLE'";
			table=Db.GetTable(command);
			if(table.Rows.Count > 0) {
		...
		public static void SetListPriKeyMaxPerBatch(string whereClause="") {
			...
			string command="SET @row=0,@maxPriKey=0; "
				...
				+"FROM ("
					...
					+") a "
					+"ORDER BY a.priKey "//this order by is specifically for MariaDB, since the subquery order by is ignored but works for MySQL
				+") b "
		...
		public static bool ColumnExists(string dbName,string tableName,string colName) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				return Meth.GetBool(MethodBase.GetCurrentMethod(),dbName,tableName,colName);
			}
			...
			List<Tuple<string,string>> indexColsAndNames=null)
		{
			...
			if(colNamesAndDefs.Count>0) {
				listChanges.AddRange(colNamesAndDefs.Select(x => (ColumnExists(dbName,_tableName,x.Item1)?"MODIFY":"ADD")
		...
		///<summary>Just for large tables instead of raw queries. Examples are a few lines down from here. Do NOT try-catch this method, even when only adding an index. This will rename the table specified to be preceded by "convTmp_"+tableName+"random number", creates a copy of the table structure and indexes, adds the column(s) and/or indexes. Once the empty table clone is made, keys will be disabled (for MyISAM only), all rows will be inserted from the renamed convTmp_ table, the keys (indexes) will be enabled (MyISAM only), and the convTmp_ table will be dropped.</summary>
		...
		List<ColNameAndDef> listColNamesAndDefs=null,List<IndexColsAndName> listIndexColsAndNames=null)
		{
			...
			//listColNameAndDefs.Add(new ColNameAndDef("DescriptOverride","varchar(255) NOT NULL"));
			//listColNameAndDefs.Add(new ColNameAndDef("Test","varchar(255) NOT NULL"));
			//AlterTable("taskhist","TaskHistNum",listColNameAndDef:listColNameAndDefs);
			//
			//(Example Index):
			...
			if(listColNamesAndDefs.Count>0) {
				for(int i=0;i<listColNamesAndDefs.Count;i++) {
					if(ColumnExists(dbName,_tableName,listColNamesAndDefs[i].ColumnName)){
			...
			if(listIndexColsAndNames.Count>0) {
				...
				List<string> listRedundantIndexes=listIndexColsAndNames.SelectMany(x => GetRedundantIndexNames(_tableName,x.IndexColumns)).ToList();
				if(listRedundantIndexes.Count>0) {
					listChanges.AddRange(listRedundantIndexes.Select(x => "DROP INDEX `"+x+"`"));
				}
			}
			if(listChanges.Count<1) {
				return;//nothing to do, just return
		...
		///<summary>Returns index names for the table that will become redundant with the addition of a new indexed columns.  Calls GetIndexNames so the names returned can be used in
		///drop index statements.  Excludes any index named 'PRIMARY'.
		///Example: if a table has existing indexes: PatNum (PatNum), PatNumStatus (PatNum,PatStatus), and PatNumBDate (PatNum,Birthdate)
		///you were to add the index: PatNumStatusBirthdate (PatNum,PatStatus,Birthdate)
		///you would call this method with newIndexColNames: "PatNum,PatStatus,Birthdate"
		///the return value would be: list with index names PatNum and PatNumStatus, both would be redundant because they are left-most prefixes of the new index.</summary>
		public static List<string> GetRedundantIndexNames(string tableName,string newIndexColNames) {
			List<string> retval=new List<string>();
			List<string> listNewIndexCols=newIndexColNames.Split(",",StringSplitOptions.RemoveEmptyEntries).ToList();
			if(listNewIndexCols.Count==0) {
				return retval;
			}
			for(int i=0;i<listNewIndexCols.Count;i++) {
				retval.AddRange(GetIndexNames(tableName,string.Join(",",listNewIndexCols.Take(i+1))).FindAll(x => x.ToLower()!="primary"));
			}
			return retval;
		}
