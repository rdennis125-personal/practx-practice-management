	...
	public class OperatoryCrud {
		...
		public static List<Operatory> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				operatory.OperatoryType = PIn.Long  (row["OperatoryType"].ToString());
		...
		public static DataTable ListToTable(List<Operatory> listOperatorys,string tableName="") {
			...
			foreach(Operatory operatory in listOperatorys) {
				table.Rows.Add(new object[] {
					...
					POut.Long  (operatory.OperatoryType),
				});
			}
			return table;
		}
		...
		public static void Update(Operatory operatory) {
			string command="UPDATE operatory SET "
				...
				+"OperatoryType =  "+POut.Long  (operatory.OperatoryType)+" "
		...
		public static bool Update(Operatory operatory,Operatory oldOperatory) {
			...
			if(operatory.OperatoryType != oldOperatory.OperatoryType) {
				if(command!="") { command+=",";}
				command+="OperatoryType = "+POut.Long(operatory.OperatoryType)+"";
			}
			if(command=="") {
				return false;
			}
		...
		public static bool Sync(List<Operatory> listNew,List<Operatory> listDB) {
			...
			listNew.Sort((Operatory x,Operatory y) => { return x.OperatoryNum.CompareTo(y.OperatoryNum); });
			listDB.Sort((Operatory x,Operatory y) => { return x.OperatoryNum.CompareTo(y.OperatoryNum); });
			int idxNew=0;
			int idxDB=0;
