	...
	public class ApptViewCrud {
		...
		public static List<ApptView> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				apptView.OnlyScheduledProvDays= PIn.Bool  (row["OnlyScheduledProvDays"].ToString());
				retVal.Add(apptView);
			}
			return retVal;
		}
		...
		public static DataTable ListToTable(List<ApptView> listApptViews,string tableName="") {
			...
			foreach(ApptView apptView in listApptViews) {
				table.Rows.Add(new object[] {
					...
					POut.Bool  (apptView.OnlyScheduledProvDays),
				});
			}
			return table;
		}
		...
		public static void Update(ApptView apptView) {
			string command="UPDATE apptview SET "
				...
				+"OnlyScheduledProvDays=  "+POut.Bool  (apptView.OnlyScheduledProvDays)+" "
		...
		public static bool Update(ApptView apptView,ApptView oldApptView) {
			...
			if(apptView.OnlyScheduledProvDays != oldApptView.OnlyScheduledProvDays) {
				if(command!="") { command+=",";}
				command+="OnlyScheduledProvDays = "+POut.Bool(apptView.OnlyScheduledProvDays)+"";
			}
			if(command=="") {
				return false;
			}
