	...
	public class CommlogCrud {
		...
		public static List<Commlog> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				commlog.DateTEntry          = PIn.DateT (row["DateTEntry"].ToString());
				commlog.ReferralNum         = PIn.Long  (row["ReferralNum"].ToString());
				commlog.CommReferralBehavior= (OpenDentBusiness.EnumCommReferralBehavior)PIn.Int(row["CommReferralBehavior"].ToString());
				retVal.Add(commlog);
			}
			return retVal;
		}
		...
		public static DataTable ListToTable(List<Commlog> listCommlogs,string tableName="") {
			...
			foreach(Commlog commlog in listCommlogs) {
				table.Rows.Add(new object[] {
					...
					POut.DateT (commlog.DateTEntry,false),
					POut.Long  (commlog.ReferralNum),
					POut.Int   ((int)commlog.CommReferralBehavior),
				});
			}
			return table;
		}
		...
		public static void Update(Commlog commlog) {
			string command="UPDATE commlog SET "
				...
				+"ReferralNum         =  "+POut.Long  (commlog.ReferralNum)+", "
				+"CommReferralBehavior=  "+POut.Int   ((int)commlog.CommReferralBehavior)+" "
		...
		public static bool Update(Commlog commlog,Commlog oldCommlog) {
			...
			if(commlog.ReferralNum != oldCommlog.ReferralNum) {
				if(command!="") { command+=",";}
				command+="ReferralNum = "+POut.Long(commlog.ReferralNum)+"";
			}
			if(commlog.CommReferralBehavior != oldCommlog.CommReferralBehavior) {
				if(command!="") { command+=",";}
				command+="CommReferralBehavior = "+POut.Int   ((int)commlog.CommReferralBehavior)+"";
			}
			if(command=="") {
				return false;
			}
