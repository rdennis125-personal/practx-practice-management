	...
	public class ApptReminderRuleCrud {
		...
		public static List<ApptReminderRule> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				apptReminderRule.SendMultipleInvites       = (OpenDentBusiness.SendMultipleInvites)PIn.Int(row["SendMultipleInvites"].ToString());
				apptReminderRule.TimeSpanMultipleInvites   = TimeSpan.FromTicks(PIn.Long(row["TimeSpanMultipleInvites"].ToString()));
		...
		public static DataTable ListToTable(List<ApptReminderRule> listApptReminderRules,string tableName="") {
			...
			table.Columns.Add("SendMultipleInvites");
			table.Columns.Add("TimeSpanMultipleInvites");
			foreach(ApptReminderRule apptReminderRule in listApptReminderRules) {
				table.Rows.Add(new object[] {
					...
					POut.Int   ((int)apptReminderRule.SendMultipleInvites),
					POut.Long (apptReminderRule.TimeSpanMultipleInvites.Ticks),
				});
			}
			return table;
		}
		...
		public static void Update(ApptReminderRule apptReminderRule) {
			string command="UPDATE apptreminderrule SET "
				...
				+"SendMultipleInvites       =  "+POut.Int   ((int)apptReminderRule.SendMultipleInvites)+", "
				+"TimeSpanMultipleInvites   =  "+POut.Long  (apptReminderRule.TimeSpanMultipleInvites.Ticks)+" "
		...
		public static bool Update(ApptReminderRule apptReminderRule,ApptReminderRule oldApptReminderRule) {
			...
			if(apptReminderRule.SendMultipleInvites != oldApptReminderRule.SendMultipleInvites) {
				if(command!="") { command+=",";}
				command+="SendMultipleInvites = "+POut.Int   ((int)apptReminderRule.SendMultipleInvites)+"";
			}
			if(apptReminderRule.TimeSpanMultipleInvites != oldApptReminderRule.TimeSpanMultipleInvites) {
				if(command!="") { command+=",";}
				command+="TimeSpanMultipleInvites = '"+POut.Long  (apptReminderRule.TimeSpanMultipleInvites.Ticks)+"'";
			}
			if(command=="") {
				return false;
			}
		...
		public static bool Sync(List<ApptReminderRule> listNew,List<ApptReminderRule> listDB) {
			...
			listNew.Sort((ApptReminderRule x,ApptReminderRule y) => { return x.ApptReminderRuleNum.CompareTo(y.ApptReminderRuleNum); });
			listDB.Sort((ApptReminderRule x,ApptReminderRule y) => { return x.ApptReminderRuleNum.CompareTo(y.ApptReminderRuleNum); });
			int idxNew=0;
			int idxDB=0;
