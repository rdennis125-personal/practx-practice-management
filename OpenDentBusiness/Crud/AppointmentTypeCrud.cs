	...
	public class AppointmentTypeCrud {
		...
		public static List<AppointmentType> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				appointmentType.CodeStrRequired        = PIn.String(row["CodeStrRequired"].ToString());
				appointmentType.RequiredProcCodesNeeded= (OpenDentBusiness.EnumRequiredProcCodesNeeded)PIn.Int(row["RequiredProcCodesNeeded"].ToString());
				appointmentType.BlockoutTypes          = PIn.String(row["BlockoutTypes"].ToString());
			...
			foreach(AppointmentType appointmentType in listAppointmentTypes) {
				table.Rows.Add(new object[] {
					...
					POut.Bool  (appointmentType.IsHidden),
					            ...
					            appointmentType.CodeStrRequired,
					POut.Int   ((int)appointmentType.RequiredProcCodesNeeded),
		...
		public static long Insert(AppointmentType appointmentType,bool useExistingPK) {
			...
			command+=
				...
				+"'"+POut.String(appointmentType.CodeStrRequired)+"',"
				+    POut.Int   ((int)appointmentType.RequiredProcCodesNeeded)+","
				+"'"+POut.String(appointmentType.BlockoutTypes)+"')";
			if(useExistingPK || PrefC.RandomKeys) {
				Db.NonQ(command);
			}
			else {
		...
		public static void Update(AppointmentType appointmentType) {
			string command="UPDATE appointmenttype SET "
				...
				+"CodeStrRequired        = '"+POut.String(appointmentType.CodeStrRequired)+"', "
				+"RequiredProcCodesNeeded=  "+POut.Int   ((int)appointmentType.RequiredProcCodesNeeded)+", "
				+"BlockoutTypes          = '"+POut.String(appointmentType.BlockoutTypes)+"' "
		...
		public static bool Update(AppointmentType appointmentType,AppointmentType oldAppointmentType) {
			...
			if(appointmentType.CodeStrRequired != oldAppointmentType.CodeStrRequired) {
				if(command!="") { command+=",";}
				command+="CodeStrRequired = '"+POut.String(appointmentType.CodeStrRequired)+"'";
			}
			if(appointmentType.RequiredProcCodesNeeded != oldAppointmentType.RequiredProcCodesNeeded) {
				if(command!="") { command+=",";}
				command+="RequiredProcCodesNeeded = "+POut.Int   ((int)appointmentType.RequiredProcCodesNeeded)+"";
			}
			if(appointmentType.BlockoutTypes != oldAppointmentType.BlockoutTypes) {
				if(command!="") { command+=",";}
				command+="BlockoutTypes = '"+POut.String(appointmentType.BlockoutTypes)+"'";
			}
			if(command=="") {
				return false;
			}
		...
		public static bool Sync(List<AppointmentType> listNew,List<AppointmentType> listDB) {
			...
			listNew.Sort((AppointmentType x,AppointmentType y) => { return x.AppointmentTypeNum.CompareTo(y.AppointmentTypeNum); });
			listDB.Sort((AppointmentType x,AppointmentType y) => { return x.AppointmentTypeNum.CompareTo(y.AppointmentTypeNum); });
			int idxNew=0;
			int idxDB=0;
