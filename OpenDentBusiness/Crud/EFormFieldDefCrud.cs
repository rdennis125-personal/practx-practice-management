	...
	public class EFormFieldDefCrud {
		...
		public static List<EFormFieldDef> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				eFormFieldDef.WidthLabel       = PIn.Int   (row["WidthLabel"].ToString());
				eFormFieldDef.SpaceToRight     = PIn.Int   (row["SpaceToRight"].ToString());
		...
		public static DataTable ListToTable(List<EFormFieldDef> listEFormFieldDefs,string tableName="") {
			...
			foreach(EFormFieldDef eFormFieldDef in listEFormFieldDefs) {
				table.Rows.Add(new object[] {
					...
					POut.Int   (eFormFieldDef.WidthLabel),
					POut.Int   (eFormFieldDef.SpaceToRight),
				});
			}
			return table;
		}
		...
		public static void Update(EFormFieldDef eFormFieldDef) {
			string command="UPDATE eformfielddef SET "
				...
				+"WidthLabel       =  "+POut.Int   (eFormFieldDef.WidthLabel)+", "
				+"SpaceToRight     =  "+POut.Int   (eFormFieldDef.SpaceToRight)+" "
		...
		public static bool Update(EFormFieldDef eFormFieldDef,EFormFieldDef oldEFormFieldDef) {
			...
			if(eFormFieldDef.WidthLabel != oldEFormFieldDef.WidthLabel) {
				if(command!="") { command+=",";}
				command+="WidthLabel = "+POut.Int(eFormFieldDef.WidthLabel)+"";
			}
			if(eFormFieldDef.SpaceToRight != oldEFormFieldDef.SpaceToRight) {
				if(command!="") { command+=",";}
				command+="SpaceToRight = "+POut.Int(eFormFieldDef.SpaceToRight)+"";
			}
			if(command=="") {
				return false;
			}
		...
		public static bool Sync(List<EFormFieldDef> listNew,List<EFormFieldDef> listDB) {
			...
			List<EFormFieldDef> listIns    =new List<EFormFieldDef>();
			List<EFormFieldDef> listUpdNew =new List<EFormFieldDef>();
			List<EFormFieldDef> listUpdDB  =new List<EFormFieldDef>();
			List<EFormFieldDef> listDel    =new List<EFormFieldDef>();
			listNew.Sort((EFormFieldDef x,EFormFieldDef y) => { return x.EFormFieldDefNum.CompareTo(y.EFormFieldDefNum); });
			listDB.Sort((EFormFieldDef x,EFormFieldDef y) => { return x.EFormFieldDefNum.CompareTo(y.EFormFieldDefNum); });
			int idxNew=0;
			int idxDB=0;
			...
			while(idxNew<listNew.Count || idxDB<listDB.Count) {
				...
				else if(fieldNew.EFormFieldDefNum<fieldDB.EFormFieldDefNum) {//newPK less than dbPK, newItem is 'next'
					listIns.Add(fieldNew);
					idxNew++;
					continue;
				}
				else if(fieldNew.EFormFieldDefNum>fieldDB.EFormFieldDefNum) {//dbPK less than newPK, dbItem is 'next'
					listDel.Add(fieldDB);
					idxDB++;
					continue;
				}
