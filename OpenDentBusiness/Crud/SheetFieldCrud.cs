	...
	public class SheetFieldCrud {
		...
		public static List<SheetField> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				sheetField.SheetFieldDefNum        = PIn.Long  (row["SheetFieldDefNum"].ToString());
				sheetField.CanElectronicallySign   = PIn.Bool  (row["CanElectronicallySign"].ToString());
				sheetField.IsSigProvRestricted     = PIn.Bool  (row["IsSigProvRestricted"].ToString());
		...
		public static DataTable ListToTable(List<SheetField> listSheetFields,string tableName="") {
			...
			foreach(SheetField sheetField in listSheetFields) {
				table.Rows.Add(new object[] {
					...
					POut.Long  (sheetField.SheetFieldDefNum),
					POut.Bool  (sheetField.CanElectronicallySign),
					POut.Bool  (sheetField.IsSigProvRestricted),
				});
			}
			return table;
		}
		...
		public static void InsertMany(List<SheetField> listSheetFields,bool useExistingPK) {
			if(!useExistingPK && PrefC.RandomKeys) {
				...
				while(index < listSheetFields.Count) {
					...
					sbRow.Append(POut.Long(sheetField.SheetFieldDefNum)); sbRow.Append(",");
					sbRow.Append(POut.Bool(sheetField.CanElectronicallySign)); sbRow.Append(",");
					sbRow.Append(POut.Bool(sheetField.IsSigProvRestricted)); sbRow.Append(")");
		...
		public static void Update(SheetField sheetField) {
			string command="UPDATE sheetfield SET "
				...
				+"SheetFieldDefNum        =  "+POut.Long  (sheetField.SheetFieldDefNum)+", "
				+"CanElectronicallySign   =  "+POut.Bool  (sheetField.CanElectronicallySign)+", "
				+"IsSigProvRestricted     =  "+POut.Bool  (sheetField.IsSigProvRestricted)+" "
		...
		public static bool Update(SheetField sheetField,SheetField oldSheetField) {
			...
			if(sheetField.SheetFieldDefNum != oldSheetField.SheetFieldDefNum) {
				if(command!="") { command+=",";}
				command+="SheetFieldDefNum = "+POut.Long(sheetField.SheetFieldDefNum)+"";
			}
			if(sheetField.CanElectronicallySign != oldSheetField.CanElectronicallySign) {
				if(command!="") { command+=",";}
				command+="CanElectronicallySign = "+POut.Bool(sheetField.CanElectronicallySign)+"";
			}
			if(sheetField.IsSigProvRestricted != oldSheetField.IsSigProvRestricted) {
				if(command!="") { command+=",";}
				command+="IsSigProvRestricted = "+POut.Bool(sheetField.IsSigProvRestricted)+"";
			}
			if(command=="") {
				return false;
			}
		...
		public static bool Sync(List<SheetField> listNew,List<SheetField> listDB) {
			...
			listNew.Sort((SheetField x,SheetField y) => { return x.SheetFieldNum.CompareTo(y.SheetFieldNum); });
			listDB.Sort((SheetField x,SheetField y) => { return x.SheetFieldNum.CompareTo(y.SheetFieldNum); });
			int idxNew=0;
			int idxDB=0;
