	...
	public class JobCrud {
		...
		public static List<Job> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				job.PhaseCur               = (OpenDentBusiness.JobPhase)PIn.Int(row["PhaseCur"].ToString());
				...
				job.TopParentNum           = PIn.Long  (row["TopParentNum"].ToString());
				retVal.Add(job);
			}
			return retVal;
		}
		...
		public static DataTable ListToTable(List<Job> listJobs,string tableName="") {
			...
			table.Columns.Add("TopParentNum");
		...
		public static void Update(Job job) {
			string command="UPDATE job SET "
				...
				+"PhaseCur               =  "+POut.Int   ((int)job.PhaseCur)+", "
				...
				+"TopParentNum           =  "+POut.Long  (job.TopParentNum)+" "
		...
		public static bool Update(Job job,Job oldJob) {
			...
			if(job.PhaseCur != oldJob.PhaseCur) {
				if(command!="") { command+=",";}
				command+="PhaseCur = "+POut.Int   ((int)job.PhaseCur)+"";
			}
			...
			if(job.TopParentNum != oldJob.TopParentNum) {
				if(command!="") { command+=",";}
				command+="TopParentNum = "+POut.Long(job.TopParentNum)+"";
			}
			if(command=="") {
				return false;
			}
