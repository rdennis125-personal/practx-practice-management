...
namespace UnitTests.MiddleTier {
	...
	public class MiddleTierTests:TestBase {
		...
		[TestMethod]
		public void MiddleTier_GetAndSendEnum_Bitwise() {
			//This unit test is for making sure bitwise enums can be passed back and forth between Middle Tier.
			...
			//Send the bitwise enum as a parameter over to Middle Tier and it will return the parameter provided as a return value.
			HostedEmailStatus hostedEmailStatusActual=WebServiceTests.GetAndSendEnum(hostedEmailStatusExpected);
		...
		[TestMethod]
		public void MiddleTier_SendTimeSpanParam() {
			TimeSpan timeSpanExpected = new TimeSpan(1,0,0);
			TimeSpan timeSpanResult = WebServiceTests.SendTimeSpanParam(timeSpanExpected);
			Assert.IsTrue(timeSpanResult!=null && timeSpanResult.Ticks==timeSpanExpected.Ticks);
		}
		...
		public void MiddleTier_GetObjectWithTable() {
			...
			PatientData patientData=WebServiceTests.GetPatientData(tableProgNotes);
			Assert.AreEqual(1,patientData.TableProgNotes.Rows.Count);
			Assert.AreEqual(tableProgNotes.Rows[0][0],patientData.TableProgNotes.Rows[0][0]);
		}
		...
		public void MiddleTier_GetPrimaryKeysFromInsert() {
			...
			foreach(MethodInfo insertMethod in listInsertMethods) {
				...
				#endregion
				#region HQ Only Skip
				else if(insertMethod.DeclaringType.Name.In("Jobs","JobTeams","JobTeamUsers","SiteLinks","BugSubmissions")) {
					continue;
				}
				#endregion
				#endregion
		...
		public void MiddleTier_DataConnection_RemotingRoleTest() {
			...
			Action a=() => {
				RemotingClient.SetRemotingRoleT(MiddleTierRole.ServerMT);
			...
			Action b=() => {
				RemotingClient.SetRemotingT("",MiddleTierRole.ClientMT,false,false);
				Assert.AreEqual(DataConnection.GetCanReconnect(),true);
			};
		...
		public void MiddleTier_IsMiddleTierAvailable_Failure() {
			MiddleTierRole roleCur=RemotingClient.MiddleTierRole;
			RemotingClient.MiddleTierRole=MiddleTierRole.ClientDirect;
			bool isSuccess=RemotingClient.IsMiddleTierAvailable();
			RemotingClient.MiddleTierRole=roleCur;
