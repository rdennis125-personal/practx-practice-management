...
namespace UnitTests {
	...
	public class PdmpPropertyTest:TestBase {
		...
		public void Bamboo_TryValidateValues_Fail() {
			...
			Assert.IsTrue(didThrow);
			didThrow=false;
			properties.Url="www.fakebamboowebsite.eu";
			try {
			properties.Validate(progCur);
			}
			catch (Exception ex) {
			...
			CDT.Class1.Encrypt("F@k3C3Rt10010010011011",out properties.BambooClientKey);
			CDT.Class1.Encrypt("f@k3p@$$w0rd",out properties.BambooClientPassword);
			try {
			properties.Validate(progCur);
			}
			catch (Exception ex) {
		...
		public void Bamboo_TryValidateValues_HappyPath() {
			...
			PdmpProperty props = new PdmpProperty() {
				...
				Url="www.fakesite.com",
				Dea="123456789",
				BambooClientKey=encryptedKey,
				BambooClientPassword=encryptedPassword
			};
			bool didThrow=false;
			try {
			props.Validate(progCur);
