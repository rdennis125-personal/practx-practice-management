...
namespace UnitTests.Procedures_Test {
	...
	public class ProceduresTest:TestBase {
		...
		private double GetProcFee(string suffix,bool doUseMedicalCode) {
			...
			return Procedures.GetProcFee(pat,listPatPlans,listSubs,listPlans,proc);
		}
		...
		public void Procedures_GetProcFee_With0PercentCoverage() {
			...
			proc.ProcFee=Procedures.GetProcFee(pat,ins.ListPatPlans,ins.ListInsSubs,ins.ListInsPlans,proc);
		...
		public void Procedures_InsertOrUpdateInsHistProcedure_ExistingProc() {
			...
			Procedure procedure=ProcedureT.CreateProcedure(patient,"D4341",ProcStat.EO,"",0,procDate:date,surf:"UL");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			//Create a Claimproc with status InsHist
			ClaimProc claimProcPri=ClaimProcT.CreateClaimProc(patient.PatNum,procedure.ProcNum,insuranceInfo.ListInsPlans.FirstOrDefault().PlanNum,
				insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum,date,-1,-1,-1,ClaimProcStatus.InsHist);
			//Change the proc date
			date=date.AddDays(4);
			List<Procedure> listProceduresEoAndC=Procedures.GetProcsByStatusForPat(patient.PatNum,new [] { ProcStat.EO,ProcStat.C });
			List<long> listProcNums=listProceduresEoAndC.Select(x=>x.ProcNum).ToList();
			List<ClaimProc> listClaimProcsForEoAndCProcs=ClaimProcs.GetForProcs(listProcNums)
				.FindAll(y => y.InsSubNum==insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum && y.Status.In(ClaimProcStatus.InsHist,ClaimProcStatus.Received));
			List<long> listCodeNums=ProcedureCodes.GetCodeNumsForInsHistPref(PrefName.InsHistPerioULCodes);
			Procedure procedureMostRecent=Procedures.GetMostRecentInsHistProc(listProceduresEoAndC,listCodeNums,PrefName.InsHistPerioULCodes);
			//The procedure and claimproc need the new date.
			Procedures.InsertOrUpdateInsHistProcedure(patient,PrefName.InsHistPerioULCodes,date,insuranceInfo.ListInsPlans.FirstOrDefault().PlanNum,
				insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum,procedureMostRecent,listClaimProcsForEoAndCProcs);
			Procedure procedureFromDb=Procedures.GetOneProc(procedure.ProcNum,false);
			Assert.AreEqual(date,procedureFromDb.ProcDate);
			ClaimProc claimProcFromDb=ClaimProcs.GetForProcs(new List<long> { procedure.ProcNum }).FirstOrDefault();
		...
		public void Procedures_InsertOrUpdateInsHistProcedure_ExistingCProc() {
			...
			Patient patient=PatientT.CreatePatient(suffix);
			DateTime date=DateTime.Today;
			Procedure procedure=ProcedureT.CreateProcedure(patient,"D4341",ProcStat.C,"",0,procDate:date,surf:"UL");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			insuranceInfo.ListAllProcs.Add(procedure);
			Claim claim=ClaimT.CreateClaim(new List<Procedure> { procedure },insuranceInfo);
			...
			Procedures.InsertOrUpdateInsHistProcedure(patient,PrefName.InsHistPerioULCodes,date,insuranceInfo.ListInsPlans.FirstOrDefault().PlanNum,
				insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum,procedureMostRecent,listClaimProcsForEoAndCProcs);
			Procedure procedureFromDb=Procedures.Refresh(patient.PatNum).FirstOrDefault(x=>x.ProcDate.Date==date.Date);
			Assert.AreEqual(date.Date,procedureFromDb.ProcDate.Date);
			Assert.AreEqual(ProcStat.EO,procedureFromDb.ProcStatus);
			ClaimProc claimProcFromDb=ClaimProcs.GetForProcs(new List<long> { procedureFromDb.ProcNum }).FirstOrDefault();
		...
		public void Procedures_GetDictInsHistProcs_ExistingCProc() {
			...
			List<long> listCodeNums=ProcedureCodes.GetCodeNumsForInsHistPref(PrefName.InsHistPerioULCodes);
			Procedure procedureMostRecent=Procedures.GetMostRecentInsHistProc(listProceduresEoAndC,listCodeNums,PrefName.InsHistPerioULCodes);
			Assert.AreEqual(date.Date,procedureMostRecent.ProcDate.Date);
			Assert.AreEqual(ProcStat.C,procedureMostRecent.ProcStatus);
		}
		...
		public void Procedures_InsertOrUpdateInsHistProcedure_NewProc() {
			...
			List<ClaimProc> listClaimProcsForEoAndCProcs=ClaimProcs.GetForProcs(listProcNums)
				.FindAll(y => y.InsSubNum==insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum && y.Status.In(ClaimProcStatus.InsHist,ClaimProcStatus.Received));
			List<long> listCodeNums=ProcedureCodes.GetCodeNumsForInsHistPref(PrefName.InsHistProphyCodes);
			Procedure procedureMostRecent=Procedures.GetMostRecentInsHistProc(listProceduresEoAndC,listCodeNums,PrefName.InsHistProphyCodes);
			//The procedure and claimproc need the new date.
			Procedures.InsertOrUpdateInsHistProcedure(patient,PrefName.InsHistProphyCodes,date,insuranceInfo.ListInsPlans.FirstOrDefault().PlanNum,
				insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum,procedureMostRecent,listClaimProcsForEoAndCProcs);
			Procedure procedureFromDb=Procedures.GetProcsByStatusForPat(patient.PatNum,ProcStat.EO).FirstOrDefault();
			Assert.AreEqual(date,procedureFromDb.ProcDate);
			Assert.AreEqual(ProcStat.EO,procedureFromDb.ProcStatus);
			ClaimProc claimProcFromDb=ClaimProcs.GetForProcs(new List<long> { procedureFromDb.ProcNum }).FirstOrDefault();
			Assert.AreEqual(date,claimProcFromDb.ProcDate);
			Assert.AreEqual(ClaimProcStatus.InsHist,claimProcFromDb.Status);
			Assert.AreEqual(insuranceInfo.ListInsSubs.FirstOrDefault().InsSubNum,claimProcFromDb.InsSubNum);
		}
		#endregion
		...
		public void Procedures_ShouldFeesChange_CatPercentProvOverride() {
			...
			List<Fee> listFees=Fees.GetListForScheds(feeSchedIns,provNum1:provNum1,feeSched2:feeSchedIns,provNum2:provNum2);
			InsuranceInfo insInfo=InsuranceT.AddInsurance(pat,suffix,feeSchedNum:feeSchedIns);
			Procedure proc=ProcedureT.CreateProcedure(pat,"D1110",ProcStat.TP,"",100,new DateTime(2018,1,10),provNum:provNum1);
			Procedure procOld=proc.Copy();
			proc.ProvNum=provNum2;
		...
		public void Procedures_SetCompleteInApptInList_MultipleCompletedProcAttachedToClaim_OtherInsuranceReceived() {
			...
			Assert.AreEqual(120,insInfo.ListAllClaimProcs.Where(x=> listProcs.Select(y => y.ProcNum).ToList().Contains(x.ProcNum)).Sum(x => x.InsPayEst));
		}
		...
		public void Procedures_ComputeEstimates_InsEstRecalcReceivedPriNotReceivedSec_False() {
			...
			double procFee=Procedures.GetProcFee(pat,listPatPlans,listSubs,listInsPlans,procedure);
...
")]
		...
		public void Procedures_ComputeEstimates_AnnualMaxReachedZeroCoverage() {
			...
			List<ClaimProcHist> loopList=new List<ClaimProcHist>();
			List<Procedure> ProcList=Procedures.Refresh(pat.PatNum);
			List<Procedure> listProeduresTPs=Procedures.GetListTPandTPi(ProcList);//sorted by priority, then toothnum
			//Validate
			for(int i=0;i<listProeduresTPs.Count;i++) {
		...
		public void Procedures_ComputeEstimates_WriteoffPPOsPriSecSamePlan() {
			...
			List<Procedure> listProceduresTPs=Procedures.GetListTPandTPi(ProcList);//sorted by priority, then toothnum
			//Validate
			Procedures.ComputeEstimates(listProceduresTPs[0],pat.PatNum,ref claimProcs,false,planList,patPlans,benefitList,
				histList,loopList,false,pat.Age,subList);
			//save changes in the list to the database
			ClaimProcs.Synch(ref claimProcs,claimProcListOld);
			claimProcs=ClaimProcs.Refresh(pat.PatNum);
...
Patient has one insurance plan, subscriber self. Benefits: annual max $1000, deductible $50, 100% coverage on preventive/diagnostic/xray, $0 deductible on preventive/diagnostic/xray, 80% coverage on all 4 general categories, 1 manually entered benefit for $45 deductible on code D0330 pano. First TP proc is D0330 for $95, second TP proc is D2150 on #30 for $200. In TP module, $45 deductible shows on the D0330, and a $5 deductible shows on the D2150.")]
		...
		public void Procedures_ComputeEstimates_FamilyDeductiblesBehaveTheSameAsIndividualDeductibles() {
			...
			List<ClaimProcHist> loopList=new List<ClaimProcHist>();
			List<Procedure>	ProcList=Procedures.Refresh(pat.PatNum);
			List<Procedure> ProcListTP=Procedures.GetListTPandTPi(ProcList);//sorted by priority, then toothnum
			//Validate
			for(int i=0;i<ProcListTP.Count;i++){
		...
		public void Procedures_ComputeEstimates_CategoryDeductiblesDontExceedLimits() {
			...
			loopList=new List<ClaimProcHist>();
			listProceduresTPs=Procedures.GetListTPandTPi(ProcList);
			for(int i=0;i<listProceduresTPs.Count;i++){
				Procedures.ComputeEstimates(listProceduresTPs[i],pat.PatNum,ref claimProcs,false,planList,patPlans,benefitList,
					histList,loopList,false,pat.Age,subList);
				//then, add this information to loopList so that the next procedure is aware of it.
...
")]
		...
		public void Procedures_ComputeEstimates_OverrideUnderFamilyMaxEstimateNote() {
			...
			List<Procedure> listProceduresTPs2=Procedures.GetListTPandTPi(ProcList2);//sorted by priority, then toothnum
			//Validate again
			for(int i=0;i<listProceduresTPs.Count;i++) {
		...
		[TestMethod]
		[Documentation.Numbering(Documentation.EnumTestNum.Procedures_ComputeEstimates_MultiVisitGroupProcedureSkipsSyncingClaimDateWhenInProcess)]
		[Documentation.VersionAdded("24.2")]
		[Documentation.Description("Procedures grouped for multiple visits in prior year. Complete D2750 for $100, Tp'd D6242 for $50. Computing estimate for D6242 is calculated for current benefit year.")]
		public void Procedures_ComputeEstimates_MultiVisitGroupProcedureSkipsSyncingClaimDateWhenInProcess() {
			//Setup patient and insurance information
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			Carrier carrier=CarrierT.CreateCarrier(suffix);
			...
			BenefitT.CreateAnnualMax(insPlan.PlanNum,125);
			PatPlanT.CreatePatPlan(1,pat.PatNum,subNum);
			//Create TP procedures with their benefit coverage for 100%
			List<Procedure> listProcs=new List<Procedure>();
			Procedure procBillable=ProcedureT.CreateProcedure(pat,"D2750",ProcStat.C,"1",100,new DateTime(2018,5,1));//PFM
			listProcs.Add(procBillable);
			Procedure procDelivery=ProcedureT.CreateProcedure(pat,"D6242",ProcStat.TP,"1",50,DateTime.Today.AddYears(-1));//Pontic
			listProcs.Add(procDelivery);
			BenefitT.CreatePercentForProc(insPlan.PlanNum,procDelivery.CodeNum,100);
			ProcMultiVisits.CreateGroup(listProcs);
			//Lists for ComputeEstimates
			Family fam=Patients.GetFamily(pat.PatNum);
			List<InsSub> listSubs=InsSubs.RefreshForFam(fam);
			List<InsPlan> listPlans=InsPlans.RefreshForSubList(listSubs);
			List<PatPlan> listPatPlans=PatPlans.Refresh(pat.PatNum);
			List<Benefit> listBenefits=Benefits.Refresh(listPatPlans,listSubs);
			List<ClaimProc> listClaimProcs=ProcedureT.ComputeEstimates(pat,listPatPlans,listPlans,listSubs,listBenefits);
			Assert.IsTrue(ProcMultiVisits.IsProcInProcess(procBillable.ProcNum));//This proc is In Process because it is set complete, while at the same time not all procs in the group are complete.
			Assert.IsFalse(ProcMultiVisits.IsProcInProcess(procDelivery.ProcNum));//This proc is not In Process because it is not set to complete.
			ClaimProc claimProcDeliveryEst=listClaimProcs.FirstOrDefault(x => x.ProcNum==procDelivery.ProcNum && x.Status==ClaimProcStatus.Estimate);
			Assert.AreEqual(claimProcDeliveryEst.ProcDate.Year,DateTime.Today.Year);//The proc date of service should be this year since procDelivery is not in process for the multi visit group.
			Assert.AreEqual(50,claimProcDeliveryEst.InsEstTotal);
		}
