...
<table>
  ...
  <tr>
    ...
    <td>true</td>
		...
		///<summary>There is a special rule (Rate2) that allows for overtime for time worked before 6:00am. This employee works from 4am-5:30am.
		///When calculating overtime, there is 90 minutes of overtime for the time worked before 6:00am.</summary>
		[TestMethod]
		public void TimeCardRules_CalculateDailyOvertime_WhileEmployeeClockedInAndOutBeforeRate2Time() {
			DateTime startDate=DateTime.Parse("2001-01-01");
			Employee emp=EmployeeT.CreateEmployee("CalculateDailyOvertimeWhileEmployeeClockedInAndOutBeforeRate2Time");
			PayPeriod payP1=PayPeriodT.CreateTwoWeekPayPeriodIfNotExists(startDate);
			PayPeriods.RefreshCache();
			Prefs.UpdateInt(PrefName.TimeCardOvertimeFirstDayOfWeek,0);
			Prefs.UpdateBool(PrefName.TimeCardsMakesAdjustmentsForOverBreaks,true);
			TimeCardRuleT.CreateAMTimeRule(emp.EmployeeNum,TimeSpan.FromHours(6));
			TimeCardRules.RefreshCache();
			long clockEvent1=ClockEventT.InsertWorkPeriod(emp.EmployeeNum,startDate.AddHours(4),startDate.AddHours(5.5),0);
			CalculateDailyOvertime(emp,payP1.DateStart,payP1.DateStop);
			//Validate
			Assert.AreEqual(TimeSpan.FromMinutes(90),ClockEvents.GetOne(clockEvent1).Rate2Auto);
		}
