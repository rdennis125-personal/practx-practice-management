...
namespace UnitTests.AlertItems_Tests {
	...
	public class AlertItemsTests:TestBase {
		...
		public void SetupTest() {
			...
			AlertCategoryT.ClearAlertCategoryTable();
			//Creates and inserts an alert category and links for all alert types.
			AlertCategoryT.CreateAlertCategory();
		}
		...
		public void AlertItems_CreateAlertsForNewWebmail() {
			...
			//Get the first AlertCategory that includes the WebMailReceived AlertType. SetupTest() ensures that we have one.
			AlertCategoryLink alertCategoryLink=AlertCategoryLinks.GetWhere(x => x.AlertType==AlertType.WebMailReceived).First();
			AlertCategory alertCategory=AlertCategories.GetOne(alertCategoryLink.AlertCategoryNum);
			...
			for(int i=0;i<5;i++) { 
				...
				Userods.Update(user);
				//WebMailReceived Alerts are only made for users that have at least one AlertSub that includes that type.
				AlertSub alertSub=new AlertSub(user.UserNum,-1,alertCategory.AlertCategoryNum);
				listAlertSubNew.Add(alertSub);
			}
			AlertSubs.Sync(listAlertSubNew,new List<AlertSub>());
			...
			string alertCount=DataCore.GetScalar("SELECT COUNT(*) FROM alertitem WHERE UserNum IN ("+string.Join(",",listTestUsers.Select(x => POut.Long(x.UserNum)))
				+") AND Type="+POut.Int((int)AlertType.WebMailReceived));
			...
			AlertItems_CreateAlertsForWebmailMethodCall();
			alertCount=DataCore.GetScalar("SELECT Description FROM alertitem WHERE Type="+POut.Int((int)AlertType.WebMailReceived)+" AND UserNum="+selectedUser.UserNum);
			Assert.AreEqual("5",alertCount);
			//
			...
			DataCore.NonQ(command);
			AlertItems_CreateAlertsForWebmailMethodCall();
			alertCount=DataCore.GetScalar("SELECT COUNT(*) FROM alertitem WHERE Type="+POut.Int((int)AlertType.WebMailReceived)+" AND UserNum="+selectedUser.UserNum);
		...
		public void AlertItems_GetUniqueAlerts_Addclinic() {
			...
			List<AlertItem> listUniqueAlertsAll=AlertItems.GetAlertsItemsForUser(userAdmin.UserNum,0);
			List<AlertItem> listUniqueAlertsOne=AlertItems.GetAlertsItemsForUser(userNormal.UserNum,0);
			...
			listUniqueAlertsAll=AlertItems.GetAlertsItemsForUser(userAdmin.UserNum,listClinics.LastOrDefault().ClinicNum);
			Assert.AreEqual(1,listUniqueAlertsAll.Count());
			//Check new clinic for user who is not subscribed to all alerts. 
			listUniqueAlertsOne=AlertItems.GetAlertsItemsForUser(userNormal.UserNum,listClinics.LastOrDefault().ClinicNum);
			Assert.AreEqual(0,listUniqueAlertsOne.Count());
