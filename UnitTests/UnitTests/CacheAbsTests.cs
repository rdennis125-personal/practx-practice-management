...
namespace UnitTests.CacheAbs_Tests {
	...
	public class CacheAbsTests:TestBase {
		...
		public void CacheAbsTests_DictTest_GetWhereForKey() {
			...
			List<Provider> listJohnsonProviders=listProviders.FindAll(x => x.LName=="Johnson");//At least one provider will be hidden.
			//Get a deep copy of the providers that can be safely manipulated.
			List<Provider> listJohnsonProviderCopies=_dictCache.GetWhereForKey(x => listJohnsonProviders.Select(y => y.ProvNum).Contains(x));
		...
		public void CacheAbsTests_DictTest_GetWhereForKeyShort() {
			...
			List<Provider> listJohnsonProviders=listProviders.FindAll(x => x.LName=="Johnson");//At least one provider will be hidden.
			//Get a deep copy of the providers that can be safely manipulated.
			List<Provider> listJohnsonProviderCopies=_dictCache.GetWhereForKey(x => listJohnsonProviders.Select(y => y.ProvNum).Contains(x),true);
			//Make sure that the lists are NOT equal because there should have been at least one hidden provider (Emma Johnson).
			Assert.AreNotEqual(listJohnsonProviders.Count,listJohnsonProviderCopies.Count);
			Provider providerAidenCopy=listJohnsonProviderCopies.First(x => x.Abbr=="Cache7" && x.FName=="Aiden" && x.LName=="Johnson");
			//First, double check that the provider we are about to manipulate are identical before manipulation.
		...
		[TestMethod]
		public void CacheAbsTests_DictTest_ClearCache() {
			//Forcefully refresh the dict cache.
			RefreshDictCache();
			//Ensure the cache is filled
			Assert.IsFalse(_dictCache.DictIsNull());
			//Clear the cache
			_dictCache.ClearCache();
			//Ensure the cache is null
			Assert.IsTrue(_dictCache.DictIsNull()); 
		}
		...
		[TestMethod]
		public void CacheAbsTests_DictNoPkTest_ClearCache() {
			//Forcefully refresh the dictNoPk cache.
			RefreshDictNoPkCache();
			//Ensure cache is filled.
			Assert.IsFalse(_dictNoPkCache.DictIsNull()); 
			//Clear the Cache
			_dictNoPkCache.ClearCache();
			//Ensure cache is empty.
			Assert.IsTrue(_dictNoPkCache.DictIsNull()); 
		}
		...
		[TestMethod]
		public void CacheAbsTests_ListTest_ClearCache() {
			//Forcefully refresh the list cache.
			RefreshListCache();
			//Ensure that cache has been filled 
			Assert.IsFalse(_listCache.ListIsNull()); 
			//Now clear the Cache
			_listCache.ClearCache();
			//Ensure that the list is null
			Assert.IsTrue(_listCache.ListIsNull());
		}
