...
namespace UnitTests.RepeatCharges_Tests {
	...
	public class RepeatChargesTests:TestBase {
		...
		///<summary>Runs repeat charges, until it reaches 1 year and 1 month after the initial repeat charge date.</summary>
		[TestMethod]
		public void RepeatCharges_RunRepeatingCharges_NumProcsReturnedByDifferentFreqiencies() {
			Patient pat=CreatePatForRepeatCharge("RepeatCharge",9);
			//add 3 repeat charges with start 08/09/2023 and different frequencies
			// monthly frequency
			RepeatCharge rc=new RepeatCharge();
			rc.ChargeAmt=15;
			rc.PatNum=pat.PatNum;
			rc.ProcCode="QRTFEE";
			rc.IsEnabled=true;
			rc.Frequency=EnumRepeatChargeFrequency.Monthly;
			rc.DateStart=new DateTime(2023,08,09);
			rc.RepeatChargeNum=RepeatCharges.Insert(rc);
			// quarterly frequency
			rc=new RepeatCharge();
			rc.ChargeAmt=75;
			...
			rc.ProcCode="QRTFEE";
			rc.IsEnabled=true;
			rc.Frequency=EnumRepeatChargeFrequency.Quarterly;
			rc.DateStart=new DateTime(2023,08,09);
			rc.RepeatChargeNum=RepeatCharges.Insert(rc);
			// annual frequency
			rc=new RepeatCharge();
			...
			rc.ProcCode="QRTFEE";
			rc.IsEnabled=true;
			rc.Frequency=EnumRepeatChargeFrequency.Annually;
			rc.DateStart=new DateTime(2023,08,09);
			rc.RepeatChargeNum=RepeatCharges.Insert(rc);
			DateTime dateRun=new DateTime(2023,08,09);
			DateTime dateEndDate=new DateTime(2024,01,08);
			List<Procedure> listProcs=Procedures.Refresh(pat.PatNum);
			while (dateRun < dateEndDate) {
				RepeatCharges.RunRepeatingCharges(dateRun);
				listProcs=Procedures.Refresh(pat.PatNum);
				dateRun=dateRun.AddMonths(1);
			}
			Assert.AreEqual(1,listProcs.FindAll(x=>x.ProcFee==275).Count);
			Assert.AreEqual(2,listProcs.FindAll(x=>x.ProcFee==75).Count);
			Assert.AreEqual(5,listProcs.FindAll(x=>x.ProcFee==15).Count);
		}
		...
		///<summary>Adds a Z-code that is in a hidden category to a repeating charge.</summary>
		[TestMethod]
		public void RepeatCharges_AddZCodeFromHiddenCategory() {
			//Setup
			Patient patient=CreatePatForRepeatCharge("RepeatCharge",15);
			RepeatCharge repeatCharge=new RepeatCharge();
			repeatCharge.ChargeAmt=15;
			repeatCharge.PatNum=patient.PatNum;
			repeatCharge.ProcCode="Z001";
			repeatCharge.IsEnabled=true;
			repeatCharge.DateStart=new DateTime(2021,08,26);
			repeatCharge.RepeatChargeNum=RepeatCharges.Insert(repeatCharge);
			ProcedureCode procedureCodeOld=ProcedureCodes.GetProcCode("Z001");
			ProcedureCode procedureCode=ProcedureCodes.GetProcCode("Z001");
			procedureCode.ProcCat=Defs.GetDefByExactName(DefCat.ProcCodeCats,"Obsolete").DefNum;//Hide Z-code that is in repeatCharge.
			ProcedureCodes.Update(procedureCode);
			DateTime dateTimeNow=MiscData.GetNowDateTime();
			//Action
			Procedure proc=null;
			OrthoCaseProcedureLinker orthoCaseProcedureLinker=OrthoCaseProcedureLinker.CreateOneForPatient(repeatCharge.PatNum);
			try {
				proc=RepeatCharges.AddProcForRepeatCharge(repeatCharge,dateTimeNow,dateTimeNow,orthoCaseProcedureLinker);
			}
			catch(ODException ex) {//Should not happen with Z-codes.
				ex.DoNothing();
			}
			ProcedureCodes.Update(procedureCodeOld);//Reset the proc code
			//Assertion
			Assert.AreEqual(ProcedureCodes.GetCodeNum("Z001"),proc?.CodeNum);//proc takes repeatCharge.ProcCode, so it should be the same.
			
		}

		///<summary>Does not add a procedure that is associated with a hidden category to repeating charges.</summary>
		[TestMethod]
		public void RepeatCharges_ErrorAddDCodeFromHiddenCategory() {
			//Create a hidden procedure code category
			Def defHiddenCategory=DefT.CreateDefinition(DefCat.ProcCodeCats,"HiddenCategory",isHidden:true);
			//Create a new procedure code associated with the hidden category.
			ProcedureCode procedureCode=ProcedureCodeT.CreateProcCode("EADCFHC",procCat:defHiddenCategory.DefNum);
			//Try to create a repeat charge for the hidden category, which should fail.
			Patient patient=CreatePatForRepeatCharge("RepeatCharge",15);
			RepeatCharge repeatCharge=new RepeatCharge();
			repeatCharge.ChargeAmt=15;
			repeatCharge.PatNum=patient.PatNum;
			repeatCharge.ProcCode=procedureCode.ProcCode;
			repeatCharge.IsEnabled=true;
			repeatCharge.DateStart=new DateTime(2021,08,26);
			repeatCharge.RepeatChargeNum=RepeatCharges.Insert(repeatCharge);
			DateTime dateTimeNow=MiscData.GetNowDateTime();
			...
			//Trying to add a repeat charge associated with a hidden category will throw an exception so the procedure should be null.
			Assert.IsNull(proc);
