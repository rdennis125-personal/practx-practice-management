...
namespace UnitTests.ProcMultiVisit_Tests {
	...
	public class ProcMultiVisitTests:TestBase {
		...
		public void ProcMultiVisitTests_CrownGroupComplete_ClaimDates() {
			...
			Claim claimResult=new Claim();
			try{
				claimResult=AccountModules.CreateClaim(claim,"P",insInfo.ListPatPlans,insInfo.ListInsPlans,listClaimProcs,listProcs,
					insInfo.ListInsSubs,pat,null,new List<Procedure> { procBillable },"",insInfo.PriInsPlan,insInfo.PriInsSub,Relat.Self);
			}
			catch(Exception e){
				Assert.Fail(e.ToString());
			}
			Assert.AreEqual(claimResult.DateService,procDelivery.ProcDate);//Claim date of service must always be the greatest date in the multi visit group.
			listClaimProcs=ClaimProcs.RefreshForClaim(claimResult.ClaimNum);
		...
		public void ProcMultiVisit_UpdateGroupForProc_UpdatedClaimDates() {
			...
			Assert.AreEqual(claimResult.DateService,procBillable.ProcDate);//Claim date of service must always be the greatest date in the multi visit group.
			listClaimProcs=ClaimProcs.RefreshForClaim(claimResult.ClaimNum);
			...
			Assert.AreEqual(claimResult.ClaimStatus,"I");//MultiVisit Group is not complete so claim should be 'Hold for In process'.
			Procedure procDeliveryOld=procDelivery.Copy();
			procDelivery.ProcStatus=ProcStat.C;
			Procedures.Update(procDelivery,procDeliveryOld);
		...
		public void ProcMultiVisit_UpdateGroupForProc_UpdateClaimNotPreAuth() {
			...
			Assert.AreEqual(listClaims.FirstOrDefault(x => x.ClaimNum==claimResult.ClaimNum).ClaimStatus,"W");//MultiVisit Group completed so claim should be		'Waiting	to  Send'.
			listClaimProcs=ClaimProcs.RefreshForClaim(claimResult.ClaimNum);
