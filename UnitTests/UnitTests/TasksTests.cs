...
namespace UnitTests.Tasks_Tests {
	...
	public class TasksTests:TestBase {
		...
		[TestMethod]
		public void Tasks_Equals_TaskObject() {
			//Tasks.Equals() has been overridden so the object doesn't technically need to be the same object in memory to equate.
			Task taskOrig=new Task() {
				TaskNum=15,
				Descript="TaskDescript",
				PriorityDefNum=643,
			};
			Task taskCopy=taskOrig.Copy();//Deep copy of the Task object
			Assert.IsTrue(taskOrig.Equals(taskCopy));
		}

		[TestMethod]
		public void Tasks_Equals_NonTaskObject() {
			DtoObject dtoObject=new DtoObject();
			Task task=new Task();
			Assert.IsFalse(task.Equals(dtoObject));
		}

		[TestMethod]
		public void Tasks_Equals_Null() {
			//Make sure that the overridden .Equals method can handle nulls.
			Task taskNull=null;
			Task task=new Task();
			Assert.IsFalse(task.Equals(taskNull));
		}
		#endregion
