...
namespace UnitTests.PayPlanEdit_Tests {
	...
	public class PayPlanEditTests:TestBase {
		...
		public void PayPlanEdit_GetListExpectedCharges_TotalInterestIsCorrectWhenDelayingInterest() {
			...
			if(startDate.Day<startDate.AddMonths(1).Day){
				startDate=startDate.AddDays(startDate.AddMonths(1).Day-startDate.Day);
			}
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,pat.PatNum,startDate,5,12,25,listProcs,listAdjs
				,PayPlanFrequency.Monthly,dateInterestStart:startDate.AddMonths(1));
		...
		[TestMethod]
		public void PayPlanEdit_GetListExpectedCharges_ChargesAreGeneratedCorrectlyWithFutureCharges() {
			/*--------------------------------------------------------------------------
			 * Create a dynamic payment plan with $190 of production attached.
			 * Monthly payments
			 * No down payment
			 * First charge due today
			 * Issue a charge 2 periods from now, and alter its value to $4 instead of $22
			 * assert that we dont estimate a charge for the one thats been issued, and that our estimated total has changed by $4.
			 *--------------------------------------------------------------------------*/
			//set up dynamic pay plan prefs
			PrefT.UpdateDateT(PrefName.DynamicPayPlanLastDateTime,DateTime.MinValue);
			PrefT.UpdateDateT(PrefName.DynamicPayPlanRunTime,DateTime.Now);
			...
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D0220",ProcStat.C,"",165,DateTime.Today));
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D0221",ProcStat.C,"",25,DateTime.Today));
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,pat.PatNum,DateTime.Today.AddMonths(-1),0,0,22,listProcs,listAdjs,frequency:PayPlanFrequency.Monthly);
			//run logic to generate charges (look at recurring charges tests for reference)
			DynamicPaymentPlanModuleData moduleData=PayPlanEdit.GetDynamicPaymentPlanModuleData(dynamicPayPlan);
			List<PayPlanCharge> listExpectedCharges=PayPlanEdit.GetListExpectedCharges(
				moduleData.ListPayPlanChargesDb,
				PayPlanEdit.GetPayPlanTerms(moduleData.PayPlan,moduleData.ListPayPlanLinks),
				moduleData.Family,
				moduleData.ListPayPlanLinks,
				moduleData.PayPlan,
				false,
				listPaySplits:moduleData.ListPaySplits);
			double totalExpectedBeforeAlteration=listExpectedCharges.Sum(x=>x.Principal);
			//Assert the charges expected
			//Issue and alter a future charge
			PayPlanCharge futureChargeAltered=listExpectedCharges[2];
			double deductionAmount=18;
			futureChargeAltered.Principal=futureChargeAltered.Principal-deductionAmount;
			PayPlanCharges.Insert(futureChargeAltered);
			moduleData=PayPlanEdit.GetDynamicPaymentPlanModuleData(dynamicPayPlan);
			listExpectedCharges=PayPlanEdit.GetListExpectedCharges(
				moduleData.ListPayPlanChargesDb,
				PayPlanEdit.GetPayPlanTerms(moduleData.PayPlan,moduleData.ListPayPlanLinks),
				moduleData.Family,
				moduleData.ListPayPlanLinks,
				...
				false,
				listPaySplits:moduleData.ListPaySplits);
			//Assert that the alterations are considered.
			//Assert that we dont issue a charge for the future charge we issued already.
			Assert.IsTrue(!listExpectedCharges.Any(x=>x.ChargeDate==futureChargeAltered.ChargeDate)); 
			//ensure that our total plan value hadn't changed.
			double totalExpectedAfterAlteration=listExpectedCharges.Sum(x=>x.Principal);
			//Since we only charged out $4 instead of $22 the estimate sum should only change by $4.
			Assert.IsTrue(CompareDouble.IsEqual(totalExpectedAfterAlteration+4,totalExpectedBeforeAlteration));
		}
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_PreviousPrincipalPaid() {
			...
			PayPlanEdit.ApplyPrepaymentToDynamicPaymentPlan(pat.PatNum,hiddenUnearnedTotal,listPayPlanChargesForPlan,listSplitsDB);
		...
		public void PayPlanEdit_ApplyPrepaymentToDynamicPaymentPlan_ExtraHiddenUnearned_CloseOut_MoveToRegularUnearned() {
			...
			PayPlanEdit.ApplyPrepaymentToDynamicPaymentPlan(pat.PatNum,paymentDynamicPrePay.PayAmt,listPayPlanCharges,new List<PaySplit>());
			...
			//Assert that the dynamic prepayment amounts were transferd to regular unearned.
		...
		[TestMethod]
		public void PayPlanEdit_IssueChargesDueForDynamicPaymentPlans_PrepaymentsAppliedFutureDatedChargeDateArrives() {
			/*--------------------------------------------------------------------------
			 * Dynamic Payment Plan (Await Complete) with three procedures, $50, $50, and $100. %12 apr.
			 * Issue one charge for $38.00 principal, $1.00 interest.
			 * Make a payment for $40, auto splits to $38 principal and $1 interest.
			 * Increase the interest split to $40, thus $39 overpayment.
			 * Rebalance on prepayment. Should move $39 to hidden unearned.
			 * Assert we've done a transfer of $39 into the unearned prepayment category.
			 * Issue a future dated charge.
			 * Assert that nothing gets allocated to it.
			 * Change the date time to the date of the future dated charge.
			 * Assert that the hidden unearned prepayment gets applied to the future dated charge.
			 *--------------------------------------------------------------------------*/
			//Test variables
			PayPlanEdit.PayPlanRecalculationData recalcData=new PayPlanEdit.PayPlanRecalculationData();
			long dynamicPaymentPlanPrePaymentDef=PrefC.GetLong(PrefName.DynamicPayPlanPrepaymentUnearnedType);
			...
			List<Procedure> listProcs=new List<Procedure>();
			List<Adjustment> listAdjs=new List<Adjustment>();
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D0200",ProcStat.C,"",50,DateTime.Today));
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D0210",ProcStat.C,"",50,DateTime.Today));
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D0220",ProcStat.C,"",100,DateTime.Today));
			...
			//Issue a future dated charge.
			DynamicPaymentPlanModuleData moduleData=PayPlanEdit.GetDynamicPaymentPlanModuleData(dynamicPayPlanAwait);
			List<PayPlanCharge> listExpectedCharges=PayPlanEdit.GetListExpectedCharges(
				moduleData.ListPayPlanChargesDb,
				PayPlanEdit.GetPayPlanTerms(dynamicPayPlanAwait,moduleData.ListPayPlanLinks),
				moduleData.Family,
				moduleData.ListPayPlanLinks,
				moduleData.PayPlan,
				false,
				false,
				moduleData.ListPaySplits);
			//Issue the next period of charges.
			List<PayPlanCharge> listChargesNextPeriod=listExpectedCharges.GroupBy(x=>x.ChargeDate).First().ToList();
			foreach(PayPlanCharge charge in listChargesNextPeriod){
			...
			double totalPaid=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==0 && x.PayPlanNum!=0).Sum(x=>x.SplitAmt);
			double totalLeftDue=recalcDataAfterFutureCharge.ListPayPlanCharges.Sum(x=>x.Interest+x.Principal)-totalPaid;
			double totalInUnearned=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType!=0 && x.UnearnedType!=dynamicPaymentPlanPrePaymentDef).Sum(x=>x.SplitAmt);
			double totalDPPUnearned=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.PayPlanNum!=0).Sum(x=>x.SplitAmt);
			double totalMovedToHiddenUnearned=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.SplitAmt>0).Sum(x=>x.SplitAmt);
			double totalMovedFromHiddenUnearned=-recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.SplitAmt<0).Sum(x=>x.SplitAmt);
			double totalChargedFutureRemaining=listChargesInDB.Sum(x=>x.Interest+x.Principal)-totalPaid;
			//Nothing should have applied yet since the charge isn't due yet
			Assert.IsTrue(CompareDouble.IsEqual(40,totalPaid)); //$40 becuase our allocations haven't change and this is what was originally paid
			Assert.IsTrue(CompareDouble.IsEqual(0,totalLeftDue)); //This should be 0 since we dont consider future charges, and one $40 charge has been issued
			Assert.IsTrue(CompareDouble.IsEqual(0,totalInUnearned)); //0 since there is not other hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(38,totalDPPUnearned)); //38 since there is still $38 in hidden unearned (we haven't transfered yet.)
			Assert.IsTrue(CompareDouble.IsEqual(38,totalMovedToHiddenUnearned)); //38 to show that our original overpayment moved into hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(0,totalMovedFromHiddenUnearned)); //0 to show that we haven't moved any money out of hidden unearned yet.
			Assert.IsTrue(CompareDouble.IsEqual(40,totalChargedFutureRemaining)); //Ensure we are expecting $40 due from the manually issued charges
			...
			totalLeftDue=recalcDataAfterFutureCharge.ListPayPlanCharges.Sum(x=>x.Interest+x.Principal)-totalPaid;
			totalInUnearned=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType!=0 && x.UnearnedType!=dynamicPaymentPlanPrePaymentDef).Sum(x=>x.SplitAmt);
			totalDPPUnearned=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.PayPlanNum!=0).Sum(x=>x.SplitAmt);
			totalMovedToHiddenUnearned=recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.SplitAmt>0).Sum(x=>x.SplitAmt);
			totalMovedFromHiddenUnearned=-recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.SplitAmt<0).Sum(x=>x.SplitAmt);
			totalChargedFutureRemaining=listChargesInDB.Sum(x=>x.Interest+x.Principal)-totalPaid;
			//The remainder ($38) of the hidden unearned should apply to the newly due charge ($40).
			Assert.IsTrue(CompareDouble.IsEqual(78,totalPaid)); //78 becuase our allocations have been changed by the service thread (IssueChargeDueForDynamicPaymentPlans)
			Assert.IsTrue(CompareDouble.IsEqual(2,totalLeftDue)); //2 because the 38 in hidden unearned didn't cover our due amount of 40
			Assert.IsTrue(CompareDouble.IsEqual(0,totalInUnearned)); //0 because there is no other hidden unearned value
			Assert.IsTrue(CompareDouble.IsEqual(0,totalDPPUnearned)); //0 since we've issued a transfer from hidden unearned to the charge
			Assert.IsTrue(CompareDouble.IsEqual(38,totalMovedToHiddenUnearned)); //38 to show that we'd moved 38 into hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(38,totalMovedFromHiddenUnearned)); //38 to show that we'd moved 38 out of hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(2,totalChargedFutureRemaining)); //Since we've allocated hidden unearned, and our future charges have become due, this should be the remainder of the value of the plan.
			DateTime_.ResetNow();
		}

		[TestMethod]
		public void PayPlanEdit_IssueChargesDueForDynamicPaymentPlans_PrepaymentsAppliedFutureDatedChargeRepetition() {
			/*--------------------------------------------------------------------------
			...
			Payment payment=PaymentT.MakePaymentNoSplits(pat.PatNum,40,payDate:DateTime.Today);
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payment);
			results.ListPaySplitsSuggested.First(x=>x.PayPlanDebitType==PayPlanDebitTypes.Interest).SplitAmt=51; //overpays by $48.00
			PaySplits.InsertMany(results.ListPaySplitsSuggested);
			...
			Assert.IsTrue(CompareDouble.IsEqual(0,totalLeftDue)); //This should be 0 since we dont consider future charges, and one $40 charge has been issued
			Assert.IsTrue(CompareDouble.IsEqual(0,totalInUnearned)); //0 since there is not other hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(49,totalDPPUnearned)); //49 since there is still $49 in hidden unearned (we haven't transfered yet.)
			Assert.IsTrue(CompareDouble.IsEqual(49,totalMovedToHiddenUnearned)); //49 to show that our original overpayment moved into hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(0,totalMovedFromHiddenUnearned)); //0 to show that we haven't moved any money out of hidden unearned yet.
			Assert.IsTrue(CompareDouble.IsEqual(40,totalChargedFutureRemaining)); //Ensure we are expecting $40 due from the manually issued charges
			...
			totalMovedFromHiddenUnearned=-recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.SplitAmt<0).Sum(x=>x.SplitAmt);
			totalChargedFutureRemaining=listChargesInDB.Sum(x=>x.Interest+x.Principal)-totalPaid;
			//A portion ($40) of the hidden unearned ($49) should apply to the newly due charge ($40).
			Assert.IsTrue(CompareDouble.IsEqual(80,totalPaid)); //80 becuase all of our 2 charges have been paid.
			Assert.IsTrue(CompareDouble.IsEqual(0,totalLeftDue)); //0 since our hidden unearned exceeded the value of the charge that just came due.
			Assert.IsTrue(CompareDouble.IsEqual(0,totalInUnearned)); //0 because there is no other hidden unearned value
			Assert.IsTrue(CompareDouble.IsEqual(9,totalDPPUnearned)); //9 since the cost of the newly issued charge didn't consume all of the hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(49,totalMovedToHiddenUnearned)); //49 to show that we'd moved 49 into hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(40,totalMovedFromHiddenUnearned)); //40 to show that we'd moved 40 out of hidden unearned
			Assert.IsTrue(CompareDouble.IsEqual(0,totalChargedFutureRemaining)); //Since we've allocated hidden unearned, and our future charges have become due, this should be the remainder of the value of the plan.
			...
			totalMovedFromHiddenUnearned=-recalcDataAfterFutureCharge.ListPaySplits.Where(x=>x.UnearnedType==dynamicPaymentPlanPrePaymentDef && x.SplitAmt<0).Sum(x=>x.SplitAmt);
			totalChargedFutureRemaining=listChargesInDB.Sum(x=>x.Interest+x.Principal)-totalPaid;
			//Nothing should have changed from the last time we ran this logic.
			Assert.IsTrue(CompareDouble.IsEqual(80,totalPaid)); //80 becuase all of our 2 charges have been paid.
			Assert.IsTrue(CompareDouble.IsEqual(0,totalLeftDue)); //0 since our hidden unearned exceeded the value of the charge that just came due.
			Assert.IsTrue(CompareDouble.IsEqual(0,totalInUnearned)); //0 because there is no other hidden unearned value
			Assert.IsTrue(CompareDouble.IsEqual(9,totalDPPUnearned)); //9 since the cost of the newly issued charge didn't consume all of the hidden unearned
		...
		[TestMethod]
		[Documentation.VersionAdded("24.2")]
		[Documentation.Numbering(Documentation.EnumTestNum.PayPlanEdit_CreateTransferForDynamicPaymentPlans_PlanOverPaymentStaysWithCurrentFamilyMember)]
		[Documentation.Description("Patient 2 is in a family with Patient 1 who is the Guarantor. Patient 2 creates a Dynamic Payment Plan with a completed procedure for $150 and a Down Payment of $10. Patient 2 then makes a payment of $30. Increase the Principle Split to be an overpayment of $20. Create the transfer for the Dynamic Payment Plan. This should only have Payment Splits for Patient 2 and not Patient 1.")]
		public void PayPlanEdit_CreateTransferForDynamicPaymentPlans_PlanOverPaymentStaysWithCurrentFamilyMember() {
			/*--------------------------------------------------------------------------
			 * Create a family of two with patient 2 being the one with the Dynamic Payment Plan.
			 * Dynamic Payment Plan (Treat as Complete) with one procedure at $150.
			 * Issue The first months charges as a Down Payment of $10.
			 * Make a payment for $30, let autosplit handle split generation.
			 * Increase the principal split to $50, thus $20 overpayment.
			 * Assert that Patient 1 (Guarantor) does not have any Payment Splits
			 *--------------------------------------------------------------------------*/

			//Set up Family with Patient 1 being the Guarantor
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat1=PatientT.CreatePatient(suffix+"-1");
			Patient pat2=PatientT.CreatePatient(suffix+"-2");
			pat2.Guarantor=pat1.PatNum;
			List<Patient> pats=new List<Patient> { pat1,pat2 };
			Family fam=new Family(pats);

			//Create the produciton that will be attached to the payment plan
			List<Procedure> listProcs=new List<Procedure>();
			List<Adjustment> listAdjs=new List<Adjustment>();
			listProcs.Add(ProcedureT.CreateProcedure(pat2,"D0200",ProcStat.C,"",150,DateTime.Today.AddMonths(1)));
			PayPlan dynamicPayPlanAwait=PayPlanT.CreateDynamicPaymentPlan(pat2.PatNum,pat2.Guarantor,DateTime.Today.AddMonths(-1),10,0,10,listProcs,listAdjs
				,PayPlanFrequency.Monthly,dynamicPayPlanTPOptions:DynamicPayPlanTPOptions.TreatAsComplete,runService:true);

			//Make a payment then manually increase the amount of the split to mimic a user manually editing the payment to an overpayment
			PaymentT.MakePayment(pat2.PatNum,30,payDate:DateTime.Today,payPlanNum:dynamicPayPlanAwait.PayPlanNum,procNum:listProcs[0].ProcNum);
			List<PaySplit> listPaySplitsFamilyMember=PaySplits.GetForPats(new List<long>{ pat2.PatNum });
			PaySplit paySplitFamilyMember=listPaySplitsFamilyMember.FirstOrDefault();
			paySplitFamilyMember.PayPlanDebitType=PayPlanDebitTypes.Principal;
			paySplitFamilyMember.SplitAmt=50;//Overpays by $20.00
			PaySplits.Update(paySplitFamilyMember);

			//Get recalculation data and rebalance
			PayPlanEdit.PayPlanRecalculationData recalcData=PayPlanT.GetRecalculationData(dynamicPayPlanAwait,pat2,fam);
			PayPlanEdit.CreateTransferForDynamicPaymentPlans(new List<PayPlanEdit.PayPlanRecalculationData> { recalcData },true);
			List<PaySplit> listPaySplitsGuarantor=PaySplits.GetForPats(new List<long>{ pat1.PatNum });
			listPaySplitsFamilyMember=PaySplits.GetForPats(new List<long>{ pat2.PatNum });
			Assert.AreEqual(0,listPaySplitsGuarantor.Count);
			Assert.AreEqual(3,listPaySplitsFamilyMember.Count);
		}
