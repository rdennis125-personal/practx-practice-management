	...
	public class PayPlansTests:TestBase {
		...
		public void PayPlansDynamic_GetEstimatedSchedule_AwaitCompleteOneTreatmentPlanned() {
			...
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,pat.PatNum,DateTime.Today.AddYears(-50),downPayment,12,25,listProcs,listAdjs
				,PayPlanFrequency.Monthly,dateInterestStart:DateTime.Today.AddYears(-49),runService:false);
		...
		///<summary>Asserts that the pay plan links for Treatment planned procedures, on a dynamic payment plan set to await complete, are ignored.</summary>
		[TestMethod]
		public void PayPlansDynamic_GetEstimatedSchedule_AwaitCompleteDoesntBackDateCharges() {
			/*--------------------------------------------------------------------------
			 * Dynamic Payment Plan (TP Await Complete) with three TP procedures, $139.1, $483, and $174.8. %0 apr.
			 * The first procedure ($139.10) is TP'd before the dynamic payment plan date of first payment, but gets completed after.
			 * Assert that this does not attempt to back date a charge.
			 *--------------------------------------------------------------------------*/
			//Test variables
			PayPlanEdit.PayPlanRecalculationData recalcData=new PayPlanEdit.PayPlanRecalculationData();
			long hiddenUnearnedDef=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			...
			List<Procedure> listProcs=new List<Procedure>();
			List<Adjustment> listAdjs=new List<Adjustment>();
			Procedure proc=ProcedureT.CreateProcedure(pat,"D0150",ProcStat.TP,"",139.1,DateTime.Today.AddDays(-10));
			listProcs.Add(proc);
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D0210",ProcStat.TP,"",483,DateTime.Today));
			listProcs.Add(ProcedureT.CreateProcedure(pat,"D1110",ProcStat.TP,"",174.8,DateTime.Today));
			//Note the passed in DateTime.Today also sets the date of first payment.
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,pat.PatNum,DateTime.Today.AddDays(-5),0,0,100,listProcs,listAdjs
				,PayPlanFrequency.Monthly,dynamicPayPlanTPOptions:DynamicPayPlanTPOptions.AwaitComplete,runService:true);
			...
			//Assert that expected charges are now producing charges for the one completed proc.
			listPayPlanCharges=PayPlanEdit.GetListExpectedCharges(recalcData.ListPayPlanCharges,recalcData.Terms,fam,recalcData.ListPayPlanLinks,dynamicPayPlan,false);
			//Get the period we currently fall into.
			int period=PayPlanEdit.CalcPeriodsForFrequency(dynamicPayPlan.DatePayPlanStart,dynamicPayPlan.ChargeFrequency);
			DateTime dateForPeriod=PayPlanEdit.CalcNextPeriodDate(dynamicPayPlan.DatePayPlanStart,period,dynamicPayPlan.ChargeFrequency);
			Assert.AreNotEqual(dynamicPayPlan.DatePayPlanStart,listPayPlanCharges.First().ChargeDate);
			Assert.AreEqual(dateForPeriod,listPayPlanCharges.First().ChargeDate);
		}

		///<summary>Asserts that the pay plan links for Treatment planned procedures, on a dynamic payment plan set to await complete, are ignored.</summary>
		[TestMethod]
		public void PayPlansDynamic_GetEstimatedSchedule_TreatAsCompleteDoesntBackDateCharges() {
			/*--------------------------------------------------------------------------
			 * Dynamic Payment Plan (TP Await Complete) with three TP procedures, $139.1, $483, and $174.8. %0 apr.
			 * The first procedure ($139.10) is TP'd before the dynamic payment plan date of first payment, but gets completed after.
			 * Assert that this does not attempt to back date a charge.
			...
			//attach the first proc
			DynamicPaymentPlanModuleData moduleData=PayPlanEdit.GetDynamicPaymentPlanModuleData(dynamicPayPlan.Copy());
			List<AccountEntry> listAccoutnEntryForPat=PayPlanEdit.GetAccountEntriesUnattachedToDynamicPaymentPlan(moduleData);
			moduleData=PayPlanEdit.AddProductionToDynamicPaymentPlan(listAccoutnEntryForPat.FindAll(x=>x.ProcNum==proc.ProcNum),moduleData);

			//Assert that expected charges are now producing charges for the one completed proc.
			List<PayPlanCharge> listPayPlanCharges=PayPlanEdit.GetListExpectedCharges(recalcData.ListPayPlanCharges,PayPlanEdit.GetPayPlanTerms(dynamicPayPlan,moduleData.ListPayPlanLinks),fam,moduleData.ListPayPlanLinks,dynamicPayPlan,false);
			//Get the period we currently fall into.
			int period=PayPlanEdit.CalcPeriodsForFrequency(dynamicPayPlan.DatePayPlanStart,dynamicPayPlan.ChargeFrequency);
			DateTime dateForPeriod=PayPlanEdit.CalcNextPeriodDate(dynamicPayPlan.DatePayPlanStart,period,dynamicPayPlan.ChargeFrequency);
			Assert.AreNotEqual(dynamicPayPlan.DatePayPlanStart,listPayPlanCharges.First().ChargeDate);
		...
		#region Dynamic Pay Plans Amount Override
		///<summary>Asserts procedures with adjustments added to them with amount override on a dynamic plan have adjustments ignored.</summary>
		[TestMethod]
		[Documentation.Numbering(Documentation.EnumTestNum.PayPlansDynamic_GetCompletedAmount_PayPlanLinksWithAmountOverride)]
		[Documentation.VersionAdded("24.3")]
		[Documentation.Description(@"Dynamic Payment Plan (TP Await Complete) with one procedure, one $78 D1110. Has an amount override of $78. 
Add a procedure adjustment for $30 on the same D1110. 
Dynamic payment plan final total should be $78.00.
		")]

		public void PayPlansDynamic_GetCompletedAmount_PayPlanLinksWithAmountOverride() {
			/*--------------------------------------------------------------------------
			 * Dynamic Payment Plan (TP Await Complete) with one procedure, one $78 D1110
			 * Has an amount override of $78
			 * Add a procedure adjustment for $30, 
			 * Assert dynamic payment plan final total is $78.00
			 *--------------------------------------------------------------------------*/
			//Test variables
			PayPlanEdit.PayPlanRecalculationData payplanRecalculationData=new PayPlanEdit.PayPlanRecalculationData();
			//set up payment plan
			Patient patient=PatientT.CreatePatient(MethodBase.GetCurrentMethod().Name);
			Family family=Patients.GetFamily(patient.PatNum);
			//create the production that will be attached to the payment plan with the await option selected, and treatment planned
			List<Procedure> listProcedures=new List<Procedure>();
			listProcedures.Add(ProcedureT.CreateProcedure(patient,"D1110",ProcStat.C,"",78.00,DateTime.Today));
			Adjustment adjustment=AdjustmentT.MakeAdjustment(patient.PatNum,-30.00,procNum:listProcedures[0].ProcNum);
			List<Adjustment> listAdjustments=new List<Adjustment>{adjustment};
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(patient.PatNum,patient.PatNum,DateTime.Today.AddMonths(-4),0,0,100,listProcedures,listAdjustments
				,PayPlanFrequency.Monthly,dynamicPayPlanTPOptions:DynamicPayPlanTPOptions.AwaitComplete,runService:true);
			payplanRecalculationData=PayPlanT.GetRecalculationData(dynamicPayPlan,patient,family);
			// Set the amount override on the first D1110
			payplanRecalculationData.ListPayPlanLinks[0].AmountOverride=78.00;
			List<PayPlanProductionEntry> payPlanProductionEntries=PayPlanProductionEntry.GetProductionForLinks(payplanRecalculationData.ListPayPlanLinks);
			double completedAmount=PayPlanProductionEntry.GetDynamicPayPlanCompletedAmount(dynamicPayPlan,payPlanProductionEntries);
			Assert.AreEqual(78.00,completedAmount);
			Assert.AreEqual(78,payPlanProductionEntries[0].AmountRemaining);
		}
		#endregion
	}
}
