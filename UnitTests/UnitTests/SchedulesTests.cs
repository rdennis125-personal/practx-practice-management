...
namespace UnitTests.Schedules_Tests {
	...
	public class SchedulesTests:TestBase {
		...
		///<summary>Makes sure that our provider collision detection logic finds collisions when start and stop times are the same.</summary>
		[TestMethod]
		public void Schedules_GetOverlappingSchedProvNums_SimpleSameStartStopTime() {
			ScheduleT.ClearScheduleTable();
			string suffix=MethodBase.GetCurrentMethod().Name;
			long provNumDoc=ProviderT.CreateProvider("DOC-"+suffix,"ProvSched","Collider");
			DateTime dateSched=DateTime.Now.AddDays(5);//Random date in the future.
			...
			ScheduleT.CreateSchedule(dateSched,new TimeSpan(new DateTime(1,1,1,9,0,0).Ticks)
				,new TimeSpan(new DateTime(1,1,1,17,0,0).Ticks),schedType: ScheduleType.Provider,provNum: provNumDoc);
			//Act like we are about to create a schedule that purposefully starts when the other stops (17:00 - 19:00).
			Schedule schedOverlap=new Schedule() {
				SchedDate=dateSched,
				StartTime=new TimeSpan(new DateTime(1,1,1,17,0,0).Ticks),
				StopTime=new TimeSpan(new DateTime(1,1,1,19,0,0).Ticks),
				SchedType=ScheduleType.Provider,
				ProvNum=provNumDoc,
				IsNew=true,
			};
			//Get all provider schedules from the database for the date in question (should only be our one schedule).
			List<Schedule> listSchedulesForDate=Schedules.GetDayList(dateSched).FindAll(x => x.SchedType==ScheduleType.Provider);
			//Assert that schedules that have the same start time as the stop time of the other schedule do not 'overlap'.
			List<long> listOverlappingSchedProvNums=Schedules.GetOverlappingSchedProvNums(
				new List<long>() { provNumDoc },
				schedOverlap,
				listSchedulesForDate,
		...
		///<summary>Makes sure that our provider collision detection logic finds collisions when start and stop times are the same.</summary>
		[TestMethod]
		public void Schedules_GetOverlappingSchedProvNums_ComplexSameStartStopTime() {
			ScheduleT.ClearScheduleTable();
			OperatoryT.ClearOperatoryTable();
			string suffix=MethodBase.GetCurrentMethod().Name;
			long provNumDoc=ProviderT.CreateProvider("DOC","DocCollider",suffix);
			...
			ScheduleT.CreateSchedule(dateSched,new TimeSpan(new DateTime(1,1,1,9,0,0).Ticks)
				,new TimeSpan(new DateTime(1,1,1,17,0,0).Ticks),schedType: ScheduleType.Provider,provNum: provNumDoc
				,listOpNums: new List<long>() { op.OperatoryNum });
			//Act like we are about to create a schedule for HYG that purposefully collides with the start time (17:00 - 19:00).
			Schedule schedOverlap=new Schedule() {
				SchedDate=dateSched,
				StartTime=new TimeSpan(new DateTime(1,1,1,17,0,0).Ticks),
				StopTime=new TimeSpan(new DateTime(1,1,1,19,0,0).Ticks),
		...
		///<summary>This test checks the correct schedules are returned when paring down lists of hygienists.  </summary>
		[TestMethod]
		public void Schedules_GetSchedulesHelper_CorrectSchedulesReturned_Hygienist() {
			//setup objects for test
			List<int> listSchedTypes=new List<int>() { (int)ScheduleType.Provider,(int)ScheduleType.Blockout };
			Clinic clinic1=ClinicT.CreateClinic("Clinic1");
			Clinic clinic2=ClinicT.CreateClinic("Clinic2");  //not included in listClinics
			...
			long provNum3=ProviderT.CreateProvider("Prov3");
			long provNum4=ProviderT.CreateProvider("Prov4");
			List<long> listProvNums=new List<long>(){provNum2};
			Operatory op1=OperatoryT.CreateOperatory("OP1",provDentist:provNum1,clinicNum:clinic1.ClinicNum);
			Operatory op2=OperatoryT.CreateOperatory("OP2",provHygienist:provNum2,clinicNum:clinic1.ClinicNum);
			Operatory op3=OperatoryT.CreateOperatory("OP3",provHygienist:provNum3,clinicNum:clinic1.ClinicNum);
			Operatory op4=OperatoryT.CreateOperatory("OP4",provHygienist:provNum4,clinicNum:clinic2.ClinicNum); //not included in listOpNums
			List<long> listOpNums=new List<long>(){op1.OperatoryNum,op2.OperatoryNum,op3.OperatoryNum};
			//create a one-day, two-hour schedule for each operatory
			Schedule sched1=ScheduleT.CreateSchedule(DateTime.Today.AddDays(1)//tomorrow from 8 am to 10 am
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,8,0,0).TimeOfDay
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,10,0,0).TimeOfDay
						,ScheduleType.Provider,clinicNum:clinic1.ClinicNum,provNum:provNum1,listOpNums:new List<long>() {op1.OperatoryNum });
			Schedule sched2=ScheduleT.CreateSchedule(DateTime.Today.AddDays(1)//tomorrow from 8 am to 10 am
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,8,0,0).TimeOfDay
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,10,0,0).TimeOfDay
						,ScheduleType.Provider,clinicNum:clinic1.ClinicNum,provNum:provNum2,listOpNums:new List<long>() {op2.OperatoryNum });
			Schedule sched3=ScheduleT.CreateSchedule(DateTime.Today.AddDays(1)//tomorrow from 8 am to 10 am
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,8,0,0).TimeOfDay
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,10,0,0).TimeOfDay
						,ScheduleType.Provider,clinicNum:clinic1.ClinicNum,provNum:provNum3,listOpNums:new List<long>()); //Schedule not assigned to operatory (dynamic)
			Schedule sched4=ScheduleT.CreateSchedule(DateTime.Today.AddDays(1)//tomorrow from 8 am to 10 am
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,8,0,0).TimeOfDay
						,new DateTime(DateTime.Now.Year,DateTime.Now.Month,DateTime.Now.AddDays(1).Day,10,0,0).TimeOfDay
						,ScheduleType.Provider,clinicNum:clinic2.ClinicNum,provNum:provNum4,listOpNums:new List<long>() {op4.OperatoryNum });
			Schedules.Update(sched1);
			...
			Schedules.Update(sched4);
			List<Schedule> returnedSchedules=Schedules.GetSchedulesHelper(DateTime.Today.AddDays(1),DateTime.Today.AddDays(2),listClinicNums,listOpNums,listProvNums,null,listSchedTypes);
			//Should return one schedule for prov2 in op2 at clinic1. This is sched2.
			Assert.AreEqual(1,returnedSchedules.Count());
			Assert.AreEqual(returnedSchedules[0].ScheduleNum,sched2.ScheduleNum);
		}
