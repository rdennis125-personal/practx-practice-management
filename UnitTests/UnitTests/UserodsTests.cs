...
namespace UnitTests.Userods_Tests {
	...
	public class UserodsTests:TestBase {
		...
		public void Userods_CopyUser_HappyPath() {
			...
			PasswordContainer loginDetailsNotExpected=user.GetPasswordContainer();
		...
		public void Userods_CopyUser_NameInUse() {
			...
			//Setup user
			//Fields given by method caller
			Userods.CopyUser(user,user.GetPasswordContainer(),false,isForCemt:false);//First copy
			long copiedUserNum2=Userods.CopyUser(user,user.GetPasswordContainer(),false,isForCemt:false).UserNum;//Second copy
