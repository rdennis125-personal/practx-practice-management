...
namespace UnitTests.PaymentEdit_Tests {
	...
	public class PaymentEditTests:TestBase {
		...
		public void SetupTest() {
			...
			PrefT.UpdateYN(PrefName.IncomeTransfersMadeUponClaimReceived,YN.Yes);
		...
		public void PaymentEdit_AllocateUnearned_SelectedAccountEntriesImplicitlyLinkLast() {
			...
			PaySplitT.CreatePrepayment(pat.PatNum,50,DateTime.Today,defNumUnearnedType:unearnedType);
		...
		public void PaymentEdit_AutoSplitForPayment_ImplicitlyLinkCredits_NeverPayTPProc() {
			...
			PaymentEdit.ConstructResults chargeResult=PaymentEdit.ConstructAndLinkChargeCredits(pat1.PatNum);
			List<AccountEntry> listTPProcs=chargeResult.ListAccountEntries.FindAll(//Create a list of all TP Procs in ListAccountCharges
				x => x.GetType()==typeof(Procedure) 
				&& ((Procedure)x.Tag).ProcStatus==ProcStat.TP
			);
			List<AccountEntry> listCProcs=chargeResult.ListAccountEntries.FindAll(//Create a list of all C Procs in ListAccountCharges
				x => x.GetType()==typeof(Procedure) 
		...
		public void PaymentEdit_AutoSplitForPayment_SplitForPaymentLessThanTotalofProcs() {
			...
			PaymentEdit.AutoSplit autoSplit=PaymentEdit.AutoSplitForPayment(pat.PatNum,pay);
			Assert.AreEqual(2,autoSplit.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplit.ListPaySplitsSuggested.Count(x => CompareDouble.IsEqual(x.SplitAmt,40)));
			Assert.AreEqual(1,autoSplit.ListPaySplitsSuggested.Count(x => CompareDouble.IsEqual(x.SplitAmt,10)));
			Assert.AreEqual(2,autoSplit.ListPaySplitsSuggested.Count(x => x.UnearnedType==0));
		}
		...
		public void PaymentEdit_AutoSplitForPayment_NoNegativeAutoSplits() {
			...
			PaymentEdit.AutoSplit autoSplits=PaymentEdit.AutoSplitForPayment(pat.PatNum,newPayment);
			Assert.AreEqual(0,autoSplits.ListPaySplitsSuggested.FindAll(x => x.SplitAmt<0).Count);//assert no negative auto splits were made.
			Assert.AreEqual(0,autoSplits.ListPaySplitsForPayment.FindAll(x => x.SplitAmt<0).Count);//auto splits not catching everything
			Assert.AreEqual(0,autoSplits.ListPaySplitsSuggested.Count(x => x.UnearnedType!=0));
		}
		...
		public void PaymentEdit_AutoSplitForPayment_NoExtraZeroSplitsForPayPlanCharges() {
			...
			PaymentEdit.AutoSplit autoSplits=PaymentEdit.AutoSplitForPayment(pat.PatNum,pay);
			//only two auto splits should exist. 1 covering the first whole payplancharge,and a second partial.
			Assert.AreEqual(2,autoSplits.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count(x => CompareDouble.IsEqual(x.SplitAmt,75)));
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count(x => CompareDouble.IsEqual(x.SplitAmt,25)));
			Assert.AreEqual(2,autoSplits.ListPaySplitsSuggested.Count(x => x.UnearnedType==0));
		}
		...
		public void PaymentEdit_AutoSplitForPayments_ProceduresAndUnattachedPayments() {
			...
			******************************************************/
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count(x => x.SplitAmt==100 && x.UnearnedType==0));
			Assert.AreEqual(4,autoSplits.ListAccountEntries.Count);
			Assert.AreEqual(1,autoSplits.ListAccountEntries.Count(x => x.AmountOriginal==55 && x.AmountEnd==0 && x.ProcNum==proc1.ProcNum));
			Assert.AreEqual(1,autoSplits.ListAccountEntries.Count(x => x.AmountOriginal==65 && x.AmountEnd==0 && x.ProcNum==proc2.ProcNum));
			Assert.AreEqual(1,autoSplits.ListAccountEntries.Count(x => x.AmountOriginal==75 && x.AmountEnd==0 && x.ProcNum==proc3.ProcNum));
			Assert.AreEqual(1,autoSplits.ListAccountEntries.Count(x => x.AmountOriginal==100 && x.AmountEnd==0 && x.ProcNum==newProc.ProcNum));
		}
		...
		public void PaymentEdit_AutoSplitForPayment_NoUnallocatedSplitsWhenPayPlanPaymentCoversMoreThanOneChargeInAPayment() {
			...
			PaymentEdit.AutoSplit autoSplits=PaymentEdit.AutoSplitForPayment(pat.PatNum,curPayment);
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count(x => CompareDouble.IsEqual(x.SplitAmt,19)));
			Assert.AreEqual(1,autoSplits.ListPaySplitsSuggested.Count(x => x.UnearnedType==0));//make sure it's not unallocated.
		}
		...
		public void PaymentEdit_AutoSplitForPayment_PaymentAutoSplitToAdjustment() {
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(results.ListPaySplitsSuggested[0].AdjNum,adjust1.AdjNum);
		}
		...
		public void PaymentEdit_AutoSplitForPayment_AutoSplitDoesntReuseAllocatedAdjustment() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payCur);
			Assert.AreEqual(2,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.FindAll(x => x.PatNum==proc.PatNum && x.ProvNum==proc.ProvNum && x.ClinicNum==proc.ClinicNum && x.ProcNum==proc.ProcNum && x.AdjNum==0 && x.UnearnedType==0 && x.SplitAmt==25).Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.FindAll(x => x.PatNum==pat.PatNum && x.ProvNum==0 && x.ClinicNum==pat.ClinicNum && x.ProcNum==0 && x.AdjNum==0 && x.UnearnedType>0 && x.SplitAmt==25).Count);
		}
		...
		public void PaymentEdit_AutoSplitForPayment_OverpaidProceduresUsedImplicitly() {
			...
			******************************************************/
			...
			AccountEntry:  Today  prov?    pat   $20
				^Represents proc2
			******************************************************/
			Assert.AreEqual(3,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				...
				&& x.ProcNum==proc1.ProcNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum
				&& x.AmountOriginal==-70
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==50
				&& x.ProcNum==0
				&& x.UnearnedType>0));
		}
		...
		public void PaymentEdit_AutoSplitForPayment_OverpaidProceduresNotUsedExplicitLinkingOnly() {
			...
			******************************************************/
			...
			Procedure proc2=ProcedureT.CreateProcedure(pat,"D0120",ProcStat.C,"",20);
			Payment payCur=PaymentT.MakePaymentNoSplits(pat.PatNum,50,DateTime.Today.AddDays(-3));
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payCur,isIncomeTxfr:true);
			/*****************************************************
			AccountEntry:  Today  provNum  pat   $50
				^Represents proc1
			AccountEntry:  Today  provNum  pat  -$70
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==20
				&& x.ProcNum==proc2.ProcNum
				&& x.UnearnedType==0));
			//One split for 30 to unearned for the income transfer system to deal with (the patient doesn't technically owe any money).
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==30
				&& x.ProcNum==0
				&& x.UnearnedType>0));
		}
		...
		public void PaymentEdit_AutoSplitForPayment_TransferredClaimsAreNotCountedTwice() {
			...
			Assert.AreEqual(1,autoSplitData.ListPaySplitsSuggested.FindAll(x => x.SplitAmt==35 && x.UnearnedType==0 && x.ProcNum==proc.ProcNum).Count);
		}
		...
		public void PaymentEdit_AutoSplitForPayment_TpProcsInPaymentWindowWhenDisplayCorrectlyAccordingToPref() {
			...
			PaymentEdit.AutoSplit autoSplitData=PaymentEdit.AutoSplitForPayment(pat.PatNum,currentPayment);
			Assert.AreEqual(1,autoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(0,autoSplitData.ListPaySplitsSuggested.FindAll(x => x.ProcNum!=0).Count);//auto split should be for reg. unallocated, not the tp.
			...
			autoSplitData=PaymentEdit.AutoSplitForPayment(pat.PatNum,currentPayment);
			Assert.AreEqual(0,autoSplitData.ListAccountEntries.Count);
		}
		...
		public void PaymentEdit_AutoSplitForPayment_CheckExplicitCreditsOnly() {
			...
			******************************************************/
			...
			PaymentEdit.AutoSplit autoSplit=PaymentEdit.AutoSplitForPayment(pat.PatNum,payment2);//Create autosplit for payment2
			PaySplits.InsertMany(autoSplit.ListPaySplitsSuggested);
			//Check explicit credits only box
			List<AccountEntry> listAccountCharges=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,doIncludeExplicitCreditsOnly:true).ListAccountEntries;
			/*****************************************************
			AccountEntry:  Today  prov?  pat
				^AmountOriginal = $100
				^AmountEnd      = $90  [$100 (proc) - $0 (no implicit) - $10 (auto split payment2)]
			******************************************************/
			Assert.AreEqual(100,listAccountCharges[0].AmountOriginal);
			Assert.AreEqual(90,listAccountCharges[0].AmountEnd);
			//Uncheck explicit credits only box
			listAccountCharges=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,doIncludeExplicitCreditsOnly:false).ListAccountEntries;
			/*****************************************************
			AccountEntry:  Today  prov?  pat
				^AmountOriginal = $100
			...
			******************************************************/
			Assert.AreEqual(100,listAccountCharges[0].AmountOriginal);
			Assert.AreEqual(40,listAccountCharges[0].AmountEnd);
		}
		...
		public void PaymentEdit_AutoSplitForPayment_HiddenPaysplitsAreNotUsedInImplicitLinking() {
			...
			******************************************************/
			Assert.AreEqual(1,autoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,autoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplitData.ListAccountEntries.FindAll(x => x.AmountOriginal==100 && x.AmountEnd==75).Count);
			Assert.AreEqual(1,autoSplitData.ListPaySplitsSuggested.FindAll(x => x.SplitAmt==25 && x.UnearnedType==0).Count);
		}
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanTest4() {
			...
			******************************************************/
			...
			List<PayPlanCharge> listPayPlanCharges=PayPlanEdit.CreatePayPlanAdjustments(-500,new List<PayPlanCharge>() { ppc1,ppd1 },0);
			//For whatever reason, the Payment Plan window will create an offsetting Credit for the adjustment charge.
			listPayPlanCharges.Add(PayPlanChargeT.CreateNegativeCreditForAdj(pat.PatNum,payPlan1.PayPlanNum,-500,provNum:provNum2));
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum1//Notice how this is the provider on the procedure and not the payment plan charge.
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==500
				&& x.ProvNum==provNum1
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==0
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==4500
				&& x.ProvNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanTest5() {
			...
			******************************************************/
			Assert.AreEqual(4,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum1
				&& x.AmountOriginal==1000
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& ((FauxAccountEntry)x).Principal==1000
				&& ((FauxAccountEntry)x).PrincipalAdjusted==500
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today
				&& x.ProcNum==proc1.ProcNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
			...
			Assert.AreEqual(4,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==500
				&& x.ProvNum==provNum1
				&& x.ProcNum==proc1.ProcNum
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==500
				...
				&& x.PayPlanNum==0
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==3500
				&& x.ProvNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
				&& x.UnearnedType==unearnedType));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanTest6() {
			...
			******************************************************/
			...
			PayPlanCharge ppd1=PayPlanChargeT.CreateOne(payPlan1.PayPlanNum,pat.Guarantor,pat.PatNum,DateTime.Today,1500,provNum:provNum2,
				chargeType:PayPlanChargeType.Debit,doInsert:false);
			List<PayPlanCharge> listPayPlanCharges=PayPlanEdit.CreatePayPlanAdjustments(-500,new List<PayPlanCharge>() { ppc1,ppc3,ppd1 },0);
			//For whatever reason, the Payment Plan window will create an offsetting Credit for the adjustment charge.
			listPayPlanCharges.Add(PayPlanChargeT.CreateNegativeCreditForAdj(pat.PatNum,payPlan1.PayPlanNum,-500,provNum:provNum2));
			PayPlanCharges.InsertMany(listPayPlanCharges);
			//Update the 'Tx Completed Amt' so that the Account module knows how much money the patient actually owes.
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum1//Note that this ISN'T the provider associated to the payment plan but is the provider on the procedure.
				&& ((FauxAccountEntry)x).Principal==1500
				&& ((FauxAccountEntry)x).PrincipalAdjusted==500
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today
				&& x.ProcNum==proc1.ProcNum));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanTest7() {
			...
			******************************************************/
			...
			Payment payCur=PaymentT.MakePaymentNoSplits(pat.PatNum,5000,DateTime.Today);
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payCur);
			/*****************************************************
			AccountEntry:      Today  provNum1  pat   $1000
				^Represents proc1
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum1//Note that this is the provider associated to the proc1 and not the payment plan.
				&& ((FauxAccountEntry)x).Principal==500
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today
				&& x.ProcNum==proc1.ProcNum));
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==5000
				&& x.ProvNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
				&& x.UnearnedType==unearnedType));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanTest8() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==500
				...
				&& x.PayPlanNum==0
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==4000
				&& x.ProvNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
				&& x.UnearnedType==unearnedType));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanTest9() {
			...
			******************************************************/
			...
			AccountEntry:      Today  provNum2  pat  -$500
				^Represents adj1
			******************************************************/
			Assert.AreEqual(6,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum1
				&& x.AmountOriginal==500
		...
		public void PaymentEdit_AutoSplitForPayment_SplitsAttachedToProcedureOrAdjustment() {
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payCur);
			Assert.AreEqual(3,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==100
				&& x.ProvNum==provNum1
				&& x.AdjNum==0
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==0
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==10
				...
				&& x.PayPlanNum==0
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==90
				&& x.ProvNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
				&& x.UnearnedType==unearnedType));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanWithAPR() {
			...
			******************************************************/
			Assert.AreEqual(13,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==255.94
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==13.07
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==payplan.PayPlanNum
				...
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==258.08
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==10.93
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==payplan.PayPlanNum
				...
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==260.23
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==8.78
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==payplan.PayPlanNum
				...
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==262.40
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==6.61
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==payplan.PayPlanNum
				...
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==264.58
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==4.43
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==payplan.PayPlanNum
				...
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==266.77
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==2.22
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==payplan.PayPlanNum
				...
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => CompareDouble.IsEqual(x.SplitAmt,385.96)
				&& x.ProvNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
				&& x.UnearnedType==unearnedType));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanProcCreditWithAdj() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==500
				...
				&& x.PayPlanChargeNum==ppd1.PayPlanChargeNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==1500
				&& x.ProvNum==0
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanProcCreditWithAdjDiffProv() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==500
				...
				&& x.PayPlanChargeNum==ppd1.PayPlanChargeNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==250
				&& x.ProvNum==provNum2
				&& x.AdjNum==adj1.AdjNum
				&& x.ProcNum==0
				&& x.PayPlanNum==0
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==1250
				&& x.ProvNum==0
				&& x.AdjNum==0
				&& x.ProcNum==0
				&& x.PayPlanNum==0
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanPreviousPaymentWithAdjDynamic() {
			...
			******************************************************/
			Assert.AreEqual(8,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.ProvNum==provNum1
			...
			Assert.AreEqual(2,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==140
				&& x.ProvNum==provNum1
				&& x.AdjNum==0
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==payPlan1.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==64.25
				&& x.ProvNum==provNum1
				&& x.AdjNum==0
				&& x.ProcNum==proc2.ProcNum
				&& x.PayPlanNum==payPlan1.PayPlanNum
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanProcedureWithAdjustmentDynamic() {
			...
			******************************************************/
			...
			Assert.AreEqual(2,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.PatNum==pat.PatNum
				&& x.SplitAmt==10
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==dynamicPayPlan.PayPlanNum
				&& x.UnearnedType==0));
		...
		public void PaymentEdit_AutoSplitForPayment_PayPlanWithPayNotAllowedForTP() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==250
				...
				&& x.PayPlanNum==payPlan1.PayPlanNum
				&& x.UnearnedType==0));
			Assert.AreEqual(2,results.ListPaySplitsSuggested.Count(x => x.SplitAmt==250
				&& x.ProvNum==provNum1
				&& x.ProcNum==0
				&& x.PayPlanNum==payPlan1.PayPlanNum
				&& x.UnearnedType==0));
		...
		public void PaymentEdit_AutoSplitForPayment_PreferThisPatient() {
			...
			******************************************************/
			...
			long provNum1=ProviderT.CreateProvider($"{suffix}-1");
			Patient pat1=PatientT.CreatePatient($"{suffix}-1");
			Patient pat2=PatientT.CreatePatient($"{suffix}-2",guarantor:pat1.PatNum);
			...
			Payment payment=PaymentT.MakePaymentNoSplits(pat2.PatNum,50,DateTime.Today);
			...
			PaymentEdit.AutoSplit autoSplitFIFO=PaymentEdit.AutoSplitForPayment(pat2.PatNum,payment,isPatPrefer:false);
			Assert.AreEqual(1,autoSplitFIFO.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplitFIFO.ListPaySplitsSuggested.Count(x => x.PatNum==pat1.PatNum
				&& x.ProvNum==provNum1
				&& x.ProcNum==proc1.ProcNum
				&& x.SplitAmt==50
				&& x.UnearnedType==0));
			...
			PaymentEdit.AutoSplit autoSplitPrefer=PaymentEdit.AutoSplitForPayment(pat2.PatNum,paymentPrefer,isPatPrefer:true);
			Assert.AreEqual(1,autoSplitPrefer.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,autoSplitPrefer.ListPaySplitsSuggested.Count(x => x.PatNum==pat2.PatNum
				&& x.ProvNum==provNum1
				&& x.ProcNum==proc2.ProcNum
				&& x.SplitAmt==50
				&& x.UnearnedType==0));
		...
		public void PaymentEdit_AutoSplitForPayment_InterestNotAttachedToProcs() {
			...
			Procedure proc1=ProcedureT.CreateProcedure(pat,"TDLP1",ProcStat.C,"",64,datePayPlan,provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"TDLP1",ProcStat.C,"",58,datePayPlan,provNum:provNum1);
			PayPlan payPlan1=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,pat.Guarantor,datePayPlan,0,10,35,
				new List<Procedure>(){ proc1,proc2 },new List<Adjustment>(),frequency:PayPlanFrequency.Monthly,provNum:provNum1);
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat.PatNum,payCur,isIncomeTxfr:true);
			Assert.AreEqual(11,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.PayPlanNum==0
				&& x.ProcNum==proc2.ProcNum
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& ((FauxAccountEntry)x).Interest==(decimal)1.02
				&& x.AmountEnd==0));
			Assert.AreEqual(10,results.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,results.ListPaySplitsSuggested.Count(x => x.PatNum==pat.PatNum
				&& x.SplitAmt==33.98
				&& x.PayPlanNum==payPlan1.PayPlanNum
				&& x.ProcNum==proc1.ProcNum
				&& x.UnearnedType==0));
			Assert.AreEqual(4,results.ListPaySplitsSuggested.Count(x => x.PatNum==pat.PatNum
				&& x.SplitAmt==1.02
				&& x.PayPlanNum==payPlan1.PayPlanNum
				&& x.ProcNum==0
				&& x.UnearnedType==0));
		...
		public void PaymentEdit_AutoSplitForPayment_DynamicPayPlanPayFirstEntries() {
			...
			******************************************************/
			...
			PaymentEdit.AutoSplit results=PaymentEdit.AutoSplitForPayment(pat1.PatNum,payment,listAccountEntriesPayFirst:listPayFirstAcctEntries);
			/*****************************************************
			...
			******************************************************/
			...
			PaySplit:      Today  provNum1  pat   $75
				^Represents ppd1 and proc3
			******************************************************/
			Assert.AreEqual(2,results.ListPaySplitsForPayment.Count);
			Assert.AreEqual(2,results.ListPaySplitsForPayment.Count(x => x.PatNum==pat1.PatNum
				&& x.ProvNum==provNum1
				&& x.SplitAmt==75
				&& x.PayPlanNum==payPlan1.PayPlanNum
				&& x.PayPlanChargeNum > 0));
		...
		public void PaymentEdit_Init_CorrectlyOrderedAutoSplits() {//Legacy_TestFortyOne
			...
			Assert.AreEqual(3,init.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[0].SplitAmt!=60 || init.AutoSplitData.ListPaySplitsSuggested[0].ProcNum!=procedure3.ProcNum);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[1].SplitAmt!=40 || init.AutoSplitData.ListPaySplitsSuggested[1].ProcNum!=procedure2.ProcNum);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[2].SplitAmt!=50 || init.AutoSplitData.ListPaySplitsSuggested[2].ProcNum!=procedure1.ProcNum);
		}
		...
		public void PaymentEdit_Init_CorrectlyOrderedAutoSplitsWithExistingPayment() {//Legacy_TestFortyTwo
			...
			Assert.AreEqual(3,init.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[0].SplitAmt!=30 || init.AutoSplitData.ListPaySplitsSuggested[0].ProcNum!=procedure2.ProcNum);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[1].SplitAmt!=40 || init.AutoSplitData.ListPaySplitsSuggested[1].ProcNum!=procedure1.ProcNum);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[2].SplitAmt!=10 || init.AutoSplitData.ListPaySplitsSuggested[2].ProcNum!=0);
		}
		...
		public void PaymentEdit_Init_AutoSplitOverAllocation() {//Legacy_TestFortyThree
			...
			Assert.AreEqual(1,init.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[0].SplitAmt!=50 || init.AutoSplitData.ListPaySplitsSuggested[0].ProcNum!=0);
		}
		...
		public void PaymentEdit_Init_AutoSplitForPaymentNegativePaymentAmount() {//Legacy_TestFortyFour
			...
			Assert.AreEqual(0,init.AutoSplitData.ListPaySplitsSuggested.Count);
		}
		...
		public void PaymentEdit_Init_AutoSplitWithAdjustmentAndExistingPayment() {//Legacy_TestFortyFive
			...
			******************************************************/
			Assert.AreEqual(4,init.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.Tag.GetType()==typeof(Procedure)
			...
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.Tag.GetType()==typeof(Procedure)
				...
				&& x.AmountOriginal==80
				&& x.AmountEnd==0));//This procedure would be -$20 had it not been for implicit linking.
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.Tag.GetType()==typeof(Adjustment)
			...
			Assert.AreEqual(2,init.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,init.AutoSplitData.ListPaySplitsSuggested.Count(x => x.ProcNum==procedure1.ProcNum
				&& x.ProvNum==provNum
				&& x.AdjNum==0
				&& x.SplitAmt==40));
			Assert.AreEqual(1,init.AutoSplitData.ListPaySplitsSuggested.Count(x => x.ProcNum==0
				&& x.ProvNum==0
				&& x.AdjNum==0
				&& x.SplitAmt==10
				&& x.UnearnedType==PrefC.GetLong(PrefName.PrepaymentUnearnedType)));
		...
		public void PaymentEdit_Init_AutoSplitProcedureGuarantor() {//Legacy_TestFortySeven
			...
			Assert.AreEqual(3,init.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[0].SplitAmt!=60 || init.AutoSplitData.ListPaySplitsSuggested[0].ProcNum!=procedure3.ProcNum 
				|| init.AutoSplitData.ListPaySplitsSuggested[0].PatNum!=patNum);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[1].SplitAmt!=40 || init.AutoSplitData.ListPaySplitsSuggested[1].ProcNum!=procedure2.ProcNum 
				|| init.AutoSplitData.ListPaySplitsSuggested[1].PatNum!=patNum2);
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[2].SplitAmt!=50 || init.AutoSplitData.ListPaySplitsSuggested[2].ProcNum!=procedure1.ProcNum 
				|| init.AutoSplitData.ListPaySplitsSuggested[2].PatNum!=patNum);
		}
		...
		public void PaymentEdit_Init_AutoSplitWithClaimPayments() {//Legacy_TestFortyEight
			...
			Assert.AreEqual(4,init.AutoSplitData.ListPaySplitsSuggested.Count);
			//First auto split not not 30, not not procedure 3, and not not patNum
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[0].SplitAmt!=30 
				|| init.AutoSplitData.ListPaySplitsSuggested[0].ProcNum!=procedure3.ProcNum
				|| init.AutoSplitData.ListPaySplitsSuggested[0].PatNum!=patNum);
			//Second auto split not not 35, not not procedure 2, and not not patNum
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[1].SplitAmt!=35 
				|| init.AutoSplitData.ListPaySplitsSuggested[1].ProcNum!=procedure2.ProcNum
				|| init.AutoSplitData.ListPaySplitsSuggested[1].PatNum!=patNum);
			//Third auto split not not 30, not not procedure 1, and not not patNum
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[2].SplitAmt!=30 
				|| init.AutoSplitData.ListPaySplitsSuggested[2].ProcNum!=procedure1.ProcNum
				|| init.AutoSplitData.ListPaySplitsSuggested[2].PatNum!=patNum);
			//Fourth auto split not not 55, and not not procNum of 0
			Assert.IsFalse(init.AutoSplitData.ListPaySplitsSuggested[3].SplitAmt!=55
				|| init.AutoSplitData.ListPaySplitsSuggested[3].ProcNum!=0);
		}
		...
		public void PaymentEdit_Init_FIFOWithPosAdjustment() {
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.Tag.GetType()==typeof(Procedure) && x.AmountEnd==55));
		}
		...
		public void PaymentEdit_Init_PayPlansWithAttachedCreditsAndUnattachedProcedureOnPaySplits() {
			...
			******************************************************/
			Assert.AreEqual(3,initData.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==50
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				...
				&& x.ProcNum==proc2.ProcNum
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count(x => x.PatNum==pat.PatNum
				&& x.ProcNum==proc2.ProcNum
		...
		public void PaymentEdit_Init_AttachedAdjustment() {
			...
			******************************************************/
			Assert.AreEqual(2,initData.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				...
				&& x.ProcNum==proc.ProcNum));
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==20
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count(x => x.SplitAmt==20
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
		...
		public void PaymentEdit_Init_UnattachedPaymentsAndAttachedAdjustments() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				...
				&& x.ProcNum==proc.ProcNum));
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count(x => x.SplitAmt==100
				&& x.AdjNum==0
				&& x.ProcNum==proc.ProcNum
				&& x.PatNum==proc.PatNum
				&& x.ProvNum==proc.ProvNum
		...
		public void PaymentEdit_Init_IncomeTransferableWhenNotExplicitlyLinkedProcs() {
			...
			******************************************************/
			...
			Assert.AreEqual(3,initData.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.FindAll(x => x.Tag.GetType()==typeof(Procedure)
				&& x.ProvNum==provNum1
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.FindAll(x => x.Tag.GetType()==typeof(PaySplit)
				&& x.ProvNum==provNum1
				&& x.ProcNum==0
		...
		public void PaymentEdit_Init_IncomeTransferableWhenNotExplicitlyLinkedAdjs() {
			...
			******************************************************/
			//Assert that the appropriate amounts are still considered transferable because they are not explicitly linked.
			Assert.AreEqual(3,initData.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.FindAll(x => x.Tag.GetType()==typeof(Adjustment)
				&& x.ProvNum==provNum1
		...
		public void PaymentEdit_Init_ProcedureWithBaseUnits() {
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.FindAll(x => x.AmountOriginal==200 && x.AmountEnd==0).Count);
		}
		...
		public void PaymentEdit_Init_ShowCorrectAmountNeedingPaymentOnChargeWhenPaymentPlanAdjustmentsExistForThePayPlanCharge() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListPaySplitsSuggested.Count(x => x.DatePay==DateTime.Today
		...
		public void PaymentEdit_Init_ChargesWithAttachedPayPlanCreditsWithPreviousPaymentsImplicit() {
			...
			******************************************************/
			...
			AccountEntry:  Today+2M  prov0  pat   $15
				^Faux entry designed for proc2 / payplan combo
			******************************************************/
			Assert.AreEqual(10,initData.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.PatNum==pat.PatNum
				&& ((FauxAccountEntry)x).Principal==30
				&& ((FauxAccountEntry)x).PrincipalAdjusted==15
				&& x.AmountEnd==0
		...
		public void PaymentEdit_Init_ShowCorrectAmountNeedingPaymentWhenPaymentPlanV2IsClosedAndPartiallyPaid() {
			...
			******************************************************/
			...
			AccountEntry:  Today     provNum  pat   $10
				^Faux entry designed for proc1 / payplan combo
			******************************************************/
			Assert.AreEqual(5,initData.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.Date==DateTime.Today.AddMonths(-4)
				&& x.AmountOriginal==100
			...
			Assert.AreEqual(1,initData.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.ProcNum==proc.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum));
			Assert.AreEqual(2,initData.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(2,initData.AutoSplitData.ListPaySplitsSuggested.Count(x => x.PatNum==proc.PatNum
				&& x.ProvNum==proc.ProvNum
				&& x.ProcNum==proc.ProcNum
				&& x.SplitAmt==30
				&& x.PayPlanNum==payplan.PayPlanNum
		...
		public void PaymentEdit_Init_PayPlanCreateAttachedSplitsForMultipleAttachedCredits() {
			...
			******************************************************/
			Assert.AreEqual(4,init.AutoSplitData.ListAccountEntries.Count);
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==100
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today.AddMonths(-3)
			...
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				...
				&& x.ProcNum==proc2.ProcNum
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==100
				&& ((FauxAccountEntry)x).Principal==100
				&& x.AmountEnd==0
			...
			Assert.AreEqual(1,init.AutoSplitData.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.PayPlanNum==payplan.PayPlanNum));
			Assert.AreEqual(1,init.AutoSplitData.ListPaySplitsSuggested.Count);
			Assert.AreEqual(1,init.AutoSplitData.ListPaySplitsSuggested.Count(x => x.PatNum==pat.PatNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.ProvNum==proc2.ProvNum
				&& x.SplitAmt==100
				&& x.UnearnedType==0));
		...
		public void PaymentEdit_MakePayment_Adjustment() {
			...
			listListAE.Add(constructResults.ListAccountEntries);
			PaymentEdit.PayResults results=PaymentEdit.MakePayment(listListAE,payment,100,constructResults.ListAccountEntries);
		...
		public void PaymentEdit_ExplicityLinkCredits_DiscountPlanAmountsForTreatmentPlannedProcedures() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.AmountOriginal==startingProcFee 
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_InsPayPlan() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults chargeResult=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum);
			/*****************************************************
			AccountEntry:  Today  provNum  pat   $0
				^Represents proc.
			******************************************************/
			Assert.AreEqual(1,chargeResult.ListAccountEntries.Count);
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_InsPayPlanTest1() {
			...
			******************************************************/
			...
			AccountEntry:  Today  provNum  pat   $200
				^Represents procB.
			******************************************************/
			Assert.AreEqual(2,chargeResult.ListAccountEntries.Count);
			Assert.AreEqual(1,chargeResult.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==500
				&& x.AmountEnd==100
		...
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_ConstructAndLinkChargeCredits_DynamicPayPlanInterestCharge)]
...
<table>
	...
	<tr>
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_DynamicPayPlanInterestCharge() {
			...
			PaySplit paySplitPrincipal=results.ListPaySplitsSuggested.First(x => x.PayPlanDebitType==PayPlanDebitTypes.Principal);
			...
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,doIncludeTreatmentPlanned:true);
			Assert.AreEqual(3,constructResults.ListAccountEntries.Count);
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanChargesGuarantorOutOfFamily() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(pat1.PatNum);
			Assert.AreEqual(2,constructResults.ListAccountEntries.Count);
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.ProvNum==provNum1
				&& x.PatNum==pat1.PatNum
				&& x.AmountOriginal==100
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today.AddMonths(-4)
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanChargesGuarantorOutOfFamilyMultiProcs() {
			...
			******************************************************/
			...
			PayPlan payplan=PayPlanT.CreatePayPlanWithCredits(pat1.PatNum,150,datePayPlanStart,provNum:provNum1,new List<Procedure>() { proc1,proc2 },
				guarantorNum:pat2.PatNum);
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(pat1.PatNum);
			Assert.AreEqual(4,constructResults.ListAccountEntries.Count);
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.ProvNum==provNum1
				&& x.PatNum==pat1.PatNum
				&& x.AmountOriginal==100
				&& x.AmountEnd==0
				...
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.ProvNum==provNum2
				&& x.PatNum==pat1.PatNum
				&& x.AmountOriginal==50
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today.AddMonths(-4)
			...
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.PatNum==pat1.PatNum
				&& ((FauxAccountEntry)x).Guarantor==pat2.PatNum
				&& ((FauxAccountEntry)x).Principal==150
				&& ((FauxAccountEntry)x).PrincipalAdjusted==100
				&& x.AmountEnd==100
				&& x.Date==datePayPlanStart
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum));
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_NoClaimProcsInIncomeTransfer() {
			...
			Assert.AreEqual(0,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(ClaimProc)).Count);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanChargesWithAttachedCredits() {
			...
			******************************************************/
			...
			AccountEntry:  Today+3M  prov?  pat   $15
				^Faux entry debit for proc2 / payplan combo
			******************************************************/
			Assert.AreEqual(10,constructResults.ListAccountEntries.Count);
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.ProvNum==pat.PriProv
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==135
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today.AddMonths(-4)
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanWithAdjustmentsOnBoth() {
			...
			******************************************************/
			...
			AccountEntry:  Today  provNum  pat  -$20
				^Faux entry representing an unattached adjustment debit.
			******************************************************/
			Assert.AreEqual(6,chargeResult.ListAccountEntries.Count);
			Assert.AreEqual(1,chargeResult.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==165
		...
		[TestMethod]
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanWithOffsettingAdjustmentsDynamic() {
			//Adjustments associated with a dynamic payment plan should not be offset by unattached negative adjustments.
			/*****************************************************
				Create Patient:  pat
				Create Provider: provNum
				Create proc:     Today  provNum  pat   $50
				Create adj1:     Today  provNum  pat   $70
				Create adj2:     Today  provNum  pat  -$20
				Create payplan:  Today  provNum  pat   $120
					^Associated to proc and adj1
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			long defNumUnearnedType=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			Patient patient=PatientT.CreatePatient(suffix);
			long provNum=ProviderT.CreateProvider(suffix);
			ProcedureCode procedureCode=ProcedureCodeT.CreateProcCode("PPWOAD");
			Procedure procedure=ProcedureT.CreateProcedure(patient,procedureCode.ProcCode,ProcStat.C,"",50,DateTime.Today,provNum:provNum);
			Adjustment adjustmentPositive=AdjustmentT.MakeAdjustment(patient.PatNum,70,provNum:provNum);
			Adjustment adjustmentNegative=AdjustmentT.MakeAdjustment(patient.PatNum,-20,provNum:provNum);
			//Create a dynamic payment plan associated with the procedure and positive adjustment ($50 + $70 = $120).
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(patient.PatNum,patient.PatNum,DateTime.Today,0,0,120,
				new List<Procedure>(){ procedure },new List<Adjustment>() { adjustmentPositive },provNum:provNum);
			//Skip implicit linking logic and skip offsetting adjustment logic (regardless of the preference value) so that the negative unattached adjustment is not considered.
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(patient.PatNum,isIncomeTxfr:true,hasOffsettingAdjustmets:false);
			/*****************************************************
			AccountEntry:  $0
				^Represents the procedure
			AccountEntry:  $0
				^Represents the positive adjustment.
			AccountEntry: -$20
				^Represents the unattached negative adjustment.
			FauxAccountEntry:  $50
				^PayPlanCharge debit for procedure.
			FauxAccountEntry:  $70
				^PayPlanCharge debit for positive adjustment
			******************************************************/
			Assert.AreEqual(5,constructResults.ListAccountEntries.Count);
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				...
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				&& x.PatNum==patient.PatNum
				&& x.AmountOriginal==70
				&& x.AmountEnd==0//Adjustment is attached to a payment plan so it has no value.
				&& x.Date==DateTime.Today
				&& x.AdjNum==adjustmentPositive.AdjNum
				&& x.ProcNum==0
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				&& x.PatNum==patient.PatNum
				&& x.AmountOriginal==-20
				&& x.AmountEnd==-20//Negative adjustment value was not offset by positive adjustment due to how ConstructAndLinkChargeCredits was invoked.
				&& x.Date==DateTime.Today
				&& x.AdjNum==adjustmentNegative.AdjNum
				&& x.ProcNum==0
				&& x.PayPlanNum==0));
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==patient.PatNum
				&& ((FauxAccountEntry)x).Principal==50//Procedure
				&& x.AmountEnd==50
				&& x.Date.Date==DateTime.Today
				&& x.ProcNum==procedure.ProcNum
				&& x.PayPlanNum==dynamicPayPlan.PayPlanNum));
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==patient.PatNum
				&& ((FauxAccountEntry)x).Principal==70//Positive adjustment debit
				&& x.AmountEnd==70
				&& x.Date.Date==DateTime.Today
				&& x.AdjNum==adjustmentPositive.AdjNum
				&& x.PayPlanNum==dynamicPayPlan.PayPlanNum));
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanProcedureWithAdjustmentWithInsOverpay() {
			/*****************************************************
				...
				Create payPlan1:            Today-4M  provNum1  pat1   $100.00
					^payPlanChargeProc:       Today-4M  provNum1  pat1   $100.00 (credit - procedure)
						^Attached to procedure1
					^payPlanCharge1:          Today-4M  provNum1  pat1   $33.34  (debit - charge 1)
					^payPlanCharge2:          Today-3M  provNum1  pat1   $33.34  (debit - charge 2)
					^payPlanCharge3:          Today-2M  provNum1  pat1   $33.32  (debit - charge 3)
					^payPlanChargeCloseOut:   Today-1M  provNum1  pat1   $0      (debit - close out charge)
					^payPlanCharge6NegAdj:    Today-1M  provNum1  pat1  -$33.32  (debit - neg adjustment)
					^payPlanCharge7NegCred:   Today-1M  provNum1  pat1  -$33.32  (credit - neg adjustment)
						^Attached to procedure1
			******************************************************/
			long unearnedType=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			List<Def> listNegAdjTypes=Defs.GetDefsForCategory(DefCat.AdjTypes,true).FindAll(x => x.ItemValue=="-");
			...
			Patient patient2=PatientT.CreatePatient($"{suffix}-2",guarantor:patient1.PatNum);
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.Date.Date==DateTime.Today.AddMonths(-4)
				&& x.AmountEnd==33.34M
				&& x.ProcNum==procedure1.ProcNum
				&& x.PayPlanNum==payPlan1.PayPlanNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& ((FauxAccountEntry)x).ChargeType==PayPlanChargeType.Debit
				&& ((FauxAccountEntry)x).Principal==33.34M
				&& x.Date.Date==DateTime.Today.AddMonths(-3)
				&& x.AmountEnd==33.34M
				&& x.ProcNum==procedure1.ProcNum
				&& x.PayPlanNum==payPlan1.PayPlanNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.AmountEnd==0M //This debit was offset by the negative debit.
				&& x.ProcNum==procedure1.ProcNum
				&& x.PayPlanNum==payPlan1.PayPlanNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==patient1.PatNum
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanProcedureWithAdjustmentDynamic() {
			...
			******************************************************/
			...
			AccountEntry:  Today  provNum  pat   $100
				^Faux entry designed for proc / payplan combo
			******************************************************/
			Assert.AreEqual(3,chargeResult.ListAccountEntries.Count);
			Assert.AreEqual(1,chargeResult.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.AmountOriginal==100
				&& x.AmountEnd==0//Procedure is attached to a payment plan so it has no value.
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_TreatmentPlanDiscounts() {
			...
			******************************************************/
			...
			Payment payment1=PaymentT.MakePayment(pat.PatNum,50,DateTime.Now.AddDays(-1));
			//Check explicit credits only box
			List<AccountEntry> listAccountCharges=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,doIncludeTreatmentPlanned:true).ListAccountEntries;
			/*****************************************************
			AccountEntry:  Today  prov?  pat
				^AmountOriginal  = $100
				^AmountAvailable = $85  []
			...
			******************************************************/
			Assert.AreEqual(100,listAccountCharges[0].AmountOriginal);
			Assert.AreEqual(85,listAccountCharges[0].AmountAvailable);
			Assert.AreEqual(85,listAccountCharges[0].AmountEnd);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_TreatmentPlanDiscountsGreaterThanProcFee() {
			...
			******************************************************/
			Assert.AreEqual(100,listAccountCharges[0].AmountOriginal);
			Assert.AreEqual(-5,listAccountCharges[0].AmountAvailable);
			Assert.AreEqual(-5,listAccountCharges[0].AmountEnd);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_TreatmentPlanDiscountsNegative() {
			...
			******************************************************/
			Assert.AreEqual(100,listAccountCharges[0].AmountOriginal);
			Assert.AreEqual(205,listAccountCharges[0].AmountAvailable);
			Assert.AreEqual(205,listAccountCharges[0].AmountEnd);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanWithAPR() {
			...
			******************************************************/
			...
			AccountEntry:  Today-2Y+5M  provNum  pat   $268.99
				...
				^Interest  = $2.22
			******************************************************/
			Assert.AreEqual(13,chargeResult.ListAccountEntries.Count);
			Assert.AreEqual(1,chargeResult.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PreviousTransfersConsideredImplicit() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults constructResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,payment:pay2);
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count);
			Assert.AreEqual(1,constructResults.ListAccountEntries.Count(x => x.PatNum==pat.PatNum
				&& x.ProcNum==proc1.ProcNum
				&& x.ProvNum==provNum1
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_NegSplitsImplicitlyUsedByPosSources() {
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(0,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(ClaimProc)).Count);
			Assert.AreEqual(1,results.ListAccountEntries.FindAll(x => x.AmountEnd==50).Count);//PaySplits are opposite signed as account charges. 
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_IncomeTransferToAdjustment() {
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(1,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(Procedure) && x.AmountEnd==25).Count);
			Assert.AreEqual(1,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(Adjustment) && x.AmountEnd==0).Count);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_ExplicitlyLinkCredits_TransferFromWrongProviderSplit() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(4,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.ProvNum==provNumA
				&& x.PatNum==pat.PatNum
				&& x.AmountEnd==0
				&& x.AmountOriginal==65
				&& x.ProcNum==proc.ProcNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.ProvNum==provNumA
				...
				&& x.AmountOriginal==-65
				&& x.ProcNum==proc.ProcNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.ProvNum==provNumB
				&& x.PatNum==pat.PatNum
				&& x.AmountEnd==65
				&& x.AmountOriginal==65
				&& x.ProcNum==proc.ProcNum));
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_UnallocatedSplitsGetTransferredAndReducedValue() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults explicitLinkResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			//The two payments to unallocated are for the same provider thus will get combined into one fake account entry worth $0.
			Assert.AreEqual(1,explicitLinkResults.ListAccountEntries.Count);
			Assert.AreEqual(1,explicitLinkResults.ListAccountEntries.Count(x => x.ProvNum==provNum
				&& x.PatNum==pat.PatNum
				&& x.AmountEnd==0
				&& x.AmountOriginal==0));
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_UnallocatedTransferAndOverpaidProcedureGetLinkedCorrectly() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults linkResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(3,linkResults.ListAccountEntries.Count);
			transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
			/*****************************************************
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_AdjustmentsAreNotCountedTwiceInTheIncomeTransferManger() {
			...
			PaymentEdit.ConstructResults linkResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(2,linkResults.ListAccountEntries.FindAll(x => x.AmountEnd!=0).Count);
			//adjustment should appear in the list of items needing to be transferred.
			Assert.AreEqual(0,linkResults.ListAccountEntries.First(x => x.GetType()==typeof(Adjustment)).AmountEnd);//adjustment should be used up
			//procedure should have it's value without considering the adjustment
			Assert.AreEqual(100,linkResults.ListAccountEntries.First(x => x.GetType()==typeof(Procedure)).AmountEnd);//adjustment should have applied to proc
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
			//Make the income transfer official by inserting the splits into the database and then run the transfer again.  No splits should be made!
			PaySplits.InsertMany(0,transferResults.ListSplitsCur);
			transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PaySplitsWithMissingLinksStillBalanceCorrectly() {
			...
			PaymentEdit.ConstructResults linkResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(-50,linkResults.ListAccountEntries.Sum(x => x.AmountEnd));//charges should sum to -50 (credit)
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
			//Make the income transfer official by inserting the splits into the database and then run the transfer again.  No splits should be made!
			PaySplits.InsertMany(0,transferResults.ListSplitsCur);
			transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_RefundsBalanceToZero() {
			...
			PaymentEdit.ConstructResults linkResults=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true);
			Assert.AreEqual(0,linkResults.ListAccountEntries.Sum(x => x.AmountEnd));//splits should counteract each other.
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
			//Make the income transfer official by inserting the splits into the database and then run the transfer again.  No splits should be made!
			PaySplits.InsertMany(0,transferResults.ListSplitsCur);
			transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
		...
		public void PaymentEdit_ConstructLinkChargeCredits_AdjustmentsLinkToPaySplitsThroughProcedures() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat1.PatNum,isIncomeTxfr:true);
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				&& x.AmountEnd==0
				&& x.AmountOriginal==10));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(PaySplit)
				&& x.AmountEnd==0
				&& x.AmountOriginal==-10));
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_DynamicPayPlanOffsettingDebit() {
			...
			******************************************************/
			...
			PayPlan payPlan1=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,pat.Guarantor,DateTime.Today,0,0,100,
				new List<Procedure>(){ proc1 },new List<Adjustment>(),frequency:PayPlanFrequency.Monthly,provNum:provNum1,dynamicPayPlanTPOptions:DynamicPayPlanTPOptions.TreatAsComplete);
			List<PayPlanCharge> listPayPlanCharges=PayPlanCharges.GetForPayPlan(payPlan1.PayPlanNum);
			...
			Assert.AreEqual(1,listPayPlanCharges.Count(x => x.ChargeDate==DateTime.Today
				...
				&& x.FKey==proc1.ProcNum
				&& x.LinkType==PayPlanLinkType.Procedure));
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,doIncludeTreatmentPlanned:true);
			Assert.AreEqual(2,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.PayPlanNum==0
				&& x.ProcNum==proc1.ProcNum
			...
			results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,doIncludeTreatmentPlanned:true);
			Assert.AreEqual(3,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.PatNum==pat.PatNum
				&& x.PayPlanNum==0
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.AmountOriginal==100
				&& ((FauxAccountEntry)x).Principal==100
				&& ((FauxAccountEntry)x).PrincipalAdjusted==80
				&& x.AmountEnd==80));
		}

		// this came about because of a rare scenario with a dynamic payment plan with the guarantor outside of the family, only having adjustments on the payment plan, and trying to run an income transfer on the guarantor with no payments. Income Transfer manager was trying to create paysplits moving money from and to the same adjustments. 
		[TestMethod]
		public void PaymentEdit_ConstructAndLinkChargeCredits_DynamicPayPlanGuarantorOutsideFamilyOnlyAdjustments() {
			// create dymanic payment plan with only an adjustment for the patient. Guarantor outside of the family.
			// try to start income-transfer process on guarantor, no payments made
			string suffix=MethodBase.GetCurrentMethod().Name;
			Def defPosAdjType=DefT.CreateDefinition(DefCat.AdjTypes,suffix,"+");
			Patient pat=PatientT.CreatePatient(suffix);
			Patient guar=PatientT.CreatePatient(suffix+"guar");
			long provNum1=ProviderT.CreateProvider(suffix);
			Adjustment adj1=AdjustmentT.MakeAdjustment(pat.PatNum,50,adjType:defPosAdjType.DefNum);//Make a positive adjustment.
			PayPlan payPlan1=PayPlanT.CreateDynamicPaymentPlan(pat.PatNum,guar.PatNum,DateTime.Today,0,0,16.18,
				new List<Procedure>() { },new List<Adjustment>() { adj1 },frequency:PayPlanFrequency.Monthly,provNum:provNum1);
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(guar.PatNum,isIncomeTxfr:true);
			// income transfer should recognize we don't have anything to transfer, just the one charge and the one adjustment
			Assert.AreEqual(2,results.ListAccountEntries.Count);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_DynamicPayPlanDebitsLinkedToProcs() {
			...
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider(suffix);
			Procedure proc1=ProcedureT.CreateProcedure(pat,"TDLP1",ProcStat.C,"",64,DateTime.Today,provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"TDLP1",ProcStat.C,"",58,DateTime.Today,provNum:provNum1);
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanPayFirstEntries() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat1.PatNum,payment:payCur,listAccountEntriesPayFirst:listPayFirstEntries);
			/*****************************************************
			AccountEntry:      Today  provNum1  pat   $200
				^Represents proc1
			AccountEntry:      Today  provNum1  pat   $75
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				...
				&& x.ProvNum==provNum1
				&& ((FauxAccountEntry)x).Principal==150
				&& ((FauxAccountEntry)x).PrincipalAdjusted==75
				&& x.AmountEnd==0
				&& x.Date==DateTime.Today
				&& x.ProcNum==proc2.ProcNum));
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_DoNotImplicitlyLinkToOverpaidProcs() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Adjustment)
				...
				&& x.ProcNum==proc2.ProcNum
				&& x.UnearnedType==0));
			Assert.AreEqual(0,results.ListPaySplits.Count);
		}
		...
		public void PaymentEdit_ConstructAndLinkChargeCredits_UpToDate() {
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum,isIncomeTxfr:true,dateAsOf:dateAsOf);
		...
		[TestMethod]
		[Documentation.VersionAdded("20.5.66")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_ConstructAndLinkChargeCredits_MismatchedOffsettingAdjustments)]
		[Documentation.Description("Patient has 1 procedure for the first provider for $200. The procedure has 2 adjustments attached. Both adjustments are for provider 2 which makes them incorrectly linked to the procedure. However, the 1st adjustment is for $300 and the 2nd adjustment is for ($300) which means that they should offset each other. The current account should reflect the procedure wanting $200 and both adjustments should be set to $0 because they technically offset.")]
		public void PaymentEdit_ConstructAndLinkChargeCredits_MismatchedOffsettingAdjustments() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat
				Create Procedure: proc1  Today  provNum1  pat   $200
				Create Procedure: adj1   Today  provNum2  pat   $300
					^Attached to proc1
				Create Procedure: adj1   Today  provNum2  pat  -$300
					^Attached to proc1
			******************************************************/
			...
			long provNum1=ProviderT.CreateProvider($"{suffix}-1");
			long provNum2=ProviderT.CreateProvider($"{suffix}-2");
			Procedure proc1=ProcedureT.CreateProcedure(pat,"MOA01",ProcStat.C,"",200,provNum:provNum1);
			//Incorrectly link some adjustments to the procedure (wrong provider) but have an offsetting adjustment.
			Adjustment adj1=AdjustmentT.MakeAdjustment(pat.PatNum,300,procNum:proc1.ProcNum,provNum:provNum2);
			Adjustment adj2=AdjustmentT.MakeAdjustment(pat.PatNum,-300,procNum:proc1.ProcNum,provNum:provNum2);
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum);
			//Asser that the procedure's value did not change and that the two incorrectly linked adjustment offset each other.
			Assert.AreEqual(3,results.ListAccountEntries.Count);
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.ProvNum==provNum1
				&& x.PatNum==pat.PatNum
		...
		[TestMethod]
		public void PaymentEdit_ConstructAndLinkChargeCredits_PayPlanWithNegativeCreditAndDebit() {
		/*****************************************************
				Create Provider:   provNum
				Create Patient:    pat

				Create procedure:           Today   provNum   pat   $74.00   Debit

				Create paymentPlan:         Today   provNum   pat   $74.00
					^payPlanChargeProc:       Today   provNum   pat   $74.00   Credit (Initial Credit for Procedure)
						^Attached to procedure
					^payPlanChargeInit:       Today   provNum   pat   $74.00   Debit (Initial PayPlan Charge)
					^payPlanChargeNegAdj:     Today   provNum   pat  -$49.00   Debit (Negative PayPlan Adjustment)
					^payPlanChargeNegCred:    Today   provNum   pat  -$49.00   Credit (Negative PayPlan Credit)
						^Attached to procedure
					^payPlanChargeCloseOut:   Today   provNum   pat   $0       Debit (Close Out Charge)

				Create payment:             Today   provNum   pat   $25
					^Attached to payPlan
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			long unearnedType=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			long provNum=ProviderT.CreateProvider($"{suffix}-Prov");
			Patient pat=PatientT.CreatePatient($"{suffix}-Pat");
			Procedure procedure=ProcedureT.CreateProcedure(pat,"D1110",ProcStat.C,"0",74,provNum:provNum);
			PayPlan paymentPlan=PayPlanT.CreatePayPlan(pat.PatNum,74,0,DateTime.Today,provNum);
			//Initial PayPlan Credit - Linked to procedure
			PayPlanCharge payPlanChargeProc=PayPlanChargeT.CreateOne(paymentPlan.PayPlanNum,pat.PatNum,pat.PatNum,DateTime.Today,74,
				provNum:provNum,chargeType:PayPlanChargeType.Credit,procNum:procedure.ProcNum);
			//Initial PayPlan Debit
			PayPlanCharge payPlanChargeInit=PayPlanChargeT.CreateOne(paymentPlan.PayPlanNum,pat.PatNum,pat.PatNum,DateTime.Today,74,
				provNum:provNum,chargeType:PayPlanChargeType.Debit);
			//$25 Payment
			Payment payment=PaymentT.MakePayment(pat.PatNum,25,payDate:DateTime.Today,provNum:provNum,payPlanNum:paymentPlan.PayPlanNum);
			//PayPlan Negative Debit
			PayPlanCharge payPlanChargeNegAdj=PayPlanChargeT.CreateOne(paymentPlan.PayPlanNum,pat.PatNum,pat.PatNum,DateTime.Today,-49,
				provNum:provNum,chargeType:PayPlanChargeType.Debit);
			//PayPlan Negative Credit - Linked to procedure
			PayPlanCharge payPlanChargeNegCred=PayPlanChargeT.CreateOne(paymentPlan.PayPlanNum,pat.PatNum,pat.PatNum,DateTime.Today,-49,
				provNum:provNum,chargeType:PayPlanChargeType.Credit,procNum:procedure.ProcNum);
			//Close Out Charge
			PayPlanCharge payPlanChargeCloseOut=PayPlanChargeT.CreateOne(paymentPlan.PayPlanNum,pat.PatNum,pat.PatNum,DateTime.Today,0,
				provNum:provNum,chargeType:PayPlanChargeType.Debit);
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(pat.PatNum);
			Assert.AreEqual(3,results.ListAccountEntries.Count);
			//Procedure should have an AmtEnd of 49. It was worth 79, until a PayPlan was introduced, which gave it $79 of credit, making the procedure fully credited/paid for.
			//The PayPlan comes with a debit of $79. After paying $25 to the payplan, the PayPlan still had debit of $49. However, a negative Credit and Debit were then introduced.
			//The -$49 PayPlan Credit is attached to the procedure, just like the initial PayPlan credit. This negative credit works like a debit, making the procedure worth $49.
			//The -$49 PayPlan Debit works like a credit, making the PayPlan worth $0 instead of $49.
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(Procedure)
				&& x.ProvNum==provNum
				&& x.PatNum==pat.PatNum
				&& x.AmountEnd==49
				...
				&& x.AdjNum==0
				&& x.UnearnedType==0));
			//The Initial PayPlan Debit was for 74. Should have an AmtEnd of 0, as it's fully paid for. (?)Should only be worth 25, since 49 was shaved off.
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
				&& x.PayPlanChargeNum==payPlanChargeInit.PayPlanChargeNum
				&& x.ProvNum==provNum
				&& ((FauxAccountEntry)x).Principal==74
				&& x.AmountEnd==0
				&& x.ProcNum==procedure.ProcNum));
			//The negative PayPlan Debit was for -49. This should have an AmtEnd of 0, as it was used to negate the Negative Credit.
			Assert.AreEqual(1,results.ListAccountEntries.Count(x => x.GetType()==typeof(FauxAccountEntry)
				&& x.PatNum==pat.PatNum
				&& x.PayPlanChargeNum==payPlanChargeNegAdj.PayPlanChargeNum
				&& x.ProvNum==provNum
				&& ((FauxAccountEntry)x).Principal==-49
				&& x.AmountEnd==0
				&& x.ProcNum==0));
		}
		...
		[TestMethod]
		[Documentation.VersionAdded("21.2.27")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_BalanceAndIncomeTransfer_DynamicPayPlanInterestCharge)]
		[Documentation.Description(@"When a user creates a Dynamic Payment Plan, with attached production, payments can be made Explicitly or Implicitly linked to either one or both. In the event that the user wishes to create offsetting payments that do not Explicitly link to production, we want to be certain the Dynamic Payment Plan is not overpaid. We also do not want to allow money to be taken directly from production. If a user creates a Dynamic Payment Plan for Patient Joe, giving the plan a 5 APR and a Procedure with a total cost of $100, they may choose to pay the first payment's principal first using the following Paysplits:
<table>
	<tr>
		<td><b>Patient</b></td>
...
<table>
	...
	<tr>
		...
		[TestMethod]
		public void PaymentEdit_BalanceAndIncomeTransfer_DynamicPayPlanWithOffsettingAdjustments() {
			//Adjustments associated with a dynamic payment plan should not be offset by unattached negative adjustments.
			/*****************************************************
				Create Patient:  pat
				Create Provider: provNum
			...
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			PrefT.UpdateBool(PrefName.AdjustmentsOffsetEachOther,true);
			PrefT.UpdateBool(PrefName.IncomeTransfersTreatNegativeProductionAsIncome,true);
			long defNumUnearnedType=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			Patient patient=PatientT.CreatePatient(suffix);
			long provNum=ProviderT.CreateProvider(suffix);
			...
			Adjustment adjustmentNegative=AdjustmentT.MakeAdjustment(patient.PatNum,-20,provNum:provNum);
			//Create a dynamic payment plan associated with the procedure and positive adjustment ($50 + $70 = $120).
			PayPlan dynamicPayPlan=PayPlanT.CreateDynamicPaymentPlan(patient.PatNum,patient.PatNum,DateTime.Today.AddMonths(1),0,0,120,
				new List<Procedure>(){ procedure },new List<Adjustment>() { adjustmentPositive },provNum:provNum);
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(patient.PatNum);
			//The negative and postitive adjustments will not offset each other due to the positive adjustment's value being completely consumed by the payment plan.
			//Assert that the negative adjustment is treated as income and is transferred to unearned since the preference to treat negative production as income is turned on.
			Assert.AreEqual(2,transferResults.ListSplitsCur.Count);
			Assert.AreEqual(1,transferResults.ListSplitsCur.Count(x => x.PatNum==patient.PatNum
				&& x.ProvNum==provNum
				&& x.AdjNum==adjustmentNegative.AdjNum
				&& x.ProcNum==0
				&& x.PayPlanNum==0
			...
			Assert.AreEqual(1,transferResults.ListSplitsCur.Count(x => x.PatNum==patient.PatNum
				...
				&& x.PayPlanNum==0
				&& x.SplitAmt==20
				&& x.UnearnedType==defNumUnearnedType));
			//Make the income transfer official by inserting the splits into the database and then run the transfer again.  No splits should be made!
			PaySplits.InsertMany(0,transferResults.ListSplitsCur);
			transferResults=PaymentT.BalanceAndIncomeTransfer(patient.PatNum);
			Assert.AreEqual(0,transferResults.ListSplitsCur.Count);
		...
		[TestMethod]
		public void PaymentEdit_BalanceAndIncomeTransfer_PayPlanGuarantorOutOfFamilyPartialAmount() {
			/*****************************************************
				Create Patient:  pat1 (family 1)
				Create Patient:  pat2 (family 2)
				Create Provider: prov1
				Create proc1:    Today-4M  prov1  pat1   $100
				Create payplan:  Today-3M  prov1  pat1   $20
				Create prepay:   Today-1M  prov0  pat2   $1000
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat1=PatientT.CreatePatient($"{suffix}-1");
			Patient pat2=PatientT.CreatePatient($"{suffix}-2");
			long provNum1=ProviderT.CreateProvider(suffix);
			Procedure proc1=ProcedureT.CreateProcedure(pat1,"PPPGOOF",ProcStat.C,"",100,procDate:DateTime.Today.AddMonths(-4),provNum:provNum1);
			DateTime datePayPlanStart=DateTime.Today.AddMonths(-3);
			//Create a payment plan for pat 1 with pat 2 as the guarantor but do not cover the entire procedure.
			PayPlan payplan=PayPlanT.CreatePayPlanWithCredits(pat1.PatNum,20,datePayPlanStart,provNum:provNum1,listProcs:new List<Procedure>() { proc1 },
				totalAmt:20,guarantorNum:pat2.PatNum);
			//Have a large amount of unearned money sitting in the account for the guarantor of the payment plan.
			PaySplit prePay=PaySplitT.CreatePrepayment(pat2.PatNum,100,DateTime.Today.AddMonths(-1));
			//Run the income transfer on the family for the guarantor of the payment plan.
			//Only $20 should be transferred to the payment plan / procedure.
			//There should be NO money directly transferred to the procedure even though it still needs $80 to be paid off.
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat2.PatNum);
			Assert.AreEqual(2,transferResults.ListSplitsCur.Count);
			Assert.AreEqual(1,transferResults.ListSplitsCur.Count(x => x.ProvNum==0
				&& x.PatNum==pat2.PatNum
				&& x.SplitAmt==-20
				...
				&& x.UnearnedType==prePay.UnearnedType));
			Assert.AreEqual(1,transferResults.ListSplitsCur.Count(x => x.ProvNum==provNum1
				&& x.PatNum==pat2.PatNum//Note that this is the guarantor on the payment plan and not the patient on the procedure.
				&& x.SplitAmt==20
				&& x.ProcNum==proc1.ProcNum
				&& x.PayPlanNum==payplan.PayPlanNum
				&& x.UnearnedType==0));
			//Make the income transfer official by inserting the splits into the database and then run the transfer again.  No splits should be made!
			PaySplits.InsertMany(0,transferResults.ListSplitsCur);
			transferResults=PaymentT.BalanceAndIncomeTransfer(pat2.PatNum);
			Assert.AreEqual(0,transferResults.ListSplitsCur.Count);
		}
		...
		public void PaymentEdit_BalanceAndIncomeTransfer_UnlinkedIncomeXferShowsProcOwingMoney() {
			...
			******************************************************/
			...
			Assert.AreEqual(1,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(Procedure) 
				&& x.AmountOriginal==50
				&& x.AmountEnd==50).Count);
			Assert.AreEqual(1,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(PaySplit)
				&& x.AmountOriginal==50 
				&& x.AmountEnd==50).Count);
			Assert.AreEqual(1,results.ListAccountEntries.FindAll(x => x.GetType()==typeof(PaySplit)
				&& x.AmountOriginal==50 
		...
		public void PaymentEdit_BalanceAndIncomeTransfer_DoNotTransferInsuranceOverpayment() {
			...
			******************************************************/
			...
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
			//We should be limited to transferring $30 away from proc1 and towards proc2.
			Assert.AreEqual(4,transferResults.ListSplitsCur.Count);
		...
		public void PaymentEdit_BalanceAndIncomeTransfer_DoNotTransferInsuranceOverpaymentPartial() {
			...
			******************************************************/
			...
			PaymentT.MakePayment(pat.PatNum,30,DateTime.Today,provNum:provNum,procNum:proc1.ProcNum);
			PaymentEdit.IncomeTransferData transferResults=PaymentT.BalanceAndIncomeTransfer(pat.PatNum);
			//We should be limited to transferring $10 away from proc1 and towards proc2.
			Assert.AreEqual(4,transferResults.ListSplitsCur.Count);
			Assert.AreEqual(1,transferResults.ListSplitsCur.Count(x => x.PatNum==pat.PatNum
				&& x.ProcNum==proc1.ProcNum
				&& x.ProvNum==provNum
		...
		[TestMethod]
		public void PaymentEdit_BalanceAndIncomeTransfer_OverpaidPayPlanWithIns() {
			/*****************************************************
				Create Provider: provNum1
				...
				Create claimProc1:          Today     provNum1  pat1   $33.32 (ins pay amt)
					^Attached to procedure1
				Create claimProc2:          Today     provNum1  pat2   $33.32 (ins estimate)
					^Attached to procedure2
				...
				Create payPlan1:            Today-4M  provNum1  pat1   $100.00
					...
					^payPlanCharge3:          Today-2M  provNum1  pat1   $33.32  (debit - charge 3)
					^payPlanChargeCloseOut:   Today-1M  provNum1  pat1   $0      (debit - close out charge)
					^payPlanChargeNegAdj:     Today-1M  provNum1  pat1  -$33.32  (debit - neg adjustment)
					^payPlanChargeNegCred:    Today-1M  provNum1  pat1  -$33.32  (credit - neg adjustment)
						^Attached to procedure1
			...
			******************************************************/
			...
			PaymentEdit.ConstructResults results=PaymentEdit.ConstructAndLinkChargeCredits(patient1.PatNum,isIncomeTxfr:true,dateAsOf:DateTime.Now);
		...
		[TestMethod]
		[Documentation.VersionAdded("21.3")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_GetIncomeTransferSplitsFIFO_SuggestImplicitTransfers)]
		[Documentation.Description("Patient has 3 procedures for 3 different providers on 3 different days. The 1st procedure is for provider #1 and is dated 01/01/2021 for $10. The 2nd procedure is for provider #2 and is dated 02/02/2021 for $20. The 3rd procedure is for provider #3 and is dated 03/03/2021 for $30. The patient made 2 payments to unearned. The 1st unearned payment is associated to provider #3 and is dated 01/01/2021 for $30. The 2nd unearned payment is associated to provider #2 and is dated 02/02/2021 for $20. Running a FIFO income transfer should take money from unearned and apply it directly to all of the 1st and 2nd procedures and a partial amount to the 3rd procedure leaving it wanting $10.")]
		public void PaymentEdit_GetIncomeTransferSplitsFIFO_SuggestImplicitTransfers() {
			//Transfer unearned money FIFO style instead of explicitly linked style.
			/*****************************************************
				proc1:  pat  prov1  01/01/2021  $10
				proc2:  pat  prov2  02/02/2021  $20
				proc3:  pat  prov3  03/03/2021  $30
				pay1:   pat  prov3  01/01/2021  $30
				pay2:   pat  prov2  02/02/2021  $20
					^FIFO will move the oldest unearned money to the three procs and proc3 will be left wanting $10.
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"{suffix}-1");
			long provNum2=ProviderT.CreateProvider($"{suffix}-2");
			long provNum3=ProviderT.CreateProvider($"{suffix}-3");
			Procedure proc1=ProcedureT.CreateProcedure(pat,"FIFOSIT1",ProcStat.C,"",10,new DateTime(2021,1,1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"FIFOSIT2",ProcStat.C,"",20,new DateTime(2021,2,2),provNum:provNum2);
			Procedure proc3=ProcedureT.CreateProcedure(pat,"FIFOSIT3",ProcStat.C,"",30,new DateTime(2021,3,3),provNum:provNum3);
			PaySplit pay1=PaySplitT.CreatePrepayment(pat.PatNum,30,new DateTime(2021,1,1),provNum3);
			PaySplit pay2=PaySplitT.CreatePrepayment(pat.PatNum,20,new DateTime(2021,2,2),provNum2);
			PaymentEdit.IncomeTransferData incomeTransferData=PaymentEdit.GetIncomeTransferDataFIFO(pat.PatNum,DateTime.Today);
			List<PaySplit> listPaySplits=incomeTransferData.ListSplitsCur;
			//FIFO will move the oldest unearned money to the three procs and proc3 will be left wanting $10.
			Assert.AreEqual(6,listPaySplits.Count);
			//$10 will be moved from unearned to proc1.
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==-10
				&& x.PatNum==pay1.PatNum
				&& x.ProvNum==pay1.ProvNum
				&& x.ClinicNum==pay1.ClinicNum
				&& x.ProcNum==0
				&& x.UnearnedType==pay1.UnearnedType));
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==10
				&& x.PatNum==proc1.PatNum
				&& x.ProvNum==proc1.ProvNum
				&& x.ClinicNum==proc1.ClinicNum
				&& x.ProcNum==proc1.ProcNum
				&& x.UnearnedType==0));
			//$20 will be moved from unearned to proc2.
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==-20
				&& x.PatNum==pay1.PatNum
				&& x.ProvNum==pay1.ProvNum
				&& x.ClinicNum==pay1.ClinicNum
				...
				&& x.UnearnedType==pay1.UnearnedType));
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==20
				&& x.PatNum==proc2.PatNum
				&& x.ProvNum==proc2.ProvNum
				&& x.ClinicNum==proc2.ClinicNum
				&& x.ProcNum==proc2.ProcNum
				&& x.UnearnedType==0));
			//$20 will be moved from unearned to proc3.
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==-20
				&& x.PatNum==pay2.PatNum
				&& x.ProvNum==pay2.ProvNum
				&& x.ClinicNum==pay2.ClinicNum
				&& x.ProcNum==0
				&& x.UnearnedType==pay2.UnearnedType));
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==20
				&& x.PatNum==proc3.PatNum
				&& x.ProvNum==proc3.ProvNum
				&& x.ClinicNum==proc3.ClinicNum
				&& x.ProcNum==proc3.ProcNum
				&& x.UnearnedType==0));
			//Make the income transfer official by inserting the splits into the database and then run the transfer again.  No splits should be made!
			PaySplits.InsertMany(0,listPaySplits);
			incomeTransferData=PaymentEdit.GetIncomeTransferDataFIFO(pat.PatNum,DateTime.Today);
			Assert.AreEqual(0,incomeTransferData.ListSplitsCur.Count);
		}

		[TestMethod]
		[Documentation.VersionAdded("21.3")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_GetIncomeTransferSplitsFIFO_DoNotTransferImplicitlyLinkedIncome)]
		[Documentation.Description("Patient has 2 procedures for 2 different providers on 2 different days. The 1st procedure is for provider #1 and is dated 01/01/2021 for $10. The 2nd procedure is for provider #2 and is dated 02/02/2021 for $20. The patient made 2 payments. The 1st payment is associated to provider #2 and the 1st procedure and is dated 01/01/2021 for $10. The 2nd payment is associated to provider #1 and the 2nd procedure and is dated 02/02/2021 for $20. Running a FIFO income transfer should do nothing because patient, provider, and clinic mismatches are ignored and the procedures are paid in full.")]
		public void PaymentEdit_GetIncomeTransferSplitsFIFO_DoNotTransferImplicitlyLinkedIncome() {
			//Do not transfer incorrectly linked income.
			/*****************************************************
				proc1:  pat1  prov1  01/01/2021  $10
				proc2:  pat1  prov2  02/02/2021  $20
				pay1:   pat1  prov2  01/01/2021  $10 (attached to proc1)
				pay2:   pat1  prov1  02/02/2021  $20 (attached to proc2)
					^FIFO will do nothing because pat/prov/clinic are ignored and the procedures are paid in full.
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			long defNumPrepaymentUnearnedType=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"{suffix}-1");
			long provNum2=ProviderT.CreateProvider($"{suffix}-2");
			Procedure proc1=ProcedureT.CreateProcedure(pat,"FIFODNTILI1",ProcStat.C,"",10,new DateTime(2021,1,1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"FIFODNTILI2",ProcStat.C,"",20,new DateTime(2021,2,2),provNum:provNum2);
			Payment pay1=PaymentT.MakePayment(pat.PatNum,10,payDate:new DateTime(2021,1,1),provNum:provNum2,procNum:proc1.ProcNum);
			Payment pay2=PaymentT.MakePayment(pat.PatNum,20,payDate:new DateTime(2021,2,2),provNum:provNum1,procNum:proc2.ProcNum);
			PaymentEdit.IncomeTransferData incomeTransferData=PaymentEdit.GetIncomeTransferDataFIFO(pat.PatNum,DateTime.Today);
			//FIFO will do nothing because pat/prov/clinic are ignored and the procedures are paid in full.
			Assert.AreEqual(0,incomeTransferData.ListSplitsCur.Count);
		}
		...
		[TestMethod]
		[Documentation.VersionAdded("21.3")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_GetIncomeTransferSplitsFIFO_InsurancePaymentPlan)]
		...
		public void PaymentEdit_GetIncomeTransferSplitsFIFO_InsurancePaymentPlan() {
			/*****************************************************
				ProcA ProvA
				ProcB ProvA
				ClaimA InsPlanA
			...
			******************************************************/
			...
			AccountEntry:  Today  provNum  pat   $200
				^Represents procB.
			******************************************************/
			//Make a large donation to unearned and make sure that procA only wants $100 and procB only wants $200 due to insurance payment plan covering everything else.
			PaySplitT.CreatePrepayment(pat.PatNum,1000,DateTime.Today);
			PaymentEdit.IncomeTransferData incomeTransferData=PaymentEdit.GetIncomeTransferDataFIFO(pat.PatNum,DateTime.Today);
			List<PaySplit> listPaySplits=incomeTransferData.ListSplitsCur;
			Assert.AreEqual(4,listPaySplits.Count);
			Assert.AreEqual(1,listPaySplits.Count(x => x.PatNum==pat.PatNum
		...
		[TestMethod]
		[Documentation.VersionAdded("21.3")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_GetIncomeTransferSplitsFIFO_ImplicitlyTransferOverpayments)]
		...
		public void PaymentEdit_GetIncomeTransferSplitsFIFO_ImplicitlyTransferOverpayments() {
			//Transferring overpayment FIFO style instead of explicitly linked style.
			/*****************************************************
				proc1:  pat1  prov1  01/01/2021  $10
				proc2:  pat1  prov2  02/02/2021  $20
				pay1:   pat1  prov2  01/01/2021  $10 (attached to proc1)
				pay2:   pat1  prov1  02/02/2021  $10 (attached to proc1)
					^FIFO will transfer money away from proc1 since it is overpaid and then will transfer it to proc2.
			******************************************************/
			PrefT.UpdateBool(PrefName.IncomeTransfersTreatNegativeProductionAsIncome,true);
			string suffix=MethodBase.GetCurrentMethod().Name;
			long defNumPrepaymentUnearnedType=PrefC.GetLong(PrefName.PrepaymentUnearnedType);
			...
			long provNum1=ProviderT.CreateProvider($"{suffix}-1");
			long provNum2=ProviderT.CreateProvider($"{suffix}-2");
			Procedure proc1=ProcedureT.CreateProcedure(pat,"FIFOITO1",ProcStat.C,"",10,new DateTime(2021,1,1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"FIFOITO2",ProcStat.C,"",20,new DateTime(2021,2,2),provNum:provNum2);
			Payment pay1=PaymentT.MakePayment(pat.PatNum,10,payDate:new DateTime(2021,1,1),provNum:provNum2,procNum:proc1.ProcNum);
			Payment pay2=PaymentT.MakePayment(pat.PatNum,10,payDate:new DateTime(2021,2,2),provNum:provNum1,procNum:proc1.ProcNum);
			PaymentEdit.IncomeTransferData incomeTransferData=PaymentEdit.GetIncomeTransferDataFIFO(pat.PatNum,DateTime.Today);
			List<PaySplit> listPaySplits=incomeTransferData.ListSplitsCur;
			//FIFO will transfer away from proc1 since it is overpaid and then will transfer it to proc2.
			Assert.AreEqual(4,listPaySplits.Count);
			//$10 will be moved from proc1 to unearned. Overpayment transfers will always match the information on the production.
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==-10
				&& x.PatNum==proc1.PatNum
				&& x.ProvNum==proc1.ProvNum
				&& x.ClinicNum==proc1.ClinicNum
			...
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==10
				...
				&& x.ClinicNum==proc1.ClinicNum
				&& x.ProcNum==0
				&& x.UnearnedType==defNumPrepaymentUnearnedType));
			//$10 will be moved from unearned to proc2.
			Assert.AreEqual(1,listPaySplits.Count(x => x.SplitAmt==-10
				&& x.PatNum==proc1.PatNum
				&& x.ProvNum==proc1.ProvNum
				&& x.ClinicNum==proc1.ClinicNum
		...
		[TestMethod]
		[Documentation.VersionAdded("21.3")]
		[Documentation.Numbering(Documentation.EnumTestNum.PaymentEdit_GetIncomeTransferSplitsFIFO_UnearnedOffsets)]
		[Documentation.Description("Patient has 1 procedure for provider #1 dated 01/01/2021 for $100. The patient has 2 unearned payments on the account. The 1st unearned payment is associated to provider #2 and is dated 01/01/2021 for $20. The 2nd unearned payment is associated to provider #1 and is dated 02/02/2021 for ($20). Running a FIFO income transfer should do nothing because there is technically no unearned money to allocate.")]
		public void PaymentEdit_GetIncomeTransferSplitsFIFO_UnearnedOffsets() {
			//Verify that unearned money offsets each other. E.g. $20 in prepayment and ($20) in donations should offset each other.
			/*****************************************************
				proc1:  pat1  prov1  01/01/2021   $100
				pay1:   pat1  prov2  01/01/2021   $20
				pay2:   pat1  prov1  02/02/2021  -$20
					^FIFO will do nothing because there is technically no unearned money to allocate.
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"{suffix}-1");
			long provNum2=ProviderT.CreateProvider($"{suffix}-2");
			Procedure proc1=ProcedureT.CreateProcedure(pat,"FIFOUO1",ProcStat.C,"",10,new DateTime(2021,1,1),provNum:provNum1);
			PaySplit pay1=PaySplitT.CreatePrepayment(pat.PatNum,20,datePay:new DateTime(2021,1,1),provNum:provNum2);
			PaySplit pay2=PaySplitT.CreatePrepayment(pat.PatNum,-20,datePay:new DateTime(2021,2,2),provNum:provNum1);
			PaymentEdit.IncomeTransferData incomeTransferData=PaymentEdit.GetIncomeTransferDataFIFO(pat.PatNum,DateTime.Today);
			//FIFO will do nothing because pat/prov/clinic are ignored and the procedures are paid in full.
			Assert.AreEqual(0,incomeTransferData.ListSplitsCur.Count);
		}
		...
		#region ReAutoSplitAllPaymentsForFamily Tests

		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_SingleSplitIntoMultiple() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat
				Create proc1: Today-1M  provNum1  pat  $100
				Create proc2: Today-1D  provNum2  pat  $100
				Create pay:   Today     provNum0  pat  $150
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			//Make some procedures for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat,"SSIM1",ProcStat.C,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"SSIM2",ProcStat.C,"",100,procDate:DateTime.Now.AddDays(-1),provNum:provNum2);
			//Make a large unallocated payment.
			Payment payment=PaymentT.MakePayment(pat.PatNum,150,DateTime.Today,provNum:0);
			//Assert that the above payment is the only payment in the database for pat and that it only has one split associated to it.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(1,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
				&& x.ProvNum==0
				&& x.SplitAmt==150
				&& x.UnearnedType==0));
			//Remake every payment split in the database by keeping payment objects (and amounts) but creating explitily linked FIFO splits.
			PaymentEdit.ReAutoSplitAllPaymentsForFamily(pat.PatNum);
			//Assert that the above payment is the only payment in the database for pat and that it only has explicitly linked splits associated to it.
			List<PaySplit> listPaySplitsAfter=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(2,listPaySplitsAfter.Count);
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc1.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==proc1.ProcNum
				&& x.ProvNum==proc1.ProvNum
				&& x.SplitAmt==100
				&& x.UnearnedType==0));
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc2.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==proc2.ProcNum
				&& x.ProvNum==proc2.ProvNum
				&& x.SplitAmt==50
				&& x.UnearnedType==0));
			//Assert that the sum of the new splits are equal to the sum of the old splits.
			Assert.AreEqual(listPaySplitsBefore.Sum(x => x.SplitAmt),listPaySplitsAfter.Sum(x => x.SplitAmt));
		}

		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_MultipleSplitsIntoSingle() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create proc1: Today-1M  provNum1  pat  $100
				Create pay:   Today                    $100
				 (^split 1:   Today     provNum0  pat   $25)
				 (^split 2:   Today     provNum0  pat   $25)
				 (^split 3:   Today     provNum0  pat   $25)
				 (^split 4:   Today     provNum0  pat   $25)
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			//Make a procedure for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat,"SSIM1",ProcStat.C,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			//Make an unallocated payment with multiple payment splits that sum up to the procedure's value.
			Payment payment=PaymentT.MakePaymentNoSplits(pat.PatNum,100,DateTime.Today,doInsert:false);
			Payments.Insert(payment,new List<PaySplit>() {
				PaySplitT.CreateOne(pat.PatNum,25,0,0,doInsert:false),
				PaySplitT.CreateOne(pat.PatNum,25,0,0,doInsert:false),
				PaySplitT.CreateOne(pat.PatNum,25,0,0,doInsert:false),
				PaySplitT.CreateOne(pat.PatNum,25,0,0,doInsert:false),
			});
			//Assert that the above payment is the only payment in the database for pat and that it only has four splits associated to it.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(4,listPaySplitsBefore.Count);
			Assert.AreEqual(4,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
				&& x.ProvNum==0
			...
			//Remake every payment split in the database by keeping payment objects (and amounts) but creating explitily linked FIFO splits.
			PaymentEdit.ReAutoSplitAllPaymentsForFamily(pat.PatNum);
			//Assert that the above payment is the only payment in the database for pat and that it only has one explicitly linked split associated to it.
			List<PaySplit> listPaySplitsAfter=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(1,listPaySplitsAfter.Count);
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc1.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==proc1.ProcNum
		...
		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_SplitsToFamilyMembers() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat1
				Create Patient: pat2
				Create Patient: pat3
				Create proc1: Today-1M  provNum1  pat1  $100
				Create proc2: Today-1D  provNum2  pat2  $100
				Create pay:   Today     provNum0  pat3  $150
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat1=PatientT.CreatePatient($"1-{suffix}");
			Patient pat2=PatientT.CreatePatient($"2-{suffix}",guarantor:pat1.PatNum);
			Patient pat3=PatientT.CreatePatient($"3-{suffix}",guarantor:pat1.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			//Make some procedures for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat1,"SSIM1",ProcStat.C,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat2,"SSIM2",ProcStat.C,"",100,procDate:DateTime.Now.AddDays(-1),provNum:provNum2);
			//Make a large unallocated payment.
			Payment payment=PaymentT.MakePayment(pat3.PatNum,150,DateTime.Today,provNum:0);
			//Assert that the above payment is the only payment in the database for pat and that it only has one split associated to it.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(new List<long>(){ pat1.PatNum,pat2.PatNum,pat3.PatNum });
			Assert.AreEqual(1,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat3.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
				&& x.ProvNum==0
				...
				&& x.UnearnedType==0));
			//Remake every payment split in the database by keeping payment objects (and amounts) but creating explitily linked FIFO splits.
			PaymentEdit.ReAutoSplitAllPaymentsForFamily(pat1.PatNum);
			//Assert that the above payment is the only payment in the database for pat and that it only has explicitly linked splits associated to it.
			List<PaySplit> listPaySplitsAfter=PaySplits.GetForPats(new List<long>(){ pat1.PatNum,pat2.PatNum,pat3.PatNum });
			Assert.AreEqual(2,listPaySplitsAfter.Count);
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc1.PatNum
				&& x.PayNum==payment.PayNum
				&& x.PayPlanNum==0
		...
		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_MultiplePayments() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat
				Create proc1: Today-1M  provNum1  pat  $100
				Create proc2: Today-1D  provNum2  pat  $100
				Create pay1:  Today     provNum0  pat  $100
					^Unallocated
				Create pay2:  Today     provNum0  pat  $100
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			...
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			//Make some procedures for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat,"MP001",ProcStat.C,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"MP002",ProcStat.C,"",100,procDate:DateTime.Now.AddDays(-1),provNum:provNum2);
			//Make two unallocated payments for the outstanding production.
			Payment payment1=PaymentT.MakePayment(pat.PatNum,100,DateTime.Today,provNum:0);
			Payment payment2=PaymentT.MakePayment(pat.PatNum,100,DateTime.Today,provNum:0);
			//Assert that the above payments are the only payments in the database for the pat.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(2,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment1.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
				&& x.ProvNum==0
				&& x.SplitAmt==100
			...
			//Remake every payment split in the database by keeping payment objects (and amounts) but creating explitily linked FIFO splits.
			PaymentEdit.ReAutoSplitAllPaymentsForFamily(pat.PatNum);
			//Assert that the above payments are the only payments in the database for the pat and that they are explicitly linked.
			List<PaySplit> listPaySplitsAfter=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(2,listPaySplitsAfter.Count);
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc1.PatNum
				&& x.PayNum==payment1.PayNum
		...
		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_MultiplePaymentsOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat
				...
				Create pay1:  Today     provNum0  pat  $100
					^Unallocated
				Create pay2:  Today     provNum0  pat  $150
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			...
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			//Make some procedures for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat,"MPO01",ProcStat.C,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"MPO02",ProcStat.C,"",100,procDate:DateTime.Now.AddDays(-1),provNum:provNum2);
			//Make two unallocated payments for the outstanding production.
			Payment payment1=PaymentT.MakePayment(pat.PatNum,100,DateTime.Today,provNum:0);
			Payment payment2=PaymentT.MakePayment(pat.PatNum,150,DateTime.Today,provNum:0);
			//Assert that the above payments are the only payments in the database for the pat.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(2,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
			...
			//Assert that the above payments are the only payments in the database for the pat and that they are explicitly linked.
			List<PaySplit> listPaySplitsAfter=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(3,listPaySplitsAfter.Count);
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc1.PatNum
				&& x.PayNum==payment1.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==proc1.ProcNum
			...
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc2.PatNum
				...
				&& x.SplitAmt==100
				&& x.UnearnedType==0));
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment2.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
				&& x.ProvNum==0
		...
		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_MultiplePaymentsPreservePayDate() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat
				Create proc1: Today-1M  provNum1  pat  $100
				Create proc2: Today-1D  provNum2  pat  $100
				Create pay1:  Today-1M  provNum0  pat  $100
					^Unallocated
				Create pay2:  Today-1D  provNum0  pat  $100
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			//Make helper DateTime objects for the procedures and corresponding payments.
			DateTime dateProc1=DateTime.Now.AddMonths(-1);
			DateTime dateProc2=DateTime.Now.AddDays(-1);
			//Make some procedures for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat,"MPPPD1",ProcStat.C,"",100,procDate:dateProc1,provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"MPPPD2",ProcStat.C,"",100,procDate:dateProc2,provNum:provNum2);
			//Make two unallocated payments for the outstanding production.
			Payment payment1=PaymentT.MakePayment(pat.PatNum,100,dateProc1,provNum:0);
			Payment payment2=PaymentT.MakePayment(pat.PatNum,100,dateProc2,provNum:0);
			//Assert that the above payments are the only payments in the database for the pat.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(2,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment1.PayNum
				&& x.DatePay.Date==dateProc1.Date
				&& x.PayPlanNum==0
				&& x.ProcNum==0
				&& x.ProvNum==0
				&& x.SplitAmt==100
			...
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc1.PatNum
				&& x.PayNum==payment1.PayNum
				&& x.DatePay.Date==payment1.PayDate.Date
				&& x.PayPlanNum==0
				&& x.ProcNum==proc1.ProcNum
				&& x.ProvNum==proc1.ProvNum
				&& x.SplitAmt==100
			...
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==proc2.PatNum
				&& x.PayNum==payment2.PayNum
				&& x.DatePay.Date==payment2.PayDate.Date
				&& x.PayPlanNum==0
				&& x.ProcNum==proc2.ProcNum
				&& x.ProvNum==proc2.ProvNum
				&& x.SplitAmt==100
		...
		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_RefundedFullPayment() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Provider: provNum3
				Create Patient: pat
				Create proc1: Today-1M  provNum1  pat  $100
				Create proc2: Today-1D  provNum2  pat  $100
				Create pay1:  Today-5D  provNum1  pat  $100
					^Unallocated
				Create pay2:  Today-4D  provNum3  pat -$100
					^Unallocated
				Create pay3:  Today     provNum0  pat  $200
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			long provNum3=ProviderT.CreateProvider($"3-{suffix}");
			//Make some procedures for the patient.
			Procedure proc1=ProcedureT.CreateProcedure(pat,"OP001",ProcStat.C,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure proc2=ProcedureT.CreateProcedure(pat,"OP002",ProcStat.C,"",100,procDate:DateTime.Now.AddDays(-1),provNum:provNum2);
			//Make two unallocated payments that offset each other (as if the user refunded the patient their payment the next day).
			Payment payment1=PaymentT.MakePayment(pat.PatNum,100,DateTime.Now.AddDays(-5),provNum:provNum1);
			Payment payment2=PaymentT.MakePayment(pat.PatNum,-100,DateTime.Now.AddDays(-4),provNum:provNum3);
			//Make a single unallocated payment for the outstanding production.
			Payment payment3=PaymentT.MakePayment(pat.PatNum,200,DateTime.Now,provNum:0);
			//Assert that the above payments are the only payments in the database for the pat.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(3,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment1.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
			...
			//Assert that the above payments are the only payments in the database for the pat and that they are explicitly linked.
			List<PaySplit> listPaySplitsAfter=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(4,listPaySplitsAfter.Count);
			Assert.AreEqual(1,listPaySplitsAfter.Count(x => x.PatNum==pat.PatNum
				&& x.PayNum==payment1.PayNum
				&& x.PayPlanNum==0
				&& x.ProcNum==0
		...
		[TestMethod]
		public void PaymentEdit_ReAutoSplitAllPaymentsForFamily_RefundedPartialPayment() {
			/*****************************************************
				Create Provider: provNum1
				Create Provider: provNum2
				Create Patient: pat
				Create proc1: Today-1M  provNum1  pat  $100
				Create proc2: Today-1D  provNum2  pat  $100
				Create pay1:  Today-5D  provNum0  pat  $100
					^Unallocated
				Create adj1:  Today-4D  provNum2  pat -$20
					^attached to proc2
				Create pay2:  Today-4D  provNum0  pat -$20
					^Unallocated
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient pat=PatientT.CreatePatient(suffix);
			...
			Procedure proc2=ProcedureT.CreateProcedure(pat,"OP002",ProcStat.C,"",100,procDate:DateTime.Now.AddDays(-1),provNum:provNum2);
			//Make two unallocated payments that offset each other (as if the user refunded the patient their payment the next day).
			Payment payment1=PaymentT.MakePayment(pat.PatNum,100,DateTime.Now.AddDays(-5),provNum:0);
			//Create a negative adjustment for part of proc2 and then "refund" it to the patient by creating a payment of the same amount.
			Adjustment adjustment1=AdjustmentT.MakeAdjustment(pat.PatNum,-20,adjDate:DateTime.Now.AddDays(-4),procDate:proc2.ProcDate,procNum:proc2.ProcNum,provNum:provNum2);
			Payment payment2=PaymentT.MakePayment(pat.PatNum,-20,DateTime.Now.AddDays(-4),provNum:0);
			//Assert that the above payments are the only payments in the database for the pat.
			List<PaySplit> listPaySplitsBefore=PaySplits.GetForPats(ListTools.FromSingle(pat.PatNum));
			Assert.AreEqual(2,listPaySplitsBefore.Count);
			Assert.AreEqual(1,listPaySplitsBefore.Count(x => x.PatNum==pat.PatNum
		...
		#region TransferInsuranceOverpaymentsForFamily Tests

		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_SingleProcOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure:  Today-1M  provNum1  pat  $100
				Create claimproc:  Today     provNum1  pat  $150
					^InsPayAmt overpaid the procedure by $50
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			...
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("SPO01");
			Benefit benefit=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Procedure procedure=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			insuranceInfo.ComputeEstimatesForProc(procedure);
			insuranceInfo.CreateClaim();
			//Have insurance overpay the procedure.
			insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure.ProcNum).InsPayAmt=150;
			...
			ClaimProc claimProc=insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure.ProcNum);
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc.PatNum
				&& x.ClinicNum==claimProc.ClinicNum
				&& x.ProcNum==claimProc.ProcNum
		...
		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_SingleProcNotOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure:  Today-1M  provNum1  pat  $100
				Create claimproc:  Today     provNum1  pat  $100
					^InsPayAmt pays procedure exactly what is owed.
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("SPNO1");
			Benefit benefit=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Procedure procedure=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			insuranceInfo.ComputeEstimatesForProc(procedure);
			insuranceInfo.CreateClaim();
			insuranceInfo.ReceiveClaims(doAddPayAmount:true);//Have insurance pay what they are supposed to.
			InsOverpayResult insOverpayResult=PaymentEdit.TransferInsuranceOverpaymentsForFamily(family);
			//Assert that the insurance overpayment does not suggest anything since insurance did not overpay.
			AssertEmptyInsOverpayResult(insOverpayResult);
		}

		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_SingleProcUnderpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure:  Today-1M  provNum1  pat  $100
				Create claimproc:  Today     provNum1  pat  $50
					^InsPayAmt pays procedure less than what is owed.
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("SPU01");
			Benefit benefit=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Procedure procedure=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			insuranceInfo.ComputeEstimatesForProc(procedure);
			insuranceInfo.CreateClaim();
			//Have insurance underpay the procedure.
			insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure.ProcNum).InsPayAmt=50;
			insuranceInfo.ReceiveClaims();
			InsOverpayResult insOverpayResult=PaymentEdit.TransferInsuranceOverpaymentsForFamily(family);
			//Assert that the insurance overpayment does not suggest anything since insurance did not overpay.
			AssertEmptyInsOverpayResult(insOverpayResult);
		...
		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_MultipleClaimProcsOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure1:  Today-1M  provNum1  pat  $100
				Create procedure2:  Today-1M  provNum1  pat  $100
				Create claimproc1:  Today     provNum1  pat  $120
					^InsPayAmt overpaid procedure1 by $20
				Create claimproc2:  Today     provNum1  pat  $130
					^InsPayAmt overpaid procedure2 by $30
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("MCPO1");
			ProcedureCode procedureCode2=ProcedureCodeT.CreateProcCode("MCPO2");
			...
			Procedure procedure1=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			...
			insuranceInfo.CreateClaim();
			//Have insurance overpay the procedure.
			insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure1.ProcNum).InsPayAmt=120;
		...
		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_MultipleClaimProcsNotOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure1:  Today-1M  provNum1  pat  $100
				Create procedure2:  Today-1M  provNum1  pat  $100
				Create claimproc1:  Today     provNum1  pat  $100
					^InsPayAmt pays procedure1 exactly what is owed
				Create claimproc2:  Today     provNum1  pat  $100
					^InsPayAmt pays procedure2 exactly what is owed
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("MCPNO1");
			ProcedureCode procedureCode2=ProcedureCodeT.CreateProcCode("MCPNO2");
			Benefit benefit1=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Benefit benefit2=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode2.CodeNum,100);
			Procedure procedure1=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure procedure2=ProcedureT.CreateProcedure(patient,procedureCode2.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
		...
		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_MultipleClaimProcsUnderpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure1:  Today-1M  provNum1  pat  $100
				Create procedure2:  Today-1M  provNum1  pat  $100
				Create claimproc1:  Today     provNum1  pat  $20
					^InsPayAmt pays procedure1 less than is owed
				Create claimproc2:  Today     provNum1  pat  $30
					^InsPayAmt pays procedure2 less than is owed
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("MCPU1");
			ProcedureCode procedureCode2=ProcedureCodeT.CreateProcCode("MCPU2");
			Benefit benefit1=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Benefit benefit2=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode2.CodeNum,100);
			Procedure procedure1=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure procedure2=ProcedureT.CreateProcedure(patient,procedureCode2.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			...
			insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure1.ProcNum).InsPayAmt=20;
			insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure2.ProcNum).InsPayAmt=30;
			insuranceInfo.ReceiveClaims();
			InsOverpayResult insOverpayResult=PaymentEdit.TransferInsuranceOverpaymentsForFamily(family);
			//Assert that the insurance overpayment does not suggest anything since insurance did not overpay.
			AssertEmptyInsOverpayResult(insOverpayResult);
		...
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_SingleProcPayAsTotalOverpaid() {
			/*****************************************************
				...
				Create procedure:  Today-1M  provNum1  pat  $100
				Create claimproc:  Today     provNum1  pat  $150
					^PayAsTotal (ProcNum=0) where InsPayAmt overpays claim by $50
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			long provNum1=ProviderT.CreateProvider($"1-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("SPPATO");
			Benefit benefit=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Procedure procedure=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			insuranceInfo.ComputeEstimatesForProc(procedure);
			insuranceInfo.CreateClaim();
			//Have insurance overpay the claim via a PayAsTotal.
			insuranceInfo.AddPayAsTotal(150);
			insuranceInfo.ReceiveClaims();
			InsOverpayResult insOverpayResult=PaymentEdit.TransferInsuranceOverpaymentsForFamily(family);
			//Assert that the insurance overpayment was converted into unearned patient income.
			Assert.AreEqual(insOverpayResult.ListPayNumPaySplitsGroups.Count,1);
			List<PaySplit> listPaySplits=insOverpayResult.ListPayNumPaySplitsGroups[0].ListPaySplits;
			//Assert that the supplemental claimprocs are exactly as anticipated.
			Assert.AreEqual(2,insuranceInfo.ListAllClaimProcs.Count);
			ClaimProc claimProc0=insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==0);//PayAsTotal
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc0.PatNum
				&& x.ClinicNum==claimProc0.ClinicNum
				&& x.ProcNum==claimProc0.ProcNum
				&& x.ProvNum==claimProc0.ProvNum
				&& x.Status==ClaimProcStatus.Supplemental
				&& x.InsPayAmt==-150
				&& x.WriteOff==0
				&& x.IsTransfer==false));
			ClaimProc claimProc1=insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure.ProcNum);
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc1.PatNum
				&& x.ClinicNum==claimProc1.ClinicNum
				&& x.ProcNum==claimProc1.ProcNum
				&& x.ProvNum==claimProc1.ProvNum
		...
		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_MultipleProcPayAsTotalOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure1:  Today-1M  provNum1  pat  $100
				Create procedure2:  Today-1M  provNum2  pat  $200
				Create procedure3:  Today-1M  provNum3  pat  $300
				Create procedure4:  Today-1M  provNum4  pat  $400
				Create procedure5:  Today-1M  provNum5  pat  $500
				Create claimproc1:  Today     provNum1  pat  $0
				Create claimproc2:  Today     provNum2  pat  $0
				Create claimproc3:  Today     provNum3  pat  $0
				Create claimproc4:  Today     provNum4  pat  $0
				Create claimproc5:  Today     provNum5  pat  $0
				Create claimproc6:  Today     provNum0  pat  $2000
					^PayAsTotal that overpays the entire claim by $500
			******************************************************/
			string suffix=MethodBase.GetCurrentMethod().Name;
			Patient patient=PatientT.CreatePatient(suffix);
			Family family=Patients.GetFamily(patient.PatNum);
			...
			long provNum2=ProviderT.CreateProvider($"2-{suffix}");
			long provNum3=ProviderT.CreateProvider($"3-{suffix}");
			long provNum4=ProviderT.CreateProvider($"4-{suffix}");
			long provNum5=ProviderT.CreateProvider($"5-{suffix}");
			InsuranceInfo insuranceInfo=InsuranceT.AddInsurance(patient,suffix);
			ProcedureCode procedureCode1=ProcedureCodeT.CreateProcCode("MPPATO1");
			ProcedureCode procedureCode2=ProcedureCodeT.CreateProcCode("MPPATO2");
			ProcedureCode procedureCode3=ProcedureCodeT.CreateProcCode("MPPATO3");
			ProcedureCode procedureCode4=ProcedureCodeT.CreateProcCode("MPPATO4");
			ProcedureCode procedureCode5=ProcedureCodeT.CreateProcCode("MPPATO5");
			Benefit benefit1=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode1.CodeNum,100);
			Benefit benefit2=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode2.CodeNum,100);
			Benefit benefit3=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode3.CodeNum,100);
			Benefit benefit4=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode4.CodeNum,100);
			Benefit benefit5=BenefitT.CreatePercentForProc(insuranceInfo.PriInsPlan.PlanNum,procedureCode5.CodeNum,100);
			Procedure procedure1=ProcedureT.CreateProcedure(patient,procedureCode1.ProcCode,ProcStat.TP,"",100,procDate:DateTime.Now.AddMonths(-1),provNum:provNum1);
			Procedure procedure2=ProcedureT.CreateProcedure(patient,procedureCode2.ProcCode,ProcStat.TP,"",200,procDate:DateTime.Now.AddMonths(-1),provNum:provNum2);
			Procedure procedure3=ProcedureT.CreateProcedure(patient,procedureCode2.ProcCode,ProcStat.TP,"",300,procDate:DateTime.Now.AddMonths(-1),provNum:provNum3);
			Procedure procedure4=ProcedureT.CreateProcedure(patient,procedureCode2.ProcCode,ProcStat.TP,"",400,procDate:DateTime.Now.AddMonths(-1),provNum:provNum4);
			Procedure procedure5=ProcedureT.CreateProcedure(patient,procedureCode2.ProcCode,ProcStat.TP,"",500,procDate:DateTime.Now.AddMonths(-1),provNum:provNum5);
			insuranceInfo.ComputeEstimatesForProcs(new List<Procedure>() { procedure1,procedure2,procedure3,procedure4,procedure5 });
			insuranceInfo.CreateClaim();
			insuranceInfo.AddPayAsTotal(2000);
			insuranceInfo.ReceiveClaims();
			InsOverpayResult insOverpayResult=PaymentEdit.TransferInsuranceOverpaymentsForFamily(family);
			//Assert that the insurance overpayment was converted into unearned patient income.
			Assert.AreEqual(insOverpayResult.ListPayNumPaySplitsGroups.Count,1);
			...
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc2.PatNum
				...
				&& x.WriteOff==0
				&& x.IsTransfer==false));
			ClaimProc claimProc3=insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure3.ProcNum);
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc3.PatNum
				&& x.ClinicNum==claimProc3.ClinicNum
				&& x.ProcNum==claimProc3.ProcNum
				&& x.ProvNum==claimProc3.ProvNum
				&& x.Status==ClaimProcStatus.Supplemental
				&& x.InsPayAmt==300
				&& x.WriteOff==0
				&& x.IsTransfer==false));
			ClaimProc claimProc4=insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure4.ProcNum);
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc4.PatNum
				&& x.ClinicNum==claimProc4.ClinicNum
				&& x.ProcNum==claimProc4.ProcNum
				&& x.ProvNum==claimProc4.ProvNum
				&& x.Status==ClaimProcStatus.Supplemental
				&& x.InsPayAmt==400
				&& x.WriteOff==0
				&& x.IsTransfer==false));
			ClaimProc claimProc5=insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure5.ProcNum);
			Assert.AreEqual(1,insOverpayResult.ListClaimProcSupplementals.Count(x => x.PatNum==claimProc5.PatNum
				&& x.ClinicNum==claimProc5.ClinicNum
				&& x.ProcNum==claimProc5.ProcNum
				&& x.ProvNum==claimProc5.ProvNum
				&& x.Status==ClaimProcStatus.Supplemental
				&& x.InsPayAmt==500
				&& x.WriteOff==0
				&& x.IsTransfer==false));
		...
		[TestMethod]
		public void PaymentEdit_TransferInsuranceOverpaymentsForFamily_MultipleProcPayAsTotalWithExplicitOverpaid() {
			/*****************************************************
				Create Provider: provNum1
				Create Patient: pat
				Create procedure1:  Today-1M  provNum1  pat  $100
			...
			******************************************************/
			...
			insuranceInfo.ComputeEstimatesForProcs(new List<Procedure>() { procedure1,procedure2,procedure3,procedure4,procedure5 });
			insuranceInfo.CreateClaim();
			//Overpay procedure 3 explicitly by $50
			insuranceInfo.ListAllClaimProcs.First(x => x.ProcNum==procedure3.ProcNum).InsPayAmt=350;
			//Make sure that the entire claim (valued at $1500) is completely overpaid by $500.
			//However, procedure3 is already overpaid by $50 (valued at $300 and was paid $350).
			//Therefore, the PayAsTotal claimproc needs to be $2000 (ultimate goal) - $350 (already paid) = $1650
			insuranceInfo.AddPayAsTotal(1650);
			insuranceInfo.ReceiveClaims();
			InsOverpayResult insOverpayResult=PaymentEdit.TransferInsuranceOverpaymentsForFamily(family);
			//Assert that the insurance overpayment was converted into unearned patient income.
			Assert.AreEqual(insOverpayResult.ListPayNumPaySplitsGroups.Count,1);
		...
		{
			return PaymentEdit.ConstructListCharges(listPatNums??new List<long>(),
				listProcs??new List<Procedure>(),
		...
		private void AssertEmptyInsOverpayResult(InsOverpayResult insOverpayResult) {
			Assert.IsTrue(insOverpayResult.ListClaimProcSupplementals.IsNullOrEmpty());
			Assert.IsTrue(insOverpayResult.ListPayNumPaySplitsGroups.IsNullOrEmpty());
			Assert.IsTrue(insOverpayResult.StringBuilderErrors.Length==0);
			Assert.IsTrue(insOverpayResult.StringBuilderVerbose.Length==0);
		}
