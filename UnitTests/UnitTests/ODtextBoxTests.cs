			...
			using(ODtextBox textBox=GetTextBox(text,50,500)) {
				...
				for(int i=1;i<=13;i++) {//There are 13 lines in total.
					...
					if(i<=7) {
						...
						if(i < 7) {
							Assert.AreEqual(spellCheckResult.WavyLineArea.ListWavyLines[i-1].ListPointFs.Count,26);
						}
						else {//The last wavy line will have less points because it isn't that long of a wavy line.
							Assert.AreEqual(spellCheckResult.WavyLineArea.ListWavyLines[i-1].ListPointFs.Count,2);
						}
					}
						...
						if(i < 13) {
							...
							Assert.AreEqual(spellCheckResult.WavyLineArea.ListWavyLines[i-1].ListPointFs.Count,16);
						}
					}
				}
			}
