...
namespace UnitTests.Etrans_Tests {
	...
	public class EtransTests:TestBase {
		...
		public void X834_ImportInsurancePlans_DoNotReplacePatPlan() {
			...
			InsuranceInfo insuranceInfoOld=InsuranceT.AddInsurance(patient,"Old Carrier"+suffix);
			//Create x834
			X834 x834=new X834(new X12object(Properties.Resources.x834Test));
			PatientFor834Import patientFor834Import = new PatientFor834Import(patient);
			List<PatientFor834Import> patientsfor834Imports=new List<PatientFor834Import>{patientFor834Import };
			EtransL.ImportInsurancePlansReturnData importInsurancePlansReturnData=new EtransL.ImportInsurancePlansReturnData();
			importInsurancePlansReturnData=EtransL.ImportInsurancePlansUsingPatientLimited(x834,patientsfor834Imports,doCreatePatient:true, doCreateEmployer:false, doDropExistingInsurance:false);
			...
			Assert.AreEqual(0,importInsurancePlansReturnData.createdPatsCount);
			//There should be two now as the old was kept.
			Assert.AreEqual(2,listPatPlans.Count);
			Assert.AreEqual(1,importInsurancePlansReturnData.createdPatPlanCount);
		}
		...
		public void X834_ImportInsurancePlans_ReplacePatPlan() {
			...
			List<PatientFor834Import> patientsfor834Imports=new List<PatientFor834Import>{patientFor834Import };
			EtransL.ImportInsurancePlansReturnData importInsurancePlansReturnData=new EtransL.ImportInsurancePlansReturnData();
			importInsurancePlansReturnData=EtransL.ImportInsurancePlansUsingPatientLimited(x834,patientsfor834Imports, doCreatePatient:true, doCreateEmployer:false, doDropExistingInsurance:true);
			//Get the pat plans for this patient from the database.
			List<PatPlan> listPatPlans=PatPlans.GetPatPlansForPat(patient.PatNum);
			Assert.AreEqual(1,listPatPlans.Count);
			Assert.AreEqual(1,importInsurancePlansReturnData.droppedPatPlanCount);
			...
			Assert.AreNotEqual(insuranceInfoOld.PriInsPlan.PlanNum,subForPatPlan.PlanNum);
		}
		...
		public void X834_ImportInsurancePlans_ReplaceSecondaryPatPlan() {
			...
			InsuranceInfo insuranceInfoPrimary=InsuranceT.AddInsurance(patient,"TestCarrierNew",ordinal:1,subscriberID:"CG00000B");
			//Create secondary insurance that does not appear in the 834.
			InsuranceInfo insuranceInfoSecondary=InsuranceT.AddInsurance(patient,"Secondary Carrier"+suffix,ordinal:2);
			//Get the pat plans for this patient from the database.
			List<PatPlan> listPatPlans=PatPlans.GetPatPlansForPat(patient.PatNum);
			Assert.AreEqual(2,listPatPlans.Count);
			//Create x834
			...
			Assert.AreEqual(1,importInsurancePlansReturnData.droppedPatPlanCount);
			//These should be different as a new plan was created and the old plan was dropped.
			Assert.AreEqual(insuranceInfoPrimary.ListPatPlans[0].PatPlanNum,listPatPlans[0].PatPlanNum);
		}
